...
The production presentation registry SHALL register `creation` schema version 5. Its available
...
`schema_version` SHALL be integer 5. The `proposal` key SHALL be present only while the
...
- **THEN** `creation` reports `available` true, `kind` `creation`, schema version 5, the preset
...
- **WHEN** a `creation` panel payload declares `schema_version` 4 (the pre-age-bounds prior
  version), 3, 2, or any other version
