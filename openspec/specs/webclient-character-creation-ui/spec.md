...
- **WHEN** a custom draft is saved with a name containing a markup delimiter, an underage value, an unknown race, an incompatible or missing subrace, an over-bound background, allocations outside the profile bounds or budget, or an affinity set that violates the race-dependent maximum
