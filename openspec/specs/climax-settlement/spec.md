# climax-settlement Specification

## Purpose

Close the `climax_phase` dead end (an entity in `進行中` must always resolve within finite
settlement time), add the climax-extension mechanic (staging, consumption, half-cost SP), and add
the two lifetime climax counters plus the male-male experience rule.
