...
Define deterministic, finite-stock merchant trading and clock-driven restocking.
...
`SHOP_REGISTRY` SHALL contain frozen definitions with stable identity, merchant component key, and
immutable offered item keys. Exact integer buy/sell copper, max/initial stock, restock quantity, and
opening/restock hours SHALL come from `guild_economy.yaml`. Loading SHALL join both sources and reject
unknown, missing, or extra references, floats, negative prices, sell above buy, buy outside the referenced
`PRICE_TABLE` range, and stock outside `0 <= initial <= max` with positive restock quantity.
