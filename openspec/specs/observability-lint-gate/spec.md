...
Define the stdlib-AST lint gate (`tools/observability_lint`) that keeps the facade the sole log writer, requires swallowed exceptions to re-raise, log, or carry a reasoned exemption, and ratchets the shrink-only freeze list to empty.
