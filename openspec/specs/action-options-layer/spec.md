...
The generative half of the AI action-options surface: the bounded-context serializer
(`build_options_context` with the fixed truncation policy and the composed `LEAK_BLOCKLIST`),
the prompt assembly through the two `action_options` prompt-library keys, and the guarded
generation pipeline (`generate_action_options`) that resolves a frozen `OptionSet` or the
`None` degrade marker, plus the atomic idempotent guardrail registration
(`register_action_options`) and the strict proposal-only boundary. This capability consumes
the vocabulary/ladder from `ai-action-options-schema`, the prompt/profile files from
`ai-action-options-prompts`, and the canonical affordance tuple from
`action-options-affordance-contract`; the trigger service (`action-options-trigger-service`)
and webclient surfaces build on it.
