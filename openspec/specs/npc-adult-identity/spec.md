...
Guarantee canonical adult identity attributes on procedurally spawned and synced
NPCs, enforcing the project's mandatory-adult invariant on production paths that
do not go through import or characterization.

## Requirements

### Requirement: Procedurally spawned NPCs carry canonical adult identity

The system SHALL initialize `age` and `apparent_age` to 18 on any synced or
spawned NPC that would otherwise lack them, as part of the spawn/sync flow.
