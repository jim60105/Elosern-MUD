...
change 11b's growth multiplier query at the moment progression is computed), and SHALL
...
- **WHEN** `tick_buffs(entity)` is called on an entity holding an active `conferred_growth_rate` buff
- **THEN** `entity.traits.magic_power.value` is unchanged before and after the call, and no other
