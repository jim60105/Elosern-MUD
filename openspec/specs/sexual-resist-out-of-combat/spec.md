# sexual-resist-out-of-combat Specification

## Purpose

Define the deterministic affinity consequence of a forced sexual act cast outside combat: an
out-of-combat coercion scan that penalizes exactly the forced outcome against the cast's own explicit
target list, running inside the settlement's outer transaction with rollback-safe snapshotting of the
touched relations and party surfaces. The capability extends `sexual-resist-turn-cost`'s in-combat
penalty to `settle_out_of_combat_cast`, delivers the auto-leave notification to the casting player, and
keeps malformed or foreign resist entries inert.
