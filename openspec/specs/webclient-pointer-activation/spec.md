...
The project's key handling SHALL be exposed as the plugin `onKeydown` hook rather than bound directly to the document, and SHALL return a claim exactly when the router consumed the event or when the open command drawer owns the key. The project plugin SHALL be ordered after the stock plugins so existing command-history recall keeps its turn. A modal capture that must pre-empt the router — such as the exploration dock's bounded rest-duration entry — MAY use a capture-phase listener, and SHALL remove it when its form closes.

#### Scenario: No unhandled-keydown noise remains
- **WHEN** the player navigates the action dock and types in the command drawer
- **THEN** the plugin handler reports no unhandled keydown, because the project plugin claims exactly the events its router consumed and the keys its open drawer owns

#### Scenario: Unclaimed keys still reach the stock plugins
- **WHEN** the player uses the stock command-history recall keys in the command drawer
- **THEN** the project plugin does not claim them and history recall works
