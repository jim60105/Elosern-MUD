...
- **WHEN** the minimap renders as the stage's minimap island at 1440x900 and at 1280x720, on a lattice payload with named edge markers and on a graph payload with remembered nodes
- **THEN** the canvas renders at exactly 208 × 208 CSS px, the title, orientation marks, marker names, and readout line remain readable, no node marker, marker name, or edge overprints other island content, the island's rendered box is identical for both payloads, and no required island content has to be scrolled to
...
- **WHEN** the in-view lattice is wide or tall enough that its derived minimum pitch does not fit the island's 208px square, so the square extent grows and the drawing scales down uniformly
- **THEN** the pre-scale geometry already satisfies the non-overlap invariant, so the uniformly scaled render remains free of marker/label collisions; while the scale stays at or above 0.75 every in-view node remains on the canvas, and below that floor (for example the two-column, sixty-four-row placement's 2574-unit square) the island draws at exactly 0.75 through a window centred on the current node and clamped inside the drawing, clipping the rest
...
- **THEN** no scale above 1 is produced at all: the island's fill is taken first as pitch grown from the derived minimum and then as symmetric coordinate margin around the node core, never as magnification, so the drawn scale is `208 / side ≤ 1` and the upward direction of the non-overlap question cannot arise
...
- **THEN** the island lays out no remembered-node list at all, its canvas is still 208 × 208 CSS px, its title and readout row keep their places, the island's size equals its size for the same placement with no remembered node, and no required island content has to be scrolled out of view
...
- **THEN** the drawn square pitch is 59 user units — the largest whole unit at which three columns, three rows, and the 14-unit label band fit the 208px square less its 8px inset, and below the 60-unit cap of one and a half times the declared pitch — so the node core spans 177 of the canvas's 208 CSS px instead of 120, the remaining room is painted as coordinate margin by the dot field registered to the 59-unit pitch, the uniform scale is exactly 1, and every marker radius and label renders at the size the island declares
...
- **THEN** the pitch grows only to its 60-unit cap of one and a half times the declared 40-unit pitch, the rest of the 208px square is coordinate margin, so roughly three and a half coordinate cells of dot field surround one marker drawn at its designed radius with its label at the island's declared 9-unit type size — exactly 9 CSS px, below the island's own 10px chrome step — and neither the marker ladder nor the label is inflated
...
- **THEN** the island's rendered width and height are identical for every one of them and its canvas is 208 × 208 CSS px each time

#### Scenario: A long authored title never reflows the island's header
- **WHEN** the committed payload's server-authored title is long enough that the title and the orientation marks together exceed the island's content width (e.g. `冒險者公會外街道圖` beside the axis marks in the island's 208px content box)
...
- **THEN** the drawn node label is at most the island's declared 9-unit type size — about 8.40 CSS px (9 × 208 / 222.91) and exactly 9.00 CSS px respectively, both below the island's own 10px chrome step — and no payload produces a node label drawn larger than the island's title

#### Scenario: The pitch clears exactly what the drawn label set needs
- **WHEN** a wilderness payload whose neighbouring cells state no place name of their own is laid out, and then a payload in which two horizontally adjacent cells both draw visible label text is laid out
- **THEN** the first's derived minimum pitch is the pitch the bare clearance term requires — the two markers' widest drawn footprints, a visible connector segment, and each label box clear of its own node and of the row beneath it — while the second's minimum rises so both truncated label boxes clear side by side; on the island either may then be drawn at a larger pitch to fill the square but never at a smaller one, and in every rendering every marker/marker, marker/label, and label/label pair keeps the geometry audit's minimum gap with `labelMax` unchanged
