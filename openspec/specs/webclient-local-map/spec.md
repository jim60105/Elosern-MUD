...
- **WHEN** the in-view lattice is wide or tall enough that its derived minimum pitch does not fit the island's 208px square, so the square extent grows and the whole drawing scales down uniformly
- **THEN** the pre-scale geometry already satisfies the non-overlap invariant, so the uniformly scaled render remains free of marker/label collisions and every in-view node remains on the canvas
