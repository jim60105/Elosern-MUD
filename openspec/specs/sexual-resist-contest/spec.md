# sexual-resist-contest Specification

## Purpose

Define the deterministic resist verdict — contest formula, affinity-stage modifier and
`auto_comply` table, and the climax-turn short circuit — through which a target's compliance or
refusal of a bidirectional sexual act resolves. The capability ships a pure, two-party
`resist_verdict()` function and its `sexual_resist.yaml` rulebook only; the acts that call it and
the turn-cost wiring that spends turns on its outcome land in later proposals.
