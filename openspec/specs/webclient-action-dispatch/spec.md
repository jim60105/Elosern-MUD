...
#### Scenario: Production registry exposes only specified combat, service, and creation mutations
- **WHEN** the production registry is loaded after the character-creation change
- **THEN** its action IDs are exactly `combat.cast`, `combat.flee`, `combat.forfeit`, `guild.register`, `guild.quest_accept`, `guild.quest_abandon`, `guild.quest_turnin`, `guild.exam_start`, `shop.buy`, `shop.sell`, `creation.preset`, `creation.custom`, `creation.activate`, and `creation.reset`, each with its own exact validator and deterministic adapter
