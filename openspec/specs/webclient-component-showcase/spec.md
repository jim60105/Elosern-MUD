...
SHALL emit `creation.*`. A surface with no backing OOB read model today — a dedicated Party/companion
panel, the intimate/adult status collapsible, a full inventory bag, and the event-log Toasts surface —
MUST NOT be built here or mocked to look real. On completion of the showcase wave the required-component
