...
The deterministic, offline-computable terrain model for the wilderness/Virtual map layer: the seven
named terrain regions of the continent, a pure coordinate-to-region partition, and a pure
coordinate-to-description arithmetic formula — no LLM, no database read, no randomness, no wall-clock
input. These functions satisfy the design doc's offline-playability criterion at the map layer.
