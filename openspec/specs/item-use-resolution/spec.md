...
Every registered item SHALL declare exactly one of a usable-item definition, an equipment-slot definition, or no mechanics. A usable-item definition SHALL contain a registered deterministic effect key, a boolean consumable flag, and a combat-use permission. An equipment definition SHALL contain exactly one `EquipmentSlot`. The registry SHALL reject an item that declares both forms, an unknown effect, or a malformed slot. Presentation kind, icon, rarity, summary, display name, and price SHALL NOT select or modify mechanics.
