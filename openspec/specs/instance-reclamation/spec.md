...
The deterministic TTL reclamation lifecycle of map instance rooms: `default_ttl_seconds` rulebook
data, reason-keyed pinning and owned-entity registration, and `reclaim_due_instances()`'s
defer/promote/despawn-relocate/delete resolution, registered as the `instance_reclamation` clock
stage at server start.
