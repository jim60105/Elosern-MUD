...
`WorldClock.advance()` SHALL settle all per-entity stages and the final `tick` increment inside a
single durable transaction with snapshot/restore of the touched entity attributes, so a process
termination during the call can never leave character state advanced without the matching tick (or
the reverse).
