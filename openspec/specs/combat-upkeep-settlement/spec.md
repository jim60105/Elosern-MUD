...
- **THEN** upkeep skips the non-living target, no additional defeat entry, kill XP, or quest effect is produced, and the round's defeat entries remain exactly the action's

### Requirement: Upkeep kill credit requires validated, resolvable source identity
The upkeep settlement SHALL attribute a lethal rate tick to the entity whose dbref the buff cache stores as `source_pk`, resolved against the battlefield roster or the object database. An attributed lethal tick on a `Monster` with a tier in `MONSTER_TIER_REGISTRY` SHALL stage exactly one `grant_combat_kill_xp()` for the source in the same round commit; a tick with no cached `source_pk`, an unresolvable (deleted or absent) source, a non-Monster target, or a target outside the tier registry SHALL award no XP. An unattributed lethal tick SHALL still apply its HP damage but SHALL emit no EventLog entries and stage no quest effects.

#### Scenario: An attributed lethal tick on a tiered monster awards the caster once
- **WHEN** the player's `fire_scorch` tick causes the lethal HP crossing of a `Monster` with `threat_tier == "low"` during combat upkeep
- **THEN** the player receives exactly one low-tier combat-kill XP award committed with that round
...
- **THEN** the target's HP still crosses, but no EventLog entries, kill XP, or quest effects are produced

#### Scenario: A non-Monster target grants no kill XP
- **WHEN** an attributed lethal tick kills an NPC or companion rather than a `Monster`
- **THEN** no combat-kill XP is staged for the source

### Requirement: Upkeep settlement honors simulated and nonlethal combat policy
`run_round` SHALL accept keyword-only `simulated` and `nonlethal_keys` policy flags and SHALL forward them to the upkeep settlement. In a simulated round (guild examination) the settlement SHALL tag every upkeep `target_defeated` entry `simulated=True` and SHALL stage no kill XP. For a target whose key is in `nonlethal_keys` (an allied companion), an upkeep lethal crossing SHALL floor the target at 1 HP, mark the target knocked out on the battlefield, and emit a `target_knocked_out` entry (the same shape the action path emits for a nonlethal crossing) instead of a `target_defeated` entry, with no kill credit.
...
- **THEN** the defeat entry carries `simulated=True`, no kill XP is staged, and no quest DEFEAT progress or protected-entity failure results
...
Callers of `tick_buffs` outside the combat-round settlement (the world-clock settlement path) SHALL ignore the returned tick records: the records SHALL cause no EventLog entries, no kill XP, and no quest effects on their own.
