...
by the `sexual-act-seeds` change; `interspecies.py` and `divine.py` SHALL remain equal to `()`
after that change. `world/skills/sexual_acts/__init__.py` SHALL import all six and merge their
contents into `SEXUAL_ACT_REGISTRY` and `SKILL_REGISTRY`.
