# altoria-learning-and-exchange Specification

## Purpose
Finish the capital's location roster with its last three unbuilt types: the
王立魔法學院 off 學院前, the 商會公所 off 東市, and the host-less 市集棚 off
市場街. The academy is the capital's lore-reveal location — its host answers
on the magic-rank ladder and the element vocabulary, making conversation the
point of the room rather than a signpost. The change's two load-bearing
refusals are written as rules: the academy implements no apprenticeship,
tutoring or skill-purchase path (skill acquisition runs through the lineage
tree's proficiency accumulation), and the merchant hall implements no escort
commissions or work-offering surface (escort work needs a quest type that
does not exist).
