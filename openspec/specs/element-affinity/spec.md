...
gate-only value: it is never a stored trait, never replaces `magic_level`, and never changes
`magic_level.max` — it exists only for the `can_cast_spell_tier` comparison.
