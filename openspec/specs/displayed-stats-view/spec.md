...
`world/rules/displayed_stats.py` SHALL provide `display_stat_block(entity) -> str | None` that
renders one `label：value` row per key in the fixed order `atk_phys`, `agility`, `defense`,
