...
In this change the violation target pool SHALL be the defeated player
alone: every attempt of every running violator SHALL target the player, and
only the player's own sexual state and counters SHALL be written. Companion
victims (the pool covering non-fled allied participants) are a later
change's extension of this requirement.

#### Scenario: Every executed attempt targets the player
- **WHEN** two violators run sequences with caps 2 and 3 against a defeated player who has one bound companion present
- **THEN** every executed attempt targets the player, the companion's sexual state and counters are untouched, and settlement completes without error
