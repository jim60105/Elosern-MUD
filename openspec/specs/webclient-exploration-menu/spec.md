...
The read-only version-3 `exploration` panel and version-7 `character` panel, the fourteen exact allowlisted exploration adapters, the free-form dialogue composition root with offline degrade, the keyboard-first exploration dock rooted at the scene overview, minimap movement submission, the client-local portrait-focus seam, and the Node/browser acceptance boundary.
