...
spell-tier eligibility; the predicate SHALL NOT read `magic_rank_title`, consult
...
- **WHEN** `can_cast_skill(entity, firestorm)` is called on an entity with
  `magic_level.value == 15`, no declared affinities, and no owned `fire_mastery` (firestorm's 30 MP
...
- **WHEN** `can_cast_skill(entity, firestorm)` is called on an entity with `magic_level.value == 1`
  whose `owned_keys()` contains `"fire_mastery"`
- **THEN** it returns `True`

#### Scenario: A favored affinity boundary passes through the predicate
- **WHEN** `can_cast_skill(entity, firestorm)` is called on an entity with `magic_level.value == 15`
