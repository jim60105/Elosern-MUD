...
forward-declare `InstanceRoom`.

## Requirements


### Requirement: GridRoom is a coordinate-aware room carrying the scene_archetype seam
`typeclasses/rooms.py` SHALL define `GridRoom`, subclassing `evennia.contrib.grid.xyzgrid.xyzroom.
XYZRoom`, with a persistent `scene_archetype: str | None` attribute defaulting to `None`. This value
SHALL NOT be validated against any registry, since no `SceneArchetype` registry exists yet (design
doc D10, change 22).
