...
age baseline — a value always within the zero-to-lifespan-maximum validation range for every
race, so the portrait age check provably passes). An occupant without a portrait policy SHALL never
