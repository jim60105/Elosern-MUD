...
`window.Elosern.narrativeInput` facade as a stream-end block, and SHALL NOT create a separate
append path: narrative text committed after the block was inserted SHALL relocate the block to
the new stream end within the same single scroll/unread decision — the newly appended text lands
before the block, the block stays last, relocating produces no additional scroll event and no
extra unread marker — and the existing scroll-keep and polite unread-marker behavior SHALL count
each committed narrative text event exactly once, unaffected by block movements. The facade SHALL
expose the block operations (attach, move-to-end, replace-in-place, remove) as the single owner
of the stream's end geometry; no other module SHALL mutate the narrative container directly for
the choice-point.

#### Scenario: Text appended after a ready commit moves the block to the new end
...
- **THEN** the newer text appears between the stream's older content and the choice-point block —
  i.e. the block relocates to the very end — and the player can still click the cards
