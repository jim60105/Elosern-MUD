...
- **Dependency-free vanilla text console (D10)** + the `base.html` mutually-exclusive script-load flag
  (Vue bundle XOR legacy plugins, single default, review-window only); the flip-to-Vue is C3.
...
- **Modified:** `web/templates/webclient/base.html` (XOR flag + vanilla console + bundle-load
  infrastructure; the flip-to-Vue mount is C3), `.github/workflows/quality-gate.yml`, `Containerfile` /
