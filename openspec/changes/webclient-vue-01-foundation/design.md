...
XOR flag off by default (legacy) until C3 flips to Vue; rollback is reverting `base.html` script wiring.
No server/protocol/Telnet impact.

## Open Questions

- Exact Containerfile build stage + entrypoint `collectstatic` ordering to serve the `dist` path (confirmed
  during A2 build).
- Exact version pins (Vue 3.5.x, Vite 6.x, Pinia 3.x, Storybook, Vitest, `@vue/test-utils`) locked in the
