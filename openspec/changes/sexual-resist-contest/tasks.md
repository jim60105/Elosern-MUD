...
- [ ] 1.1 Confirm `world/rules/sexual_state.py::SexualState` currently exposes `climax_phase` (an
...
- [ ] 1.2 Confirm `world/rules/disengage.py::_adjusted_agility` and `_attempt_flee` still have their
...
- [ ] 1.2a Confirm `world/rules/combat.py::_adjusted_attack` still adds `atk_phys` as a flat addend
...
- [ ] 1.3 Confirm `world/rules/affinity_config.py::get_config().stages` still returns exactly the
...
- [ ] 1.4 Confirm `typeclasses/npcs.py::NPC` and `typeclasses/monsters.py::Monster` are both
...
- [ ] 2.1 Create the YAML file with `agility_weight`, `atk_phys_weight`,
...
- [ ] 2.2 Write the loader in `world/rules/sexual_resist.py` (or a small private helper it calls)
...
- [ ] 2.3 Load the YAML once at module import time (matching `affinity_config.py`'s and
  `BUFF_DEFINITIONS`'s existing eager-load pattern), not lazily per call.

## 3. `world/rules/sexual_resist.py`: the verdict type and blended score

- [ ] 3.1 Define `ResistVerdict` as a frozen dataclass: `resisted: bool`, `auto_comply: bool`,
  `roll: int | None`, `actor_score: float`, `resister_score: float` (design.md Decision 6).
- [ ] 3.2 Implement a private `_blended_score(entity) -> float` helper, giving each stat **its own**
  ...
  blocking bug in an earlier draft of this task).
- [ ] 3.3 Implement a private `_affinity_term(actor, resister) -> tuple[float, bool]` returning
...
- [ ] 3.4 Implement a private `_climax_turn_short_circuit(resister) -> bool`: `True` when
  `getattr(resister, "sexual", None) is not None` and `resister.sexual.climax_phase.level ==
  "進行中"` and `resister.sexual.climax_turns <= climax_turn_auto_comply_limit`.

## 4. `resist_verdict()` itself

- [ ] 4.1 Implement `resist_verdict(actor, resister, *, rng=roll_d100) -> ResistVerdict`:
...
- [ ] 4.2 Import `roll_d100` from `world.rules.dice` as the default `rng` (spec scenario: default RNG
  is the shipped dice roller, not a private reimplementation).
- [ ] 4.3 Import `COMBAT_YAML` from `world.rules.combat` for `defender_constant` — do not hardcode
  `51` locally.
- [ ] 4.4 Add `resist_verdict`, `ResistVerdict` to this module's `__all__`.

## 5. Tests

- [ ] 5.1 `world/rules/tests/test_sexual_resist.py` (pure `unittest.TestCase`, no Evennia database
...
- [ ] 5.2 One test per spec scenario in `specs/sexual-resist-contest/spec.md` — enumerate them
...
- [ ] 5.3 A dedicated asymmetry test: construct a fixture where swapping `actor`/`resister` changes
  which side gets the affinity or climax-turn short circuit, and assert the verdict actually flips
  (design.md Risk 1).
- [ ] 5.4 A monotonicity test across the five numeric affinity stages (`acquaintance` through
...
- [ ] 5.5 A test asserting `_affinity_term` never reads `.relations` for a `Monster` resister — patch
...
- [ ] 5.6 A test constructing a participant owning `retainer_martial_training` or dual-wielding with
...
- [ ] 5.7 A test asserting a negative-weight `sexual_resist.yaml` (summing to `1.0`) raises at load
  time, distinct from the sum-mismatch case already covered by 5.2's scenario enumeration.
- [ ] 5.8 A test asserting an `NPC` resister paired with a non-player `actor` (another `NPC`, or a
...
- [ ] 6.1 Run `uv run --locked python -m compileall -q world` to catch syntax errors early.
- [ ] 6.2 Run the new test module directly:
  `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --keepdb
  world.rules.tests.test_sexual_resist`.
- [ ] 6.3 Run `uv run --locked python -m tools.spec_traceability check` and address any reported gap
  before finishing.
- [ ] 6.4 Run `openspec validate sexual-resist-contest --strict` and resolve every finding.
- [ ] 6.5 Run `uv run --locked -m unittest discover -s tests -t .` to confirm no repository-wide
