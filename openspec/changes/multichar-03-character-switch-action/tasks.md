...
- [x] 1.1 `web/webclient/actions/account_actions.py`: module docstring stating the
...
- [x] 1.2 Stable codes and Traditional Chinese messages as module constants:
...
- [x] 1.3 `validate_account_character_switch_payload`: exactly `{character_id}`,
  `type(value) is int` (booleans excluded), positive. Raises the module's `AccountActionError` on
  any violation.

## 2. The scheduling seam

- [x] 2.1 Module-level injectable clock following `world/ai/client.py`'s idiom: a private `_clock`
...
- [x] 3.1 `_attach_puppet(session, account, target)`: send `send_unpuppet_transition(session)` →
...
- [x] 3.2 `_recover_transition(session, account, previous, target, cause)`: the three-rung ladder —
...
- [x] 3.3 `_perform_switch(session, account, character_id, previous)`: re-resolve the target inside
  ...
  `account.set_last_puppet(target)` and `synchronize_session(session, target)`; on failure call
  `_recover_transition`.

## 4. Adapter and registration

- [x] 4.1 `_account_character_switch_adapter(actor, payload, session)`: resolve the account from
...
- [x] 4.2 `web/webclient/actions/registry.py`: register `account.character.switch` with empty
  affected_panels, and extend `build_production_action_registry`'s docstring inventory.

## 5. Tests

- [x] 5.1 `web/webclient/actions/tests/test_account_actions.py` — validator: exact payload shape;
  boolean, negative, non-integer, extra, and missing-field rejection.
- [x] 5.2 Decisions with an injected `Clock`, asserting **nothing is scheduled**: foreign id →
...
- [x] 5.3 Acceptance: the adapter returns success and `no_presentation` while the puppet is still
...
- [x] 5.4 Recovery rung 1 — `puppet_object` patched to return silently without releasing the
...
- [x] 5.5 Recovery rung 2 — `puppet_object` patched to release the previous puppet and then return
...
- [x] 5.6 Recovery rung 3 — `puppet_object` patched to fail every time after releasing: the session
...
- [x] 5.7 Late re-validation failure — the target is removed from `account.characters`, or the
...
- [x] 5.8 Ordering test through the real dispatcher: submit a switch and assert the recorded session
  ...
  prevent).
- [x] 5.9 Cross-puppet isolation: committed action-option cards and a transient concept proposal on
  ...
  unchanged.
- [x] 5.10 `covers_requirement` annotations for the six new `webclient-character-roster`
...
- [x] 6.2 `uv run --locked python -m tools.observability_lint check` (the recovery ladder adds
  `log_warn` / `log_error` call sites) and
  `uv run --locked python -m tools.spec_traceability check`.
- [x] 6.3 Live container check: from a browser session with two characters, switch back and forth
