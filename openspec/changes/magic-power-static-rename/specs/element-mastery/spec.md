...
**Reason**: the magic-rank display ladder (學徒/術師/大師/賢者/主宰) names the XP
system being retired; `magic-xp-engine-retirement` deletes the numeric gate that fed
it and the title-system change line (`title-fixed-core`) owns title presentation.
**Migration**: `magic_rank_title()` and `MAGIC_RANK_BANDS` are deleted; the WebClient
head card's rank line becomes guild-rank-only (see the `webclient-contextual-hud`
delta in this change). No replacement display exists until the title line lands.
