...
- [x] 1.1 In `world/lore/races.py`: make `StaticBand` four-dimensional
...
- [x] 1.2 In `world/lore/races.py`: add `magic_band: tuple[int, int]` to
      ...
      race band.
- [x] 1.3 In `world/lore/monsters.py`: every `MonsterTier.static_band` gains
      `magic_power=(0, 0)`.
- [x] 1.4 In `world/lore/magic.py`: delete `RankTitle` and
...
- [x] 2.1 `STATIC_KEYS = ("atk_phys", "agility", "defense", "magic_power")`,
...
- [x] 2.2 `trait_config_for_values(values)` loses the `magic_cap` parameter
...
- [x] 3.1 Rename every `magic_level` read in `world/rules/combat.py`
...
- [x] 3.2 `world/rules/status_query.py`, `world/rules/status_text.py`,
...
- [x] 3.3 `world/rules/progression.py`: delete `MAGIC_RANK_BANDS` and
      ...
      and re-key both actual callers (`accrue_magic_study` and
      `grant_combat_kill_xp` — `grant_skill_practice_xp` stays independent of
      the multiplier in this interim change); keep
      `MAGIC_TIER_THRESHOLDS`, `can_cast_spell_tier`, `can_cast_skill`
      reading `entity.traits.magic_power`.
- [x] 3.4 `world/rules/buffs.py`: `_NO_OP_RATE_TARGETS = {"skill_practice"}`;
      docstring naming the pull reader; extend `load_buff_definitions` to
      fail closed at load on any `rate.target` outside the gauge keys plus
      `"skill_practice"` (so a stale `magic_level_growth` target is rejected
      at buff-table load, per design doc §7); update `buffs.yaml` conferred
      growth-rate `rate` target to `skill_practice` and re-key the
      `bounds` list entries targeting `magic_level` to `magic_power`.

## 4. Skills

- [x] 4.1 `world/skills/effects.py`: `GrowthRateEffect` accepts stat key
      `practice` only; `growth_rate:magic:*` raises at parse (fail-closed
      registry load).
- [x] 4.2 `world/skills/registry.py`: `reincarnation_boon_elosia` effect
...
- [x] 5.1 `schema.py`: `stats`/`disguised_stats` vocabulary renames
      `magic_level` → `magic_power` (eight keys, count unchanged).
- [x] 5.2 `validate.py`: above `static_baseline.magic_power[1]` rejects with
      `Issue("stats.magic_power", ...)`; below `[0]` warns like the other
      static axes; disguised subset check keys follow.
- [x] 5.3 `examples/example_character.json`: `"magic_power": 60`.

## 6. Creation (rules + AI + commands)

- [x] 6.1 `world/rules/character_creation.py`: delete
...
- [x] 6.2 `world/ai/character_creation.py`: duplicated `ALLOCATABLE_AXES`
      gains `magic_power` (mirror test keeps pinning equality).
- [x] 6.3 `commands/character_creation.py` + `web/webclient/actions/
...
- [x] 7.1 `world/quests/scene_builder.py`: NPC stats come from
      `build_initial_traits(race_key, tier=...)` only (no
      `starting_magic_level` read).
- [x] 7.2 Persona no-leak secret set (`world/ai/`): true-value keys rename to
      `magic_power`.

## 8. WebClient Python mirrors

- [x] 8.1 `web/webclient/presentation/character.py`: trait labels/keys;
      head-card presenter no longer sends any magic-rank word.
- [x] 8.2 `web/webclient/actions/tests/test_service_actions.py` trait-key
...
- [x] 9.1 `character-identity.js`: delete `magicRankTitle` and its band
...
- [x] 9.2 `CharacterStatusDrawer.vue`: `ATTRIBUTE_KEYS` renames the fourth
      key; delete `TRAIT_LABEL_OVERRIDES` (server label is now 魔力).
- [x] 9.3 `CreationOverlay.vue`: allocation reactive object, zeroing, draft
      hydration, briefing, and submit payload gain `magic_power` (seventh
      axis); profile axes come from the server descriptor.
- [x] 9.4 `stories/fixtures.js`, `stories/Data/CharacterHead.stories.js`,
...
- [x] 10.1 `docs/game/command-reference.md`: look-block stat wording row
...
- [x] 11.1 `world/rules/tests/`: `test_traits.py` (four-axis bands, static
...
- [x] 11.2 `typeclasses/`, `commands/tests/test_background.py` (floor +
      allocation value assertion), `web.webclient` creation suites field
      lists.

## 12. Verification

- [x] 12.1 `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --keepdb world.rules world.imports world.skills typeclasses commands web.webclient`
- [x] 12.2 `npm test` (vitest suites above).
- [x] 12.3 `uv run --locked python -m tools.spec_traceability check`
- [x] 12.4 `uv run --locked python -m compileall -q world typeclasses commands server`
- [x] 12.5 `openspec validate magic-power-static-rename --strict`
- [x] 12.6 `git diff --check`
