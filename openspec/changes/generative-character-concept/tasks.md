...
- [ ] 1.1 Extend `world/prompts/registry.py`'s `character_creation.system` spec: allowlist gains
      `concept` and `race_catalog` placeholders; remove the forward-declared-seam comment
- [ ] 1.2 Update `prompts/character_creation.yaml`: template text uses `{concept}` and
      `{race_catalog}`, keeps the adult framing, and states the output contract (only real
      registry keys, no numbers, no age)
- [ ] 1.3 Add a `race_catalog` renderer (deterministic, registry-derived, hard size bound) that
...
- [ ] 2.1 Add `character_creation` to `world/ai/profiles.py::LAYER_NAMES`; confirm
      `default_profiles()` in `server/conf/settings.py` constructs the profile without manual
      entries
- [ ] 2.2 Create the `world/ai/` layer module for character creation: output jsonschema for the
...
- [ ] 2.3 Register the layer through the guardrail's existing registration mechanism
      (`register_semantic_validator` / degrade fallback / schema), keeping `world/ai/` free of
      writer and typeclass imports
- [ ] 2.4 Add the idempotent `character_creation` startup registration helper and call it from
      ...
      startup path
- [ ] 2.5 Reuse `preflight_character_creation`-style checks for allocations/race validation where
      possible instead of duplicating band logic

## 3. Command surface

- [ ] 3.1 Add `character concept <構想>` (aliases 構想) to the pending-character command set:
...
- [ ] 3.2 Confirm the ordinary custom flow, activation semantics, and the adult gate are untouched
      (no new path around them); no draft, persona, or suggested-skill persistence in this change
- [ ] 3.3 Update `docs/game/commands.md`, `docs/game/command-reference.md`, and
      `tests/test_command_docs.py` for the new command surface

## 4. Tests

- [ ] 4.1 Layer tests with `FakeLLMClient` replays: valid proposal accepted; unregistered
      ...
      touching the DB
- [ ] 4.2 Command integration tests: concept → summary → interactive name/ages → activation;
...
- [ ] 4.3 Startup test: `at_server_start()` installs the `character_creation` schema, validators,
      and degrade fallback; a broken `character_creation.system` key degrades the layer without
      blocking startup
- [ ] 4.4 Prompt-library tests: `character_creation.system` allowlist contains `concept` and
      `race_catalog`; template text lives only in `prompts/*.yaml`
- [ ] 4.5 Offline-playability regression: with every LLM profile failing, `character concept`
...
- [ ] 5.2 Run `uv run --locked python -m tools.spec_traceability check` and confirm the
      ...
      covered
- [ ] 5.3 Run the focused test packages (world ai, world prompts, commands creation tests) and
