...
SHALL be a logged warning that never blocks startup. `render_prompt()` SHALL trigger a one-time
auto-load on first use when no explicit load happened, and `reset_prompt_library()` SHALL clear
the loaded registry for tests.
