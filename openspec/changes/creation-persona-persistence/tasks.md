...
- [ ] 1.1 Extend the creation-wizard draft service (`world/rules/creation_wizard.py` and
...
- [ ] 1.2 Implement the deterministic concept-apply service (`apply_concept_proposal`): re-validates
...
- [ ] 1.3 Custom-save persona rule: `save_custom_draft` preserves the concept draft's persona block
      only when the submitted race equals the concept draft's race; otherwise clears it
- [ ] 1.4 Confirm `clear_draft` and activation clear the concept stage and persona block atomically;
      non-concept drafts behave byte-identically

## 2. Activation persona write

- [ ] 2.1 Extend `activate_player_character()`: inside the existing `transaction.atomic()` block,
      ...
      block
- [ ] 2.2 Confirm a persona write failure rolls back the whole activation (character stays
      pending, no canonical identity/trait/persona state written)
- [ ] 2.3 Confirm `world/imports/loader.py` remains unchanged and the import-time persona write is
      untouched

## 3. WebClient concept surface

- [ ] 3.1 Register the fifth creation action `creation.concept` in the production action registry
...
- [ ] 3.2 Add the creation-panel concept text field (bounded, keyboard-first) that submits
...
- [ ] 3.3 Retro-fit the Telnet `character concept` command (from generative-character-concept) to
...
- [ ] 4.1 Draft tests: concept stage persists across reconnect; persona block cleared on race
...
- [ ] 4.2 Apply-service tests: fingerprint mismatch returns stale and writes nothing (simulate a
...
- [ ] 4.3 Activation tests: concept draft persists persona in the six-key import-card shape with
      `{}` containers; persona write failure rolls back activation; no-block draft writes nothing;
      adult gate regression
- [ ] 4.4 Adapter/panel tests: `creation.concept` success fills the draft; offline degrade with no
...
- [ ] 4.5 Browser acceptance: keyboard-only concept → draft → complete form → activate journey at
...
- [ ] 5.3 Run the focused test packages (world rules creation tests, web webclient actions and
