...
- [ ] 1.1 `web/webclient/presentation/local_map.py`: delete key/alias-based `_gate_direction`
...
- [ ] 1.2 Verify (do not special-case) that wilderness adjacency emits no node/edge for
...
- [ ] 2.1 Extend `web/webclient/presentation/tests/test_local_map.py`: footprint never appears
...
- [ ] 2.2 `web/tests/browser/seed.py`: replace `entry.wilderness_xy` with a gate approach cell
      ...
      class locally).
- [ ] 2.3 Audit Vitest fixtures/stories that pin gate-node labels or the legend/state list for
      now-impossible single-gate payloads; `npm test` green.

## 3. Traceability + gates

- [ ] 3.1 Annotate requirement-owning tests with `covers_requirement` literal IDs from
      ...
      (expected: none).
- [ ] 3.2 Focused runs green: `web.webclient` presentation tests; no player-command surface
