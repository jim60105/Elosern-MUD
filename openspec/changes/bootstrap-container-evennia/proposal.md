...
- Add a multi-stage `Containerfile`: builder stage installs dependencies with BuildKit cache
  mounts; runtime stage carries only the venv and application code, runs as a non-root,
  arbitrary-UID-capable user (group-0 writable, OpenShift-compatible).
- Add `compose.yaml` defining the `evennia` service (ports 4000 telnet / 4001 webserver / 4002
  websocket; volumes for the SQLite DB, scene art store, and `server/logs`); Ollama and sd-webui
  are reached over the network (`extra_hosts` / environment variables), never bundled into the
  image.
- Add `.dockerignore` excluding VCS, caches, local virtualenvs, and anything not needed in the
  build context.
...
- **Verify and correct design doc §4** (the Contrib Reuse Matrix) against the actually-installed
  ...
  against the real source tree. Three corrections were made in place:
...
### New Capabilities
- `container-image`: The Containerfile, compose.yaml, and .dockerignore that build and run the
  Evennia service as a container per design doc §9 (multi-stage, non-root/arbitrary-UID, correct
  ports and volumes, no GPU runtime in the image).
...
- **No existing runtime code is affected** — this is a greenfield change with zero prior
  implementation.
- **Dependency confirmed**: Evennia 6.1.0, requires Python >=3.12 (verified working on 3.14.6 in
  an isolated venv during this proposal's research; the Containerfile should pin an explicit
  supported version).
