...
- [ ] 1.1 In `web/webclient-app/components/MapLattice.vue`, add the `<defs>` for the
  ...
  `(nodeY - cy) % pitch === 0`.
- [ ] 1.2 In the same file, add the knowledge-edge vignette behind a `fogVignette` boolean
...
- [ ] 1.3 In the same file, add the axis cross behind a `showAxis` boolean prop
...
- [ ] 1.4 Add the scoped styles for the three layers and confirm no declaration animates or
...
- [ ] 2.1 In `MapLattice.vue`, add a `labelFont` number prop (default `11`, so the overlay is
  ...
  baseline.
- [ ] 2.2 In `MapLattice.vue`, compute the effective pitch per axis as
...
- [ ] 2.3 In `MapLattice.vue`, add the `fieldFill` boolean prop (default `false`) and the
...
- [ ] 2.4 In `MapLattice.vue`, delete the `maxUpscale` prop and its term in `widthCaps()`, so
...
- [ ] 2.5 In `web/webclient-app/components/LocalMap.vue`, declare the island's geometry on the
...
- [ ] 2.6 Confirm `web/webclient-app/components/MapOverlay.vue` needs no edit and assert it:
...
- [ ] 3.1 Extend `web/tests/browser/test_browser_local_map.py`'s island geometry audit
  ...
  width cap.
- [ ] 3.2 In the same file, add a contrast gate: compute the resolved composite colour of a
  ...
  ground.
- [ ] 3.3 In the same file, assert the audit's own exclusions hold: the collected
...
- [ ] 3.4 Re-run `test_densely_populated_lattice_scales_down_without_reintroducing_overlap`
...
- [ ] 3.5 Re-run `web.tests.browser.test_browser_contextual_hud` unchanged, and add one case
  for the axis/words coupling: the island draws the axis exactly where it states `北↑ 東→`,
  and the full-map surface draws none.
- [ ] 3.6 Add the island- and overlay-scale stories to
  ...
  with its dot field.
- [ ] 3.7 Run the full gates and record the counts: `npx vitest run`, `node --test
