...
- [x] 1.1 Rewrite `world/lore/wilderness_entry.py`: frozen `WildernessGate` +
...
- [x] 1.2 Add `validate_wilderness_entries()` (pure) covering every rejection rule in the
...
- [x] 1.3 Update `world/lore/tests/` for the v2 schema: derived-geometry values, point-shape
...
- [x] 2.1 `world/maps/wilderness_provider.py`: exclude registry footprint cells in
      `is_valid_coordinates` via a registry-identity-keyed cache (design D2); no anchor-key
      special-casing.
- [x] 2.2 `at_prepare_room`: open the gate exit's locks
...
- [x] 2.3 Update/extend provider tests: footprint invalid / approach valid, registry-patch
...
- [x] 3.1 `world/maps/wilderness_destination.py`: make `wilderness_neighbor` return `None` for
...
- [x] 3.2 `typeclasses/exits.py`: `WildernessReturnExit.at_traverse` uses the shared helper and
...
- [x] 3.3 Gateway tests: north at `(60,97)` → 南門 `(2,0)`, south at `(60,103)` → 北門 `(2,4)`,
...
- [x] 4.1 `world/maps/bootstrap.py::sync_wilderness()`: provision one `WildernessGateExit` per
...
- [x] 4.2 Bootstrap tests: both gates provisioned, idempotence (no duplicates), attribute
      healing, per-gate graceful skip, immediately-usable gate exits.

## 5. Consumer migration + re-pinned contracts

- [x] 5.1 `world/maps/wilderness_population.py`: redefine `CAPITAL_ENTRY_XY` as the north gate's
...
- [x] 5.2 `world/lore/tests/test_wilderness_entry.py` and
...
- [x] 5.3 `world/maps/tests/test_wilderness_destination.py` + `typeclasses/tests/test_exits.py`:
...
- [x] 5.4 `world/maps/tests/test_bootstrap.py`, `test_city_wilderness_roundtrip.py`,
...
- [x] 5.5 Re-pin contract tests for the three reworded specs (`wilderness-terrain`
      ...
      wording states.
- [x] 5.6 Minimal honest webclient survival migration (full per-gate presentation stays in
...
- [x] 6.1 Annotate the requirement-owning tests with `covers_requirement` literal IDs from
...
- [x] 6.2 Confirm no player-command surface changed (gates remain directional exits; no alias
...
- [x] 6.3 Focused runs green: `world.lore`, `world.maps`, `world.rules`, `typeclasses` exits
