...
- [x] 1.2 Implement mutually exclusive fixed/recovery rate validation and persistent caster snapshots in the existing buff owner/application path; verify malformed policies reject and fixed-rate behavior is unchanged.
- [x] 1.3 Implement live recipient-state tick amount plus exact timing/refresh/removal/refetch accounting; verify 10/20/30 ticks, equal split advances, no revive, caster deletion and no replay.
- [x] 1.4 Verify timed defense via a synthetic marker and existing combat rules; verify expiration and refresh alter real damage without activating inert bounds globally.
- [x] 1.5 Remove the per-key test correspondence checker and its requirement, retaining useful behavior tests; verify behavior-based test collection and no new light data-freeze exemptions.
