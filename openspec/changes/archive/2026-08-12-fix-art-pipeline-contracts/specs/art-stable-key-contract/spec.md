...
Every producer of a portrait/scene stable key — character import, quest characterization, and any blueprint policy — SHALL validate keys against the same rules: no `|`, `/`, `:`, `{`, `}`, or control characters, a maximum length of 64 characters, and a maximum of 200 UTF-8 bytes (so every accepted key keeps the worker output identity and the queue record key within the filesystem name-length limit). This is the single shared key contract; the import/creation validation changes mirror it.
