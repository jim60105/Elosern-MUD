...
- [x] 1.1 Confirm that C7 (`webclient-typewriter-reading-prefs`) and everything before it is archived. `grep -n "Core/MessageWindow" web/webclient-app/component-manifest.json` matches, and `openspec list` shows no earlier AVG series change pending. Stop and report otherwise.

## 2. Model and router (Node gate)

- [x] 2.1 `web/static/webclient/js/elosern/exploration_menu.js`: add `overviewMenu(panel, options)` per design D1 and export it.
...
- [x] 2.2 In the same file, add `verbMenuFor(model, target)` per design D2 (delegates to `targetMenuFor`, drops `target-empty`, inserts `look-target` before the back row) and export it. `targetMenuFor` is unchanged.
- [x] 2.3 `web/static/webclient/js/tests/exploration_menu.test.js`: add cases built only from synthesized fixtures (the file is in `tests/test_data_independence_js_webclient.py`'s `MIGRATED_FILES`):
...
- [x] 2.4 `web/static/webclient/js/elosern/keyboard_router.js`: implement the `sections` geometry per design D3 in `move()` and `projectFocus` (list projection). Update the file header comment. `web/static/webclient/js/tests/keyboard_router.test.js`: add cases for:
...
- [x] 3.2 `web/webclient-app/components/DockMenuItem.vue`: add the optional `glyph` prop (design D4), rendered as an `aria-hidden` leading span. `pnpm exec vitest run web/webclient-app/tests/action/dock_menu_item.test.js` stays green, and one new case covers the glyph.
- [x] 3.3 Create `web/webclient-app/components/SceneOverview.vue` per design D4, with:
...
- [x] 3.4 Create `web/webclient-app/components/DockVerbPopover.vue` per design D5, with:
...
- [x] 4.1 Create `web/webclient-app/stories/fixtures/scene_overview.js` with `explorationPanelFixture`, `overviewArgs`, and `verbArgs` (design D7), importing `../../lib/exploration_menu.js` only.
- [x] 4.2 Create `web/webclient-app/stories/Action/SceneOverview.stories.js` (`FullRoom`, `EmptyRows`, `DisabledExit`, `Overflowing`) and `DockVerbPopover.stories.js` (`DialogueHost`, `HostileTarget`, `LookOnly`, each over an inactive overview in a positioned 640×300 box). Args come only from the helper.
- [x] 4.3 `web/webclient-app/component-manifest.json`: add `"Action/SceneOverview"` and `"Action/DockVerbPopover"` after `"Action/DockBreadcrumb"`. Add both titles to the manifest snapshots in:
...
- [x] 4.4 Create `web/webclient-app/tests/action/scene_overview.test.js`. Mount `SceneOverview` with menus from `overviewMenu` and cover:
...
- [x] 4.5 Create `web/webclient-app/tests/action/dock_verb_popover.test.js`. Cover:
...
- [x] 6.2 Open `Action/SceneOverview` → `Overflowing` and `Action/DockVerbPopover` → `DialogueHost` in the built Storybook (`.storybook-out`) with `agent-browser`. Check that the chips wrap inside the box with no horizontal scrollbar and that the popover sits inside the box above its bottom edge. Close the browser afterwards.
- [x] 6.3 Run `openspec validate webclient-scene-overview-component --strict` and `git diff --check`. Both clean.
