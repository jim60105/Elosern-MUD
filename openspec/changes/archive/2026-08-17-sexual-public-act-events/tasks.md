...
- [x] 1.1 Add `_ACTOR_SCOPED_EVENTS = frozenset({"self_exposure", "public_exposure",
      "watched_during_activity", "public_sexual_activity"})` to
      `world/skills/sexual_acts/_builder.py` beside `_FORBIDDEN_SEXUAL_EVENTS`.
- [x] 1.2 Extend `_act_family()`'s effect emission: a declared event name in `_ACTOR_SCOPED_EVENTS`
      emits `sexual_event_actor:<name>`, any other name emits `sexual_event:<name>`, preserving
      declaration order.
- [x] 1.3 Add `world/skills/effects.py` `parse_effect` support for the `sexual_event_actor` prefix
      with a single-argument payload, mapping to a new typed effect (e.g.
      `ActorSexualEventEffect`).
- [x] 1.4 Add `_handle_actor_sexual_event` in `world/rules/action.py`: stage one `PendingEffect`
...
- [x] 2.1 Inject `event_context["room"]` in `RoomActionContext.__init__`
      (`world/rules/targeting.py`), overwriting any caller-supplied value.
- [x] 2.2 Add `observers_present(actor, targets, event_context) -> bool` to
...
- [x] 2.3 Add `_OBSERVER_GATED_EVENTS = frozenset({"watched_during_activity"})` and
      `_OBSERVER_GATED_COUNTERS = frozenset({"watched_count"})` to `world/rules/sexual_act_effects.py`.
- [x] 2.4 Gate `watched_during_activity` in the actor-scoped event handler: skip the event when
      `observers_present()` is false.
- [x] 2.5 Gate `watched_count` in `_handle_sexual_counter_effect`
...
- [x] 3.1 `shame_hem_lift`, `shame_half_expose_chest`, `shame_half_expose_lower`,
      `shame_loosen_collar`, `shame_full_expose`: `sexual_events=("self_exposure",
      "public_exposure")`.
- [x] 3.2 `shame_public_masturbation`: `sexual_events=("self_exposure", "public_exposure",
      "public_sexual_activity", "masturbation_climax", "watched_during_activity")`.
- [x] 3.3 `shame_public_performance`: `sexual_events=("self_exposure", "public_exposure",
      "public_sexual_activity", "watched_during_activity")`.
- [x] 3.4 `shame_devoted_pose`: `sexual_events=("self_exposure", "public_exposure",
      "watched_during_activity")` (no `public_sexual_activity`).
- [x] 3.5 `shame_shameless_declaration`: `sexual_events=("self_exposure", "public_exposure",
      "public_sexual_activity", "watched_during_activity")`.
- [x] 3.6 `shame_provocative_gaze`: unchanged (`sexual_events=()`).

## 4. Tests

- [x] 4.1 Structural tests: channel classification (each declared event resolves to exactly one
...
- [x] 4.2 `observers_present` unit tests: AREA target list, SELF alone/occupied room, empty
      battlefield, missing context (annotate).
- [x] 4.3 Gating tests: an unobserved cast skips `watched_during_activity` and `watched_count`
      while staging all other events/counters; an observed cast fires both (annotate).
- [x] 4.4 Actor-channel tests: `sexual_event_actor:self_exposure` reaches the actor and never a
      target; the AREA `self_exposure` scenario flips to the performer (annotate).
- [x] 4.5 Shame catalog tests (annotate): every shame act's declared event tuple per tasks 3.x;
      公開自慰 alone-vs-observed counter/experience outcomes; 公開表演 performer experiences; the
      public-vocabulary scenarios.
- [x] 4.6 `RoomActionContext` injection tests (annotate): constructed context carries `"room"`,
...
- [x] 5.1 Run `openspec validate --change sexual-public-act-events --strict` and resolve any
