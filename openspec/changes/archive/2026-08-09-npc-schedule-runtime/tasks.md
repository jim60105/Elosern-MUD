...
- [x] 1.1 Implement `settle_npc_schedules(start_tick, end_tick)` in `world/rules/npc_schedules.py`:
...
- [x] 1.2 Settle `move` entries: resolve target (stable key via registries or dbref override) to a
...
- [x] 1.3 Emit events with JSON-safe payloads (stable NPC identity; `state` or `from`/`to` target)
  and `due_tick = day_start + tick_offset`
- [x] 1.4 Register the source: `register_event_source("npc_schedules", settle_npc_schedules)` in
  the same composition-root pattern as the other sources (verify placement against the
  sync-ordering guard test)
- [x] 1.5 Per-entry failure isolation: unresolvable target, no exit, locked exit, or vanished
...
- [x] 2.1 Implement `interaction_reason(npc, interaction_kind) -> str | None` (stable authored
  reasons; `None` when unblocked); declare the `talk`, `engage`, `service_shop`, and
  `service_guild` kinds
- [x] 2.2 Consult the `talk` gate in the scripted-talk entry path (`commands/talk.py` **and** the
  WebClient `explore.talk_scripted` action in `web/webclient/actions/exploration_actions.py`):
  blocked interactions show the stable reason and write no state (no affinity, no guide
  progress, no turn-in)
- [x] 2.3 Consult the `talk` gate in the free-form seam (`typeclasses/npcs.py::at_talked_to` /
  `run_npc_exchange`): blocked interactions build no prompt, run no pipeline, append no memory,
  apply no intent
- [x] 2.4 Consult the `service_shop` gate in the shop buy/sell commands (`commands/economy.py`)
...
- [x] 2.5 Consult the `service_guild` gate in the guild operation commands (`commands/guild.py`)
  **and** the WebClient guild action adapters whenever the resolved local host is the NPC

## 3. Tests

- [x] 3.1 `EvenniaTest`: due state entry updates `schedule_state` + emits `npc_state_changed`
  with payload
- [x] 3.2 `EvenniaTest`: due move entry relocates along a real Exit, sets the template's
...
- [x] 3.3 `EvenniaTest`: locked exit and unresolvable target skip only that entry (log only, no
  failure event); other NPCs settle normally
- [x] 3.4 Multi-day skip settles every due occurrence exactly once in `(due_tick, npc_stable_id,
  entry_index)` order, and equals repeated day-by-day advances (A→B→A route)
- [x] 3.5 Mid-day assignment: passed occurrences never settle (no replay)
- [x] 3.6 Gate tests on every surface: scripted talk blocked (stable reason, no
...
- [x] 3.7 Registration test: `npc_schedules` source is `settle_npc_schedules`; existing
  `settlement-stage-order` stage-order tests stay green

## 4. Verification

- [x] 4.1 Run `openspec validate npc-schedule-runtime --strict`
- [x] 4.2 Run the affected package tests (`world/rules`, talk surfaces, clock suites,
  webclient service action tests); keep `git diff --check` clean
- [x] 4.3 Record canonical requirement IDs (`tools.spec_traceability list` after sync) and add
