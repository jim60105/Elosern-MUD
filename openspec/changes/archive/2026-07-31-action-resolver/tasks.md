...
- [x] 1.1 Confirm `world/rules/` exists with changes 3/6's `traits.py`/`rulebook/`/`buffs.py`/
      ...
      change.
- [x] 1.2 Confirm `commands/` exists as an empty package (change 1); create `commands/action.py` as an
      empty module.
- [x] 1.3 Confirm the exact import paths for `world.skills.registry.{SkillDef, SkillKind, TargetSpec,
      SKILL_REGISTRY}`; add the proposal-required `FactionConstraint` there, and use change 5's landed
      `world.rules.skill_effects.{record_conferred_grant, apply_disguise_effect}` seams plus
...
- [x] 1.4 Confirm the `django.db.transaction` import path against the installed Django version Evennia
...
- [x] 2.1 In `world/rules/action.py`, define `RejectReason(StrEnum)` with every value listed in
      design.md D-2 (one per pipeline step, plus the four target sub-reasons,
      `UNSNAPSHOTTED_EFFECT_SURFACE`, and `COMMIT_FAILED`).
- [x] 2.2 Define `RejectedAction(Exception)` and `CommitFailed(Exception)`, both carrying
      `reason: RejectReason` and `detail: str`, per design.md D-2.
- [x] 2.3 Define `ActionRequest` (frozen dataclass): `actor`, `skill_key: str`,
...
- [x] 2.4 Define `ActionResult` (frozen dataclass) with `outcome: Literal["success", "rejected"]`,
...
- [x] 3.1 Import `FactionConstraint` from `world.skills.registry` (change 5 — do **not** redefine a
...
- [x] 3.2 Define `ActionContext` as a `Protocol` (or ABC) with `battlefield: Battlefield | None`,
      `is_present(actor, target) -> bool`, `relation_to(actor, target) -> Relation`,
      `is_in_range(actor, target, skill) -> bool`, per design.md D-4.
- [x] 3.3 Implement `RoomActionContext` (out-of-combat, built now): `is_present()` checks room
...
- [x] 3.4 Implement `validate_faction(relation: Relation, constraint: FactionConstraint) -> bool` per
      design.md D-5's exact truth table (`ANY` always true; `SELF_ONLY` requires `SELF`; `ALLY` accepts
      `SELF` or `ALLY`; `ENEMY` requires `ENEMY`).
- [x] 3.5 Implement the four ordered per-candidate validation functions (`_validate_presence`,
...
- [x] 3.6 Implement `resolve_targets(request, skill, candidates) -> list[LivingEntity]`: for
...
- [x] 3.7 Implement `expand_target_shorthand(actor, context, shorthand: str) -> list[LivingEntity]` for
      ...
      validation path.
- [x] 3.8 Declare (docstring/type-only, no implementation) `BattlefieldActionContext`'s expected shape
...
- [x] 4.1 Define `EventEntry` (frozen dataclass): `kind: str`, `actor: str`, `target: str | None`,
      `data: dict`, `text_template: str`, per design.md D-8.
- [x] 4.2 Define `EventLog` (frozen dataclass): `actor: str`, `skill_key: str`,
      `targets: tuple[str, ...]`, `entries: tuple[EventEntry, ...]`, `time_cost_seconds: int`.
- [x] 4.3 Implement `render_plain_text(event_log: EventLog) -> str` per design.md D-8: joins every
...
- [x] 4.4 Confirm `EventEntry`/`EventLog` round-trip through `dataclasses.asdict()` → `json.dumps()` →
...
- [x] 5.1 Implement `_step1_ownership(request) -> SkillDef` per design.md D-3: looks up
...
- [x] 5.2 Implement `_step2_resource_check(actor, skill) -> None`: for every `(resource_key, amount)`
      in `skill.cost`, raises `RejectReason.INSUFFICIENT_RESOURCE` if
      `getattr(actor.traits, resource_key).value < amount`.
- [x] 5.3 Implement `_step3_targeting(request, skill) -> list[LivingEntity]`: expands shorthand (task
...
- [x] 5.4 Implement `_step4_capability(actor) -> None`: raises `RejectReason.ACTION_FORBIDDEN` if
      `blocks_action(actor)` (change 6's exact seam) is `True`.
- [x] 5.5 Implement `SNAPSHOTTED_SURFACES = frozenset({"traits", "sexual", "buffs", "skill_grants"})`,
...
- [x] 5.6 Implement the `confer_skill_partial` handler, registered with
      `surfaces=frozenset({"skill_grants"})`: stages `record_conferred_grant(target, source_key,
      skill_key, trait_keys, scale)` (change 5's landed deterministic write seam) as a
      `PendingEffect`, reading
...
- [x] 5.7 Implement the `set_disguise` handler, registered with `surfaces=frozenset({"traits"})`: stages
      `apply_disguise_effect(target, overrides)` (change 5's exact D-7 function) as a `PendingEffect`.
- [x] 5.8 Implement the `buff_apply:<key>` handler, registered with `surfaces=frozenset({"buffs"})`:
      stages `target.buffs.add(key, **buff_kwargs)` per target (change 6's `BuffHandler` mount, one
      `PendingEffect` per target for `AREA`).
- [x] 5.9 Implement the `confer_growth_rate` handler, registered with `surfaces=frozenset({"buffs"})`
...
- [x] 5.10 Implement the self-arming `sexual_event:<name>` handler, registered with
      `surfaces=frozenset({"sexual", "traits"})` because the landed rulebook can also spend `sp`,
      per design.md D-7: lazily imports
...
- [x] 5.10a Add a test-only synthetic handler registration exercising an unsupported surface (e.g.
...
- [x] 5.11 Implement `_step6_resource_deduction(actor, skill) -> list[PendingEffect]`: for every
...
- [x] 5.12 Implement `_step7_build_event_log(request, skill, pending) -> EventLog` per design.md D-8:
      builds one or more `EventEntry` records per `PendingEffect` (using each effect's `description` and a
      ...
      handlers, plus `trait_delta` alongside a resource trait change), from staged data only, before
      any `PendingEffect.apply()` runs. Raises
      `RejectReason.EVENT_LOG_CONSTRUCTION_FAILED` on malformed entry data (fault-injectable for
      testing).
- [x] 5.13 Implement `_step8_time_cost(request, skill) -> int` per design.md D-9:
...
- [x] 5.14 Implement `PendingEffect` (frozen dataclass: `entity`, `description: str`,
...
- [x] 5.15 Implement `_snapshot_entity_state(entity) -> dict` and `_restore_entity_state(entity,
      snapshot) -> None` per design.md D-1: snapshot captures complete raw persistent state and
      attribute existence for traits (including gauge timestamps), disguise, sexual state, buffs, and
      skill grants without materializing absent lazy handlers; restore replaces/removes those
      attributes exactly and invalidates affected handler caches.
- [x] 5.16 Implement `_commit(pending: list[PendingEffect]) -> None` per design.md D-1: **first**,
...
- [x] 5.17 Implement `ActionResolver.resolve(request: ActionRequest) -> ActionResult` per design.md
...
- [x] 6.1 Implement `CmdCast` per design.md D-10: parses `cast <skill_key>[=<target_key>]`, resolves
...
- [x] 6.2 Confirm `REJECTION_MESSAGES` covers every `RejectReason` value defined in task 2.1, with a
      generic fallback for any value it does not explicitly name.

## 7. Tests

- [x] 7.1 `world/rules/tests/test_action_pipeline_rejections.py` — one test per named rejection
...
- [x] 7.2 `world/rules/tests/test_action_pipeline_atomicity.py` — the centerpiece: stage three
      ...
      entity.
- [x] 7.3 `world/rules/tests/test_targeting.py` — per the `targeting-validation` capability: each of the
      ...
      out of combat.
- [x] 7.4 `world/rules/tests/test_event_log.py` — per the `event-log` capability: `EventEntry`/
...
- [x] 7.5 `world/rules/tests/test_effect_handlers.py` — per the `action-resolution-pipeline`
...
- [x] 7.6 `world/rules/tests/test_sexual_event_self_arming.py` — assert a `sexual_event:` effect
      resolves successfully and mutates `entity.sexual`, since change 7b is now landed; retain an
      import-failure test proving the handler rejects cleanly rather than crashing if the module is
      unavailable.
- [x] 7.7 `world/rules/tests/test_no_combat_branching.py` — per design.md D-6: the forbidden-token
...
- [x] 7.8 `world/rules/tests/test_cmd_cast.py` — `CmdCast` end-to-end: a successful cast renders via
      `render_plain_text()`; a rejected cast renders the matching `REJECTION_MESSAGES` entry.

## 8. Verification

- [x] 8.1 Run the full `world/rules/tests/` suite added by this change and confirm every test passes,
      including the now-armed sexual-transition integration test.
- [x] 8.2 Confirm no function in `world/rules/action.py`, `world/rules/targeting.py`, or
...
- [x] 8.3 Confirm `ActionResolver.resolve()` is the only function in `world/rules/` or `world/skills/`
      that constructs an `EventLog` (grep-based check).
- [x] 8.4 Confirm modifications to earlier files are limited to the explicitly required
      `SkillDef.faction_constraint` contract/test, the disguise-boundary test's allowance for atomic
      snapshot/restore, and registering `CmdCast` in the default command set.
- [x] 8.5 Confirm every effect handler registered by this change (task 5.6–5.10) declares a `surfaces`
...
- [x] 8.6 Run `openspec validate action-resolver --strict` and confirm it passes.
