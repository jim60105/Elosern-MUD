...
- [x] 1.1 Confirm `world/rules/` contains the already-landed changes' modules; create
      `world/rules/rulebook/__init__.py` and `world/rules/tests/__init__.py` if either is missing.
- [x] 1.2 Create `world/rules/rulebook/schema.py`, `world/rules/buffs.py`,
...
- [x] 1.3 Confirm the `evennia.contrib.rpg.buffs` import path (`BuffHandler`, `BaseBuff`) against the
...
- [x] 2.1 Define the `Condition` mapping type alias and frozen `Rule` dataclass
      (`id: str`, `when: Condition`, `then: dict`) per design.md D-2.
- [x] 2.2 Implement `load_rules(path: Path) -> list[Rule]` per design.md D-2: parses a YAML list of
...
- [x] 2.3 Implement `evaluate_condition(when: dict, context: Mapping[str, Any]) -> bool` per design.md
...
- [x] 2.4 Confirm `evaluate_condition()` never inspects `then` and no function in this module branches
...
- [x] 3.1 Author `world/rules/rulebook/combat_modifiers.yaml` per design.md D-3: `poison_agility_penalty`
...
- [x] 3.2 Implement `_build_context(entity) -> dict` in `combat_modifiers.py` per design.md D-3: always
...
- [x] 3.3 Implement `evaluate_combat_modifiers(entity) -> dict` per design.md D-3: loads
      ...
      attribute.
- [x] 3.4 Confirm no conditional in `combat_modifiers.py` distinguishes a buff-origin rule from a
...
- [x] 4.1 Author `world/rules/rulebook/buffs.yaml` per design.md D-4: `poisoned` (duration 300, tick
      ...
      Reject unsupported stacking policies. Persist `tick_interval` as metadata for the future
      deterministic clock; do not enable contrib real-time tick scheduling.
- [x] 4.2 Define `RulebookBuff(BaseBuff)` in `world/rules/buffs.py` per design.md D-4: one generic
      subclass parameterized by cached `definition_key` looked up in `buffs.yaml` at tick time — no
      per-buff Python subclass. Use the verified Evennia 6.1 `at_tick(initial, ...)` hook.
- [x] 4.3 Implement `_apply_rate_modifier(entity, rate_mod: dict) -> None` per design.md D-4
      ...
      `entity.traits`' gauge keys (`hp`/`mp`/`sp`), applies the delta through the GaugeTrait
      current-value setter (additive only, with the contrib enforcing its bounds); when
      `rate_mod["target"]` is in `_NO_OP_RATE_TARGETS` (currently exactly
      ...
      to ignore.
- [x] 4.4 Implement `entity_active_buffs(entity) -> set[str]` per design.md D-4: derive logical
      definition keys from the verified `entity.buffs.all` property.
- [x] 4.5 Implement `blocks_action(entity) -> bool` per design.md D-4: returns whether
...
- [x] 4.6 Implement `grant_conferred_growth_rate(entity, source_key: str, scale: float) -> None` per
      design.md D-5: call `entity.buffs.add(RulebookBuff, key="conferred_growth_rate:<source>",
      to_cache={...})`, caching the definition key, source key, and scale. Performs no ownership or
      resource check, mirroring change 5's `grant_conferred()`.
- [x] 4.7 Implement `growth_rate_multiplier(entity) -> float` per design.md D-5: iterates
...
- [x] 4.8 Implement a plain buff-tick callable (e.g. `tick_buffs(entity)`) per design.md's Non-Goals:
      ...
      Exclude expired, paused, and zero-stack cached instances from ticking and active-buff queries.
- [x] 4.9 In `typeclasses/entities.py`, **replace** change 3's `buffs = AttributeProperty(default=None)`
...
- [x] 5.1 `world/rules/tests/test_rulebook_schema.py` — per the `rulebook-schema` capability:
...
- [x] 5.2 `world/rules/tests/test_schema_docstring.py` (or folded into 5.1) — asserts
...
- [x] 5.3 `world/rules/tests/test_combat_modifiers.py` — per the `combat-modifier-table` capability: one
...
- [x] 5.3a `world/rules/tests/test_combat_modifiers.py` — **the unit-test half for the two sexual-field
...
- [x] 5.3b `world/rules/tests/test_combat_modifiers_self_arming.py` — **the integration-test half,
      kept in its own module so neither half of this pair can be dropped without the omission being
      conspicuous.** Implement `test_high_arousal_rule_fires_once_sexual_state_exists`, guarded by a
      real import and `unittest.skipUnless` (the Evennia runner uses unittest discovery): constructs a real entity with a real
...
- [x] 5.4 `world/rules/tests/test_rule_id_test_correspondence.py` — the mechanical check per design.md
...
- [x] 5.5 `world/rules/tests/test_buffs.py` — per the `buff-handler-integration` capability: one test
...
- [x] 5.6a `world/rules/tests/test_buffs.py::test_conferred_growth_rate_tick_is_a_no_op` — **the
      ...
      this defect.
- [x] 5.6 `world/rules/tests/test_conferred_growth_rate.py` (or folded into 5.5) — per design.md D-5/D-6:
...
- [x] 5.7 `world/rules/tests/test_blocks_action.py` (or folded into 5.5) — `blocks_action()` returns
      `True` with `paralysis` active, `False` with only `fear` active, and has no observable side
      effect.
- [x] 5.8 `world/rules/tests/test_buff_tick_seam.py` — invokes the buff-tick callable (task 4.8) directly
...
- [x] 5.9 `world/rules/tests/test_entity_mount.py` — confirms `typeclasses/entities.py`'s diff for this
...
- [x] 6.1 Run the full `world/rules/tests/` suite added or extended by this change and confirm every
...
- [x] 6.2 Confirm no function in `world/rules/buffs.py` or `world/rules/combat_modifiers.py` assigns to
      `entity.traits.<anything>.value`/`.base`/`.mod`; `_apply_rate_modifier()` uses only the
      documented additive GaugeTrait current-value setter (grep by hand, mirroring change 3's task 7.5 and change 5's
      task 8.2 discipline).
- [x] 6.3 Confirm `world/rules/rulebook/combat_modifiers.yaml` and `world/rules/rulebook/buffs.yaml` each
      parse via `load_rules()`/the buff-definition loader with no `id`/`key` missing or duplicated.
- [x] 6.4 Confirm every rule ID in `combat_modifiers.yaml` and every buff key in `buffs.yaml` has exactly
      one corresponding test function (task 5.4's mechanical check, run explicitly as part of this
      verification pass).
- [x] 6.5 Run `world/rules/tests/test_combat_modifiers_self_arming.py` (task 5.3b) in isolation and
...
- [x] 6.6 Run `openspec validate buffs-rulebook --strict` and confirm it passes.
