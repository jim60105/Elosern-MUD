...
- [x] 1.1 In `world/skills/registry.py` (lines 572-580), change `dual_wield_style` to
...
- [x] 1.2 In `world/skills/tests/test_registry.py`, move the `dual_wield_style` shape assertions
...
- [x] 1.3 In the same file, update `test_dual_wield_style_ownership_has_no_bearing_on_cost`
...
- [x] 1.4 Confirm `world/skills/tests/test_cost_tiers.py:15-25` needs no edit — `spell_tier_for`
      already returns `None` for the now-PASSIVE `dual_wield_style`

## 2. Preset kit

- [x] 2.1 In `world/lore/player_presets.py` (lines 98-100), move `dual_wield_style` from
      `yuka_darknight`'s `active_skills` to the start of its `passive_skills`, keeping
      `dual_blade_mastery` and `shadow_slash` active
- [x] 2.2 Extend `world/lore/tests/test_player_presets.py::test_kit_validation_rejects_unknown_kind_mismatch_and_divine_gate`
      with the regression case `(make(active_skills=("dual_wield_style",)), "classifies it as")`
- [x] 2.3 Run the preset tests to confirm `test_every_preset_skill_resolves_with_matching_kind`
      passes with the moved bucket

## 3. Clean cast rejection at the ownership step

- [x] 3.1 Add `test_cast_of_reclassified_dual_wield_style_is_rejected_as_passive` to
...
- [x] 3.2 Add a companion assertion in `world/rules/tests/test_action_preview.py` that
      `preview_skill(...).reason` reports the same `SKILL_NOT_ACTIVE` for an owned
      `dual_wield_style` (the requirement's preview half)
- [x] 3.3 Confirm no existing test casts `dual_wield_style` expecting `UNKNOWN_EFFECT_ID` or a
      successful outcome (grep `dual_wield_style` in `world/rules/tests/` and
      `world/skills/tests/` after the edits)

## 4. Verification

- [x] 4.1 Run the affected domains: `uv run --locked evennia test --settings test_settings.py
...
- [x] 4.2 Confirm the combat-modifier and status-query tests pass with zero edits (the rule row,
      its no-create contexts, and the status-display row are untouched)
- [x] 4.3 Check `git diff --check` is clean; run `openspec validate --change
      fix-dual-wield-style-stance --strict`
- [x] 4.4 Do NOT add `covers_requirement` annotations for the new delta requirement during
      ...
      scenario 2 has a direct association — DONE: all four annotated with the canonical ID
      `skill-registry::dual-wield-style-is-a-passive-stance-not-a-castable-active-skill`
      (verified via `tools.spec_traceability list` after syncing)
