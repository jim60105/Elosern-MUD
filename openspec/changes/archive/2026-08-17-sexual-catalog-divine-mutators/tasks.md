...
- [x] 1.1 **Precondition, not a routine confirmation:** `sexual-catalog-divine-core` (`C7a`) must be
...
- [x] 1.2 Confirm `_step4b_sexual_resist_gate` (`world/rules/action.py`) is unchanged.
- [x] 1.3 Confirm `SexualState.__init__`'s `Monster` shame-pinning (`shame.min = shame.max = 0`) is
  unchanged — `clamp_shame_to`'s Monster rejection depends on this staying true.
- [x] 1.4 Confirm `SexualState.virgin`'s public setter is still unconditionally a no-op once `False`
  (`if not self.virgin: return`) — `restore_purity()`'s non-regression argument (design.md D-4) depends
  on this exact shape.
- [x] 1.5 Confirm `resist_verdict()`'s existing two short-circuit terms (affinity `auto_comply`,
  `_climax_turn_short_circuit`) and its no-create discipline (`resister.attributes.get(...)`, never
  `resister.sexual`) are unchanged.
- [x] 1.6 Confirm Evennia objects expose a stable, unique `.id` (database primary key) —
...
- [x] 2.1 Add `saturate_sensitivity()`: iterate `BODY_PARTS` (non-`Monster`) or just
  `GENERIC_BODY_PART` (`Monster`), setting each to `SENSITIVITY_LEVELS[-1]` via the existing
  `sensitivity` proxy's `__setitem__`.
- [x] 2.2 Add `clamp_shame_to(level)`: raise `ValueError` for a `Monster` entity without mutating
  state; otherwise set the leading bound first — `.max` then `.min` when the target ordinal is at or
  above the current `max`, `.min` then `.max` otherwise (design.md D-3's conditional order, safe for
  every reachable prior bound state in both directions, superseding the original `max`-first-only
  argument).
- [x] 2.3 Add `mark_submission(caster_key)`: read the current `submission_marks` (default
...
- [x] 2.4 Add `restore_purity()`: write `entity.attributes.add("virgin", True, category="sexual_state")`
  directly — never call the public `virgin` setter.
- [x] 2.5 Add all four to `SexualState`'s `__all__`-equivalent surface (public methods/property), no
...
- [x] 3.1 Add `_submission_term(actor, resister) -> bool`: read
...
- [x] 3.2 Change `resist_verdict()`'s short-circuit condition from `affinity_auto_comply or
...
- [x] 3.3 Confirm no other line in `resist_verdict()` changes — same parameters, same return shape,
  same two pre-existing terms' behavior.

## 4. `world/skills/effects.py`: four new typed effect dataclasses

- [x] 4.1 Add `SaturateSensitivityEffect`, `ClampShameEffect`, `MarkSubmissionEffect`,
  `RestorePurityEffect` (frozen dataclasses, no fields).
- [x] 4.2 Register all four in `parse_effect`'s dispatch table.

## 5. `world/rules/action.py`: four new effect handlers

- [x] 5.1 Add `_handle_saturate_sensitivity(actor, targets, effect_id, context, scale)`: for each
...
- [x] 5.2 Add `_handle_clamp_shame(actor, targets, effect_id, context, scale)`: for each entity in
...
- [x] 5.3 Add `_handle_mark_submission(actor, targets, effect_id, context, scale)`: for each entity in
  `targets` excluding `actor`, stage a `PendingEffect` calling
  `entity.sexual.mark_submission(str(actor.id))`.
- [x] 5.4 Add `_handle_restore_purity(actor, targets, effect_id, context, scale)`: for each entity in
  `targets` excluding `actor`, stage a `PendingEffect` calling `entity.sexual.restore_purity()`.
- [x] 5.5 Register `"divine_saturate_sensitivity"`, `"divine_clamp_shame"`, `"divine_mark_submission"`,
  and `"divine_restore_purity"` in `_EFFECT_HANDLERS`.

## 6. `world/skills/sexual_acts/divine.py`: the four hand-built acts

- [x] 6.1 Add `感度創世` (`TargetSpec.SINGLE`, `requires_divine_arts=True`, `unlock={}`,
  `target_part=None`, `resistible=True`, `actor_counters=()`, `participant_counters=()`,
  `effects=["divine_saturate_sensitivity:<key>"]`).
- [x] 6.2 Add `恥辱剝奪` (same shared fields, `effects=["divine_clamp_shame:<key>"]`).
- [x] 6.3 Add `絕對從屬` (same shared fields, `effects=["divine_mark_submission:<key>"]`).
- [x] 6.4 Add `無垢回歸` (same shared fields, `effects=["divine_restore_purity:<key>"]`).
- [x] 6.5 Populate each `SexualActDef`'s required-but-unused pleasure fields with the same documented
  placeholder convention `sexual-catalog-divine-core` established (`base_pleasure=1`,
  `actor_pleasure_ratio=0.0`, `actor_part=None`).
- [x] 6.6 Extend `DIVINE_ACTS` to seven entries: the three existing pairs, unmodified, plus these four.
...
- [x] 7.2 Add `world/skills/sexual_acts/tests/test_divine_mutators_catalog.py` covering: the race gate
...
- [x] 7.3 Add or extend `world/rules/tests/test_sexual_state.py` (or the equivalent existing module)
  covering all `sexual-state-handler` scenarios for the four new mutators, including
  `clamp_shame_to`'s Monster rejection, its general level contract (a parameterized test over all
  five `SHAME_LEVELS` members from floor-pinned, ceiling-pinned, mid-range, and fresh prior bounds —
  added after the rubber-duck review found the fixed `max`-first order could not clamp down), and
  `restore_purity`'s explicit non-regression scenario (the public setter's existing one-way test
  still passes unmodified after `restore_purity` exists).
- [x] 7.4 Add or extend `world/rules/tests/test_sexual_resist.py` (or equivalent) covering the new
...
- [x] 7.5 Apply `covers_requirement(...)` (using the IDs from 7.1) to each test function whose
  assertions establish that requirement, across all three delta specs.
- [x] 7.6 Run `uv run --locked python -m tools.spec_traceability check` and confirm every requirement
...
- [x] 7.7 Sync the delta specs into `openspec/specs/`: create
  `openspec/specs/sexual-catalog-divine-mutators/spec.md`, append the four mutator requirements to
  `openspec/specs/sexual-state-handler/spec.md`, apply the modified and added `sexual-resist-contest`
  requirements, and amend `openspec/specs/sexual-catalog-divine-core/spec.md`'s registration
  requirement per the new `specs/sexual-catalog-divine-core/spec.md` delta (the shipped "exactly
  three" count pin can no longer hold once the tuple holds seven; the requirement now pins the three
  pairs' identity and fields instead).
...
- [x] 8.3 Run `openspec validate sexual-catalog-divine-mutators --strict` and resolve any reported
  issue.
- [x] 8.4 Confirm no file outside `world/rules/sexual_state.py`, `world/rules/sexual_resist.py`,
  ...
  byte-for-byte unchanged. Two count pins in pre-existing test modules were updated as a consequence
  of the tuple growing (the same update `C7a`'s implementation made for its own growth): the
  `len(DIVINE_ACTS)` assertion in `world/skills/sexual_acts/tests/test_seed_acts.py` (3 → 7) and the
  C7a registration tests in `world/skills/sexual_acts/tests/test_divine_core_catalog.py`, which now
  scope their assertions to the three `_DIVINE_KEYS` instead of the whole tuple. The pinned
  category-classification table in `world/skills/tests/test_skill_registry.py` also gained the four
  new `sexual_act` keys, matching the update `C7a`'s implementation made for its three.
