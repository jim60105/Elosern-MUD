...
- FROM: `### Requirement: Settlement stages run in the fixed order regen, buffs, sexual decay, magic study,`

- TO: `### Requirement: Settlement stages run in the fixed order regen, buffs, sexual decay, practice settlement,`

- FROM: `### Requirement: buff_ticks, sexual_decay, and magic_study are skipped for combat-sourced advances`

- TO: `### Requirement: buff_ticks, sexual_decay, and practice settlement are skipped for combat-sourced advances`
...
**Reason**: the lazy seam existed to reach `world.rules.progression.accrue_magic_study()`, which
`magic-xp-engine-retirement` deletes; the surviving stage is a placeholder implemented inline in
`world/rules/clock.py`, so no cross-package seam (and no double-gate composition) remains.
**Migration**: `declared-practice-skip` installs the real writer behind the same stage slot and
modifies this capability's contracts when it does.
