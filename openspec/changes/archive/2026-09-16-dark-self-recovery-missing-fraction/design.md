...
- Verdict: the magnitude-mode extension IS needed — folding the nodes into the catalog change would silently drop the 10%/15% clauses (a stat-basis self-heal is a different quantity, not an approximation the lore sanctions). (Frozen-doc census: the phoenix name also appears in `docs/superpowers/specs/2026-08-30-use-driven-progression-design.md`'s ladder sketch — likewise never a registry row, requirement ID, or test; conclusion unchanged.)
