...
adapter 依既有三參 ABI 取得 session，把驗證過的提案連同 session 內單調遞增的暫態序號 `revision` 寫入 `session.ndb.concept_proposal`，並仿 `options_state` 的 owner 綁定記錄 `owner_actor_id`（快照 factory 拒繪非本 actor 的槽；換偶即清）；`web/webclient/presentation/ingress.py` 的 `build_presentation_context`（全倉庫所有發布路徑唯一的 context 工廠，現行即在此深拷貝 `options_state`）依同款先例深拷貝為提案快照欄；creation presenter 渲染 optional 頂層 `proposal` 鍵。回傳一般 success（code `concept_applied`）並宣告 affected `creation`，走既有 panel-update 發布路徑。概念路徑零持久寫入，故完全不讀寫啟動確認指紋（`FINGERPRINT_NDB_KEY`）：成功的 save 確認照常存活，概念完成也永遠不會授權未經確認的 draft。
