...
`world/rules/equipment.py` SHALL define the mutating `add_item(entity, item_key)` and
`remove_item(entity, item_key)` deterministic-core operations. The read-only
`world/skills/equipment.py` SHALL define `list_items(entity)`. They operate on
`entity.db.inventory` — the same raw attribute change 4's loader already writes
(`entity.db.inventory = record["inventory"]`) with no additional seam declaration required.
