...
- [x] 1.1 Extend `_spawn_npc` in `world/quests/scene_builder.py` to apply characterization
...
- [x] 1.2 Add the deterministic adult baseline constant (25) with a documented rationale: always
      within the adult-to-lifespan-maximum validation range for every race
- [x] 1.3 Add scene-builder unit/integration tests for every input shape: full characterization
...
- [x] 2.1 Add an integration test materializing a quest scene with a named occupant (template
...
- [x] 2.2 Add a baseline-for-elf test: a portrait-bearing `elven_civilian` occupant without ages
      is spawned with ages 25 and passes the adult gate
- [x] 2.3 Add a shared-key test: two scenes with the same `stable_key` resolve to one subject,
      settle to one completed asset, and the first materialized description wins
- [x] 2.4 Add a rollback test: a rolled-back materialization emits no post-commit portrait job

## 3. Repository guards and verification

- [x] 3.1 Add a guard test asserting a portrait-bearing occupant always carries canonical adult
      ages before the policy is set (the adult gate's inputs are guaranteed present)
- [x] 3.2 Confirm no art-layer files changed (subjects, adult gate, queue, worker, presenter stay
      read-only) and existing art-assets / scene-builder suites stay green
- [x] 3.3 Run the affected Evennia test domains
...
- [x] 3.4 Run `uv run --locked python -m tools.spec_traceability check` and confirm the new main
      requirements carry `covers_requirement` annotations
- [x] 3.5 Run `openspec validate spawn-named-portraits --strict` and confirm the change is
      apply-ready
