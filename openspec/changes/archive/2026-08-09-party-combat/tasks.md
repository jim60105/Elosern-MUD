...
- [x] 1.1 Use `world/rules/party.py`'s safe companion-resolution API to collect the player's
      bound, co-located, living, non-knocked-out companions.
- [x] 1.2 Extend `engage` in `world/rules/combat_session.py`: include those companions in
...
- [x] 1.3 Build the per-entity nonlethal context: `_context_for` sets `nonlethal_keys` to the
      companion keys in hostile sessions; the exam mode keeps the session-wide flag unchanged.
- [x] 1.4 Add integration tests: co-located living companions join the allied team; distant, dead,
      or knocked-out companions do not; an empty-party engage behaves exactly as before; the
      player's ENEMY-targeting skills never select a companion; opposing-team combatants can
      target companions; and a companion's fallback policy attacks the opposing team without
      fleeing.

## 2. Knockout as battlefield state

- [x] 2.1 Add `knocked_out: set[str]` to `Battlefield` in `world/rules/combat.py` (beside `fled`),
      initialize it in `reconstruct_battlefield` from `record.knocked_out_ids`, and expose a
      shared `is_knocked_out(key)` predicate.
- [x] 2.2 Extend `world/rules/action.py`'s battlefield-shaped snapshot/restore to cover `fled`
      AND `knocked_out` (duck-typed, no `Battlefield` import); extend the commit-rollback path
      accordingly.
- [x] 2.3 Mark the knockout at damage-commit time: the per-entity nonlethal floor in
...
- [x] 2.4 Convert every alive-check consumer to the predicate: initiative order, the round and
...
- [x] 2.5 Persist `knocked_out_ids` at round end as today (existing `_knocked_out_ids` backfill
      plus in-round markings).
- [x] 2.6 Clear the marker only when HP rises above 1 via regen; make `engage` exclude
      knocked-out companions until then.
- [x] 2.7 Add tests: a knocked-out companion receives no policy request, is excluded from
...
- [x] 3.1 Update the knockout decision in `world/rules/action.py` (`_defeated_entry` / step-7
      builder): per-damaged-target `nonlethal = session_flag or target.key in nonlethal_keys`.
- [x] 3.2 Update the HP-crossing floor in `world/rules/combat.py` to apply the nonlethal floor per
      damaged target under the same rule and to mark the battlefield knockout (2.3).
- [x] 3.3 Add tests: a companion crossing to non-positive HP floors at 1, emits
...
- [x] 4.1 Rewrite `_terminal_outcome` in `world/rules/combat_session.py`: the player fled → fled;
...
- [x] 4.2 Fix `clear_session` / forfeit / recovery cleanup to resolve every participant dbref from
      the persisted record so a failed reconstruction still unregisters surviving companions and
      monsters from skip safety.
- [x] 4.3 Add tests: player defeated with companions standing settles as defeat end to end (no
...
- [x] 5.1 Run the touched test labels (`world.rules` combat/combat_session/monster_behaviour/
...
- [x] 5.2 Run `uv run --locked python -m compileall -q world typeclasses commands server` and keep
      `git diff --check` clean.
- [x] 5.3 Annotate substantive new tests with `covers_requirement` canonical IDs; run
