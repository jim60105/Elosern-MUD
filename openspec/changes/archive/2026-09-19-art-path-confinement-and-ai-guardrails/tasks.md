...
- [x] 1.1 Grep for tests pinning the OLD weak shape before flipping:
...
- [x] 1.2 `world/art/worker.py`: delete `_store_root`/`_resolved_under_root` (lines 111-124);
...
- [x] 1.3 `web/art_media.py`: delete `_store_root`/`_resolved_under_root` (lines 68-80); the
...
- [x] 1.4 Security-regression pin: confirm `world/art/tests/test_gallery.py`'s symlink/NUL
...
- [x] 2.1 Add `GuardrailHooks` (frozen dataclass, `install()` / `uninstall_own()`) to
...
- [x] 2.2 Migrate `world/ai/narrator.py`: delete `_uninstall_fallback`, `_uninstall_validator`,
...
- [x] 2.3 Migrate `world/ai/npc_dialogue.py`, `world/ai/scene_flavor.py`,
...
- [x] 2.4 Startup seams stay green (they call `register_*` through
...
- [x] 3.1 Create `world/ai/immutable.py` with `reject_mutable_containers(value, path)` —
...
- [x] 4.1 `uv run --locked python -m tools.spec_traceability check` unchanged-passes.
- [x] 4.2 `uv run --locked python -m tools.observability_lint check` passes; no new
  `tools/observability_freeze.json` entries (`git diff` on it is empty).
- [x] 4.3 `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --keepdb tests.test_evennia_test_optimization_contract`
