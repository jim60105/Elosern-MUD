...
- [x] 1.1 Extract `_transfer_items`'s delivery-advance block verbatim into
  ...
  green untouched.
- [x] 1.2 Seam-level tests in `world/quests/tests/test_deliver.py`: advance on a matching transfer,
...
- [x] 2.1 Canonical-key branch: snapshot both parties' surfaces and the pin rooms before the
  ...
  re-raising.
- [x] 2.2 Materialized-object branch: wrap `_transfer_with_plan` in an outer transaction, call the
  ...
  `[]`) stays untouched.
- [x] 2.3 Command integration tests in `commands/tests/test_localized.py`: registry-key give
...
- [x] 3.1 `docs/game/commands.md`'s `給` row states that handing a quest item to its bound
  recipient advances the delivery and that `交付` is the quest-scoped verb.
- [x] 3.2 `docs/game/command-reference.md`'s `給` canonical entry's 說明 states the same; the drift
...
- [x] 4.2 `uv run --locked python -m tools.observability_lint check`,
...
- [x] 4.3 `uv run --locked python -m compileall -q world typeclasses commands` and
  `git diff --check`.
