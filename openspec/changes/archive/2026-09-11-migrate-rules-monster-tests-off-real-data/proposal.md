...
monster behaviour/flee/overwhelm/defeat_aftermath tests hardcode shadow_slash, fire_ball, calamity, and monster-tier display prose inside asserted selections.
...
tagged data-contract tests; monster and aftermath behavior tests are the remaining debt in this
area.

## What Changes

- Migrate the 14 flagged test files under world/rules/tests/ (monster cluster) to the synthetic test-data kit
  (`world/tests/synthetic_data.py`) or file-local synthetic fixtures
  (9 assertion-literal files, 5 setup-literal files).
...
- Remove this area's debt entries from `tools/test_data_freeze.json` and add the area
  closure test in `tests/test_data_independence_rules_monster.py`. Its `@covers_requirement` annotation is added in the
...
| file | tier |
| --- | --- |
| `world/rules/tests/test_defeat_aftermath_core.py` | A |
| `world/rules/tests/test_defeat_aftermath_digest.py` | B |
| `world/rules/tests/test_defeat_aftermath_violation.py` | A |
| `world/rules/tests/test_monster_behaviour_determinism.py` | B |
| `world/rules/tests/test_monster_behaviour_golden.py` | A |
| `world/rules/tests/test_monster_behaviour_integration.py` | A |
| `world/rules/tests/test_monster_behaviour_policy.py` | A |
| `world/rules/tests/test_monster_behaviour_selection.py` | A |
| `world/rules/tests/test_monster_flee_decision.py` | A |
| `world/rules/tests/test_monster_scale.py` | B |
| `world/rules/tests/test_monster_sexual_baseline.py` | A |
| `world/rules/tests/test_overwhelm_compression.py` | A |
| `world/rules/tests/test_overwhelm_resolution.py` | B |
| `world/rules/tests/test_overwhelm_threshold.py` | B |
...
- `test-data-independence`: adds the Monster and aftermath closure requirement — every test file in this
  change's migration manifest passes the test-data lint gate and the manifest's freeze-list
  debt entries are removed. Files under world/rules/tests/ (monster cluster) owned by other changes (contract-tagged
  files, sibling migration areas) are unaffected by this requirement.

## Impact

- The 14 listed test files plus the new closure test file `tests/test_data_independence_rules_monster.py`.
