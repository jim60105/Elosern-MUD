...
- [x] 1.1 Add a `cap_breaks` list to `world/rules/rulebook/affinity.yaml` with one documented
      example entry (`npc_key` or `role`, `quest_key`, `new_cap`)
- [x] 1.2 Extend `world/rules/affinity_config.py` to load and validate `cap_breaks`: non-empty
...
- [x] 2.1 Implement `world/rules/affinity.py::raise_affinity_cap(npc, player, new_cap) -> bool`:
      ...
      changed
- [x] 2.2 Confirm serialization round trips preserve a raised cap (e.g. 150) with unchanged value
      and daily fields, including a fresh record created by the raise

## 3. Turn-in integration

- [x] 3.1 In the deterministic guild-quest turn-in path, precompute `raise_affinity_cap` calls for
      every then-in-party companion matching a `cap_breaks` entry for the completed `quest_key`
- [x] 3.2 Apply the cap raises **before** the `quest_completion` gains in the transaction, so a
      record at the old cap cannot clamp the +2; a companion matching multiple entries resolves to
      the highest `new_cap`
- [x] 3.3 Commit the cap raises inside the existing atomic reward transaction, so a fault at any
      write position restores values and caps together

## 4. Tests

- [x] 4.1 Pure/unit tests: `raise_affinity_cap` monotonic/idempotent/below-cap/record-creation
...
- [x] 4.2 `EvenniaTest` turn-in tests: matching entry raises caps for each matching companion in
...
- [x] 4.3 Ladder regression: a value above the old natural cap renders 絕對羈絆 with no numeric
      display
- [x] 4.4 Regression: existing affinity, party-quest, and quest-reward-settlement suites stay
      green
...
- [x] 5.3 Run the focused test packages (world rules and world quests tests) and confirm green
