...
- [x] 1.1 Add `TargetRequirement` to `world/rules/targeting.py`: a frozen dataclass with
...
- [x] 1.2 Add `SkillDef.target_requirement` in `world/skills/registry.py` producing
...
- [x] 2.1 Change the four validators in `world/rules/targeting.py` to
...
- [x] 2.2 Change `resolve_targets` to `(actor, context, requirement, candidates)` and
...
- [x] 2.3 Replace the `SkillCategory.SEXUAL_ACT` branch in `resolve_targets`'s SINGLE arm with a
...
- [x] 2.4 Drop the third parameter from `ActionContext.is_in_range`, `RoomActionContext.is_in_range`,
...
- [x] 3.1 Create `world/rules/action_gates.py` and move `damage_requires_battlefield()` into it with
...
- [x] 3.2 Update the two callers (`world/rules/action.py:56`, `world/rules/action_preview.py:48`) to
  import from the new module and verify `world/rules/targeting.py` no longer imports `DamageEffect`.
## 4. Update call sites

- [x] 4.1 Update `_step3_targeting` in `world/rules/action.py:394` to pass
  `(request.actor, request.context, skill.target_requirement, candidates)`. Verify the existing cast
  pipeline tests pass unchanged.
- [x] 4.2 Update the three `candidate_rejection` call sites in `world/rules/action_preview.py`
...
- [x] 5.1 Port every `resolve_targets` / `candidate_rejection` call in
...
- [x] 5.2 Update `world/rules/tests/test_battlefield_action_context.py:37` to the two-argument
  `is_in_range` and verify the suite passes.
- [x] 5.3 Run the full deterministic test suite and verify zero behavioral diffs across the cast
  pipeline, combat, action-preview, and sexual-act suites — the whole point of this change is that
  nothing else moves.
## 6. Documentation and validation

- [x] 6.1 Update the `world/rules/targeting.py` module docstring to state the new contract (the
...
- [x] 6.2 Run `openspec validate refactor-target-resolution-srp` and the repository's lint/observability
