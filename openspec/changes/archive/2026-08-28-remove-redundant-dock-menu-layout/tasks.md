...
- [x] 1.1 In `DockMenu.vue`, make the template a fragment: the `.dock-menu`
...
- [x] 1.2 In `AppClient.vue`, add the `hud-drawer__body--dock` flex-row
...
- [x] 1.3 Verify both hosts render list + detail side by side for split frames
...
- [x] 2.1 Update `web/webclient-app/tests/action/dock_menu.test.js` and
...
- [x] 2.2 Migrate `web/tests/browser/test_browser_combat.py:776` from
