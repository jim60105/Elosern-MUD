...
- [x] 1.1 Expose committed `dialogue` panel availability + host/bond/line/choices in the Vue
...
- [x] 2.1 `NarrativeFeed.vue`: mode+availability branch — head label `對話` with no `完整日誌`
...
- [x] 2.2 Free-dialogue row focuses the borrowed command line through the existing freeform
  path; pick activation dispatches `explore.talk_scripted {npc_id, keyword_id}`.

## 3. Dock dialogue form

- [x] 3.1 `dialogue.root` descriptor in the resolver registry: single `對話選項` tab; pane rows
...
- [x] 3.2 Legend swaps to `數字鍵 1–4 選 · → 指令列自由對話` (kbd `→`) while the dialogue form
...
- [x] 4.1 Vitest (feed): box fields incl. bond-null who segment; dispatch-once pick payload;
...
- [x] 4.2 Vitest (dock/router): `dialogue.root` rows verbatim from panel; degradation marker;
...
- [x] 4.3 Matrix test update: dialogue column visible across islands/minimap/party/tracker/
  command line; backdrop art unchanged on mode flip.

## 5. Verification

- [x] 5.1 Focused Vitest labels + `tools.spec_traceability check`.
- [x] 5.2 Live container check 1600x900: scripted talk → dialogue variant matches the draft
...
- [x] 6.1 Apply AFTER webclient-align-03/05/09/10 land (W3): the contextual-hud delta restates
