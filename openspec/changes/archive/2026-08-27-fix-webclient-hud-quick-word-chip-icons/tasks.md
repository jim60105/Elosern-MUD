...
- [x] 1.1 In `web/webclient-app/components/dock-icons.js`, add a new `get` entry to `GLYPHS` with the
...
- [x] 1.2 Add a Vitest assertion that `glyphPath("get")` returns the new path, alongside the existing
...
- [x] 2.1 In `web/webclient-app/components/QuickWordChips.vue`, import `glyphPath` from `./dock-icons.js`
...
- [x] 2.2 Add `display: inline-flex; align-items: center; gap: 6px;` to `.qwc__chip` (it currently has
...
- [x] 2.3 Add a Vitest assertion that every rendered chip (exploration and combat sets) contains exactly
      one `aria-hidden` icon element alongside its text label.

## 3. Stories and verification

- [x] 3.1 Update `web/webclient-app/stories/Core/QuickWordChips.stories.js`'s existing
...
- [x] 3.2 Confirm `openspec validate fix-webclient-hud-quick-word-chip-icons --strict` passes against the
      delta spec in `specs/webclient-contextual-hud/spec.md`.
- [x] 3.3 Run the focused test slice: `npm test -- quick_word` (or the component's existing test file
      name) plus the new/extended dock-icons assertion.
- [x] 3.4 Re-check the live client (`podman compose`, `http://localhost:4001/webclient/`) with
