...
- [x] 1.1 Rewrite `ItemTouchedJournal` as actor-owned surfaces (inventory, quest log, mirror,
  ...
  case.
- [x] 1.2 Run the in-process cache drop for **every** captured entity on restore, not only the actor:
...
- [x] 1.3 Assert entity identity on restore so a record can only be written back to the entity it came
...
- [x] 1.4 Verify the existing single-target rollback tests
...
- [x] 2.1 Remove the self-only restriction from `world/rules/item_effects.py` and add the scope →
  ...
  scenario.
- [x] 2.2 Add a structural test that item and skill targets validate identically: an item effect
...
- [x] 3.1 Add `target: Any | None = None` to `ItemUseRequest` — a scalar, never a sequence or a
  shorthand (D1). Verify a supplied group shorthand token rejects.
- [x] 3.2 Add the `context` parameter to `preflight_item_use` (D2). Verify `in_combat` still gates the
  `combat_allowed` permission independently of the context's kind.
- [x] 3.3 Resolve each effect's targets through the shared resolver and expand group scopes through
...
- [x] 3.4 Add `NO_TARGET` and `TARGET_INVALID` to `ItemUseReason` with Traditional Chinese messages,
...
- [x] 3.5 Extend the effectiveness gate across targets: reject only when no effect is effective
  against any of its targets. Verify the "One effective target among several carries the whole use"
  scenario.

## 4. Settlement and event log

- [x] 4.1 Expand the plan to one step per effective effect-and-target pair (D4), keeping every step
...
- [x] 4.2 Capture the journal for every entity the plan touches before the first write. Verify the
  "A mid-settlement failure restores every touched entity" scenario with fault injection after two of
  four targets.
- [x] 4.3 Emit one `item_used` entry per effective pair, each naming its own target, and set the
...
- [x] 5.1 Pass the target and the battlefield context through both `ItemUseRequest` constructions in
  `world/rules/combat_session.py` (`:1235`, `:1248`). Verify the existing combat item-turn suite
  passes unchanged.
- [x] 5.2 Fold the multi-entity journal into `world/rules/combat.py`'s item branch rollback contract.
...
- [x] 5.3 Verify a four-target item consumes exactly one round and adds no separate item-use time —
  the two new `item-use-resolution` combat scenarios.

## 6. Player surfaces

- [x] 6.1 Extend `commands/items.py` to `使用 <item_key> [target]`, resolving the target token to a
  present entity. Verify the missing-target and telnet/WebClient-parity scenarios in the
  `inventory-item-actions` delta.
- [x] 6.2 Add the optional bounded `target_key` to the `inventory.use` payload in
...
- [x] 6.3 Build a `RoomActionContext` in `world/rules/service_view.py:770` so inventory-row enablement
...
- [x] 7.1 Update `docs/game/commands.md` and `docs/game/command-reference.md` for the new `使用`
  syntax — required by `inventory-item-actions` to land in this same change. Verify both files show
  the optional target argument.
- [x] 7.2 Update the 第六層 scope table in `docs/lore/items.md` and `docs/development/adding-items.md`
...
- [x] 7.3 Run the full deterministic suite plus the item regression test from
  `add-declarative-item-effects` task 1.1, and verify the four shipped items still behave identically.
- [x] 7.4 Run `openspec validate add-item-effect-targeting` and the repository's lint/observability
