...
- [x] 1.1 In `world/lore/sexual_vocab.py`, add `BODY_PARTS = ("口唇", "頸項", "耳朵", "乳房",
      "腰腹", "臀部", "大腿", "足部", "私處", "後庭")`.
- [x] 1.2 Add `GENERIC_BODY_PART = "軀體"`.
- [x] 1.3 Add both new names to the module's `__all__` list.
- [x] 1.4 Update the module docstring: keep the existing statement naming `CHARACTER_SCHEMA_V1` as
...
- [x] 1.5 Add a module-level `assert GENERIC_BODY_PART not in BODY_PARTS` immediately after both
...
- [x] 2.1 In `world/lore/tests/test_sexual_vocab.py`, add a test asserting `BODY_PARTS` equals the
      exact 10-tuple in order.
- [x] 2.2 Add a test asserting `GENERIC_BODY_PART` equals `"軀體"`.
- [x] 2.3 Add a test asserting `GENERIC_BODY_PART not in BODY_PARTS`.
- [x] 2.4 Confirm the existing "no import of a rules or imports module" test still covers the file
      as a whole (no per-symbol exemption needed — the constants introduce no new import).
- [x] 2.5 Confirm every existing assertion for the six original tuples is unchanged and still
      passes.
- [x] 2.6 Add a new test that reads `sexual_vocab.__doc__` directly and asserts it contains the
...
- [x] 3.1 Run `uv run --locked python -m tools.spec_traceability list` to get the current requirement
      IDs, then:
      ...
      - Add a `covers_requirement` decorator for the second requirement ("module documents itself
        ...
        test added in 2.6.
- [x] 3.2 Run `uv run --locked python -m tools.spec_traceability check` and confirm zero unknown-ID
      and zero uncovered-requirement errors.
- [x] 3.3 Run `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --keepdb
      world.lore`.
- [x] 3.4 Run `openspec validate sexual-body-parts --strict`.
