...
- [x] 1.1 Create `world/tests/synthetic_data.py`: `t_`-prefixed catalog dicts built from
...
- [x] 1.2 Add the `REGISTRY_TARGETS` table (including the `world/lore/sync.py`
...
- [x] 1.2b Add the idempotent `install_synthetic_catalogs()` process bootstrap (D2b):
...
- [x] 1.3 Add the `make_item` / `make_skill` / `make_region` / ... factory helpers with
  local-registration support (`extra=`)

## 2. JavaScript mirrors

- [x] 2.1 Create `web/webclient-app/tests/support/synthetic-data.mjs` and
...
- [x] 2.2 Add a Node-gate mirror self-test (e.g.
...
- [x] 3.1 Create `world/tests/test_synthetic_data.py` with
...
- [x] 3.2 Register `world/tests/test_synthetic_data.py` in exactly one shard of
  `.github/evennia-shards.json` and keep
  `tests.test_evennia_test_optimization_contract` green
- [x] 3.3 Run the focused label and
  `uv run --locked python -m tools.spec_traceability check`

## 4. Documentation

- [x] 4.1 Add the kit section to `docs/development/evennia-testing-guide.md` (when to
  use shared catalogs vs `make_*` local fixtures; the frozen-catalog seam; JS mirror
  rule)
- [x] 4.2 Run `openspec validate add-test-synthetic-data-kit --strict` and
  `git diff --check`
