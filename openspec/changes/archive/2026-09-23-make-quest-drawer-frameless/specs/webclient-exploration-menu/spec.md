...
- **THEN** neither the tab bar nor the keyboard root carries a 角色狀態, 任務, or 背包 entry - those surfaces are opened from the top navigation bar - and the remaining root entries keep their authored order
...
#### Scenario: The interact workspace pairs targets and affordances side by side
- **WHEN** the player opens Interact, points at or selects a present target, and the workspace renders that target's affordances
- **THEN** the target grid holds the first column, the selected target's heading and its single-column affordance rows hold the second column, keyboard target switching and vertical affordance navigation traverse the same frames the router owns, and the frame widens its dock band without overlapping the narrative caption
