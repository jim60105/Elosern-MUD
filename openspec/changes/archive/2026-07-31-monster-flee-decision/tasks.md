...
- [x] 1.1 Add `flee_hp_fraction` to every archetype in
...
- [x] 1.2 Add `flee_hp_fraction: float | None` to the frozen `BehaviourProfile`.
- [x] 1.3 Define `MonsterBehaviourConfigError(ValueError)` with stable prefix
...
- [x] 1.4 Test every shipped archetype and tier default, including that null disables flight and
      booleans, strings, out-of-range values, missing fields, and unknown references fail loudly.
- [x] 1.5 Test that an instance `behaviour_tree` override changes the flee threshold without changing
      the monster's tier or any persistent state.

## 2. Deterministic flee decision branch

- [x] 2.1 Add a focused predicate that reads `combat._stored_hp()` and `combat._max_hp()`, guards a
      non-positive maximum, and uses the inclusive current/max comparison from design D-2.
- [x] 2.2 Insert the flee predicate after non-monster delegation and living-enemy discovery but before
      damage-skill enumeration, area/target/skill selection, or seeded tie-breaking.
- [x] 2.3 Import 10c's `FLEE_SKILL_KEY` from `world.rules.disengage` and return a self-targeted
      `ActionRequest` using that constant with
      `BattlefieldActionContext(battlefield, event_context={"battlefield": battlefield})`.
- [x] 2.4 Test exact-boundary selection, above-threshold attack preservation, below-threshold
      selection, null disabling, and the non-positive-maximum guard.
- [x] 2.5 Test that a threshold-triggered monster can flee with no affordable damage skill, calls no
      policy `roll_d100()`, and returns `None` rather than flee when no living enemy remains.
- [x] 2.6 Test the existing non-monster delegation path and every above-threshold 10b target,
      skill-choice, area-choice, and fixed-seed golden test unchanged.
- [x] 2.7 Test that evaluating the branch leaves HP gauge backing data, `Battlefield.fled`, traits,
      sexual state, buffs, skill grants, inventory, and currency unchanged.

## 3. Resolver and combat integration

- [x] 3.1 Add a focused request-shape test for actor, self target, `"flee"` skill key, targeting
      battlefield, and `event_context["battlefield"]` identity.
- [x] 3.2 Add a fresh-interpreter import test proving importing `world.rules.monster_behaviour`
...
- [x] 3.3 Test a successful fixed-roll flee through `run_round()` and `ActionResolver`: the monster
...
- [x] 3.4 Test a failed fixed-roll flee through `run_round()`: the monster remains in the battlefield,
      attacks no target that turn, and emits 10c's failed-attempt EventLog.
- [x] 3.5 Test `resolve_overwhelm()` with the same policy: no special branch is added to
      `overwhelm.py`, and a successful flee affects the next existing reclassification.
- [x] 3.6 Add fixed-state/fixed-dice golden cases for each tier-default archetype and at least one
...
- [x] 4.1 Add source-scan assertions that monster behaviour imports no `world.ai`, performs no
...
- [x] 4.2 Run all 10b monster-behaviour and 10c combat-disengage focused tests together and resolve
      regressions without changing their ownership boundaries.
- [x] 4.3 Run `uv run --locked evennia test --settings settings.py .`.
- [x] 4.4 Run `uv run --locked -m unittest discover tests` and
      `uv run --locked python -m compileall -q world typeclasses commands server`.
- [x] 4.5 Run `openspec validate monster-flee-decision --strict` and `git diff --check`.
