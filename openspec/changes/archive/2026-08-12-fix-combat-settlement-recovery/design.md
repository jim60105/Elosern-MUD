...
- **Snapshot surface growth**: the snapshot set now includes all participants, the battlefield, the session room, every active quest stage room, and the actor's session/exam attributes; reuse the existing handler registration so new surfaces raise instead of silently missing. The friendly-fire scan's party/relations surfaces are snapshotted and restored alongside. Residual: quest-log effects targeting *other* player characters (impossible in single-player — the acting player is the only `PlayerCharacter`) are covered only by the per-effect commit snapshots, so an outer rollback leaves their in-process caches stale until reload; the database is always rolled back.
...
- **Exam opponents are deleted post-commit**: `settle_exam_outcome` no longer deletes the temporary opponent in its `finally`; `settle_session` deletes it only after the settlement transaction commits, so a rolled-back settlement keeps the opponent alive for exactly one retry and never strands a stale deleted instance in the idmapper cache.
- **Restore still terminates invalid sessions deterministically**: `restore_active_session` maps any reconstruction failure to defeat/exam-FAIL settlement; that is the pre-existing contract of `fix-startup-session-restore-order` (missing/deleted/moved/duplicated/malformed participants close the session), unchanged by this work. Only the already-settled marker short-circuits settlement.
