...
- [x] 1.1 In `_build_from_baseline()`, unconditionally add all eleven counter traits (design.md D-1
...
- [x] 2.1 Add eleven read-only properties (`masturbation_count`, `toy_use_count`,
...
- [x] 2.2 Add eleven mutators (`record_masturbation()`, `record_toy_use()`, `record_exposure_act()`,
      ...
      shipped.
- [x] 2.3 Update `__all__` in `sexual_state.py` only if any new name needs to be importable elsewhere
...
- [x] 3.1 One test per the delta spec's six scenarios: zero-start for all eleven; a mutator affects
      only its own counter; repeated calls accumulate linearly; `reset_daily_counters()` leaves all
      eleven untouched (every counter given a non-zero value first); `climax_count`/`record_climax_count()`
      is independent of the existing `climax_today`/`record_climax()`; a structural test grepping
      (or AST-inspecting) every non-`sexual_state.py` module across all production roots (`world/`,
      `commands/`, `typeclasses/`, `web/`, `server/`, `tools/`) for `._traits` access naming any of
      ...
      SexualState's private handler" requirement's own test). Plus a bounds test pinning each
      counter's `min == 0` and `max is None` (the requirement's "unbounded" clause).
- [x] 3.2 A single table-driven test iterating all eleven `(field, mutator)` pairs, calling each
...
- [x] 4.1 Run `uv run --locked python -m tools.spec_traceability list` and confirm the requirement id
...
- [x] 4.2 Run the focused test module:
      `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --keepdb
      world.rules.tests.test_sexual_state`.
- [x] 4.3 Run `uv run --locked python -m tools.spec_traceability check`.
- [x] 4.4 Run `openspec validate sexual-counters --strict`.
- [x] 4.5 Confirm `pleasure-gauge` (this proposal's dependency) is applied first, or that this
