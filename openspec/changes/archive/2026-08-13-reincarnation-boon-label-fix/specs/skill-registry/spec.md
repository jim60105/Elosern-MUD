...
declares that boon in `PLAYER_PRESET_REGISTRY`. Their keys, `effects`, costs, kinds, and target
specs SHALL NOT change. The derived `status_display.yaml` row `reincarnation_boon_yuka_agility_bonus`
...
- **THEN** the label equals exactly `轉生祝福·<display_name>` (轉生祝福·悠花, 轉生祝福·悠奈,
  ...
  `growth_rate:magic:100` / `combat_prediction:武感` / `sexual_magic_mastery` respectively)
