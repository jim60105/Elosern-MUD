...
- [x] 1.1 Confirm `world/rules/` holds change 8's `action.py`/`targeting.py`/`event_log.py`, change 6's
      ...
      new files.
- [x] 1.2 Confirm the exact import paths for `world.rules.action.{ActionResolver, ActionRequest,
...
- [x] 1.3 Confirm `evennia.contrib.rpg.dice.roll()`'s exact call signature and whether it exposes a
      seed/RNG-injection parameter, per design.md D-1.

## 2. Dice roller (`world/rules/dice.py`)

- [x] 2.1 Implement `roll_d100() -> int` wrapping `evennia.contrib.rpg.dice.roll()`, per design.md D-1.
- [x] 2.2 Confirm reproducibility under a fixed seed (Python's `random.seed()` if the contrib exposes no
...
- [x] 3.1 Author `to_hit.defender_constant: 51` with the design.md D-2 derivation referenced in an
      inline YAML comment (not restated in full — point to design.md).
- [x] 3.2 Author `damage.{crit_multiplier, solid_hit_margin, solid_hit_multiplier, base_multiplier,
      floor}` per design.md D-3, flagged inline as invented placeholder values.
- [x] 3.3 Author `initiative.agility_weight: 10` per design.md D-8.
- [x] 3.4 Author `round.seconds: 6` per design doc §6.3.
- [x] 3.5 Implement a small loader in `world/rules/combat.py` (`COMBAT_YAML = yaml.safe_load(...)`) —
...
- [x] 4.1 Implement `_apply_percent_mod(base: float, pct: str | None) -> float` parsing
      `combat_modifiers.yaml`-style percentage strings (e.g. `"-20%"`) against a base value, returning
      `base` unchanged when `pct` is `None`.
- [x] 4.2 Implement the to-hit calculation exactly as design.md D-2/D-5 specify: `roll_d100() +
...
- [x] 4.3 Implement `_roll_multiplier(raw_roll: int, margin: int) -> float` per design.md D-3: returns
      `crit_multiplier` if `raw_roll == 100`, else `solid_hit_multiplier` if `margin >=
      solid_hit_margin`, else `base_multiplier`.
- [x] 4.4 Implement the damage number: `max(round(effective_attack_stat * roll_multiplier) -
...
- [x] 5.1 Implement `effective_power(entity) -> float` per design.md D-4: sum of
...
- [x] 5.2 Construct the four worked reference cases from design.md D-4 (human elite, elf, mid-tier
...
- [x] 6.1 Implement `Battlefield` (frozen or plain dataclass): `teams: dict[str, frozenset[str]]`
      (exactly two team keys), `roster: dict[str, LivingEntity]`, `fled: set[str]`, and
      `team_of(key) -> str | None`, per design.md D-6.
- [x] 6.2 Implement `BattlefieldActionContext` conforming to `world.rules.targeting.ActionContext`:
      ...
      `is_present()` checks roster membership only, while `is_in_range()` returns `False` for any
      target in `battlefield.fled` and `True` otherwise — melee-vs-ranged explicitly not built.
- [x] 6.3 Confirm `expand_target_shorthand()` resolves `"all-enemies"`/`"all-allies"`/`"all"`
      correctly against `Battlefield.teams`; correct the landed roster-dict iteration to use
      `.values()` so the existing validators receive entity objects.

## 7. damage:* effect handler (`world/rules/combat.py`)

- [x] 7.1 Implement `_handle_damage(actor, targets, effect_id, event_context) -> list[PendingEffect]`
      per design.md D-5: parses the landed seed convention `damage:<element>:<school>`; for each
      target, computes
...
- [x] 7.2 Implement `_apply_hp_delta(entity, delta) -> None`: the only mutator any `PendingEffect.apply`
      from this handler calls, writing through `entity.traits.hp.value`.
- [x] 7.3 Call `register_effect_handler("damage", _handle_damage, surfaces=frozenset({"traits"}))` at
      module import time — confirm it raises no `UnsnapshottedSurfaceError` (traits is already in
      change 8's `SNAPSHOTTED_SURFACES`).
- [x] 7.4 Confirm every stat `_handle_damage` reads (`atk_phys`/`magic_level`, `agility`, `defense`)
...
- [x] 7.5 Confirm `evaluate_combat_modifiers()` is read for both the acting entity and each target, and
...
- [x] 7.6 Extend change 8's `_entries_from_effect()` conversion seam for structured
      `damage|target|roll|hit|amount` descriptions, emitting a `"roll"` entry and a `"damage"` entry
      on hits without moving combat math into `action.py`.

## 8. Initiative and the turn loop (`world/rules/combat.py`)

- [x] 8.1 Implement `roll_initiative(battlefield) -> list[str]` per design.md D-8.
- [x] 8.2 Implement `default_attack_policy(entity, battlefield) -> ActionRequest | None` per design.md
...
- [x] 8.3 Implement `run_round(battlefield, action_provider) -> list[EventLog]` per design.md D-9: for
...
- [x] 8.4 Implement `_end_of_round_upkeep(battlefield) -> None`: calls `tick_buffs(entity)` and
      `world.rules.sexual_state.decay_tick(entity, COMBAT_YAML["round"]["seconds"])` for every living
      roster member.
- [x] 8.5 Implement `is_battle_over(battlefield) -> bool` (or equivalent): a side with zero living,
      non-fled members ends the battle.
- [x] 8.6 Implement `run_battle(battlefield, action_provider, max_rounds) -> BattleResult` per design.md
...
- [x] 9.1 `world/rules/tests/test_dice.py` — `roll_d100()` stays in `[1, 100]`; reproducibility under a
      fixed seed; per the `dice-roller` capability.
- [x] 9.2 `world/rules/tests/test_to_hit_calibration.py` — per the `combat-resolution` capability's
      ...
      the exact ranges design.md D-2's final table states; a 50+ point agility gap saturates fully in
      both directions; a natural 100 does not override a saturated miss.
- [x] 9.3 `world/rules/tests/test_damage_bands.py` — per the damage-multiplier requirement: bare hit,
...
- [x] 9.4 `world/rules/tests/test_effective_power.py` — per the `effective_power` requirement: reads
      ...
      change when an `effective_value()` output changes (a multiplier skill activating); and the
      elf-at-low-current-hp counterexample from design.md D-4 — computed with `.max`
      hp, the elf/human ratio does not flip toward the human.
- [x] 9.5 `world/rules/tests/test_battlefield_action_context.py` — per the `battlefield-action-context`
      capability: full protocol conformance; `relation_to()`'s three-way truth table over team
      membership; `is_present()` checks roster membership and `is_in_range()` owns fled status; the out-of-range
...
- [x] 9.6 `world/rules/tests/test_damage_effect_handler.py` — per the `damage-effect-handlers`
...
- [x] 9.7 `world/rules/tests/test_initiative_and_turn_loop.py` — per the initiative and turn-loop
      ...
      `ActionResolver.resolve()` and emits an `"action_skipped"` entry; `tick_buffs()` and
      `sexual_state.decay_tick(..., 6)` each run once per living entity every round.
- [x] 9.8 `world/rules/tests/test_golden_combat.py` — per design.md D-10: the normal-exchange golden
...
- [x] 10.1 Run the full `world/rules/tests/` suite added by this change and confirm every test passes.
- [x] 10.2 Confirm `world/rules/combat.py` never calls `WorldClock.advance()` or anything resembling it
      (grep-based check).
- [x] 10.3 Confirm `register_effect_handler` is the only way `world/rules/combat.py` registers into
...
- [x] 10.4 Confirm `git diff --stat` contains only the three new runtime files, this change's tests
      and artifacts, plus the two reviewed change-8 seam corrections in `world/rules/action.py` and
      `world/rules/targeting.py`.
- [x] 10.5 Confirm change 8's own no-combat-branching tripwire test suite
      (`test_no_combat_branching.py`) still passes unmodified — the reviewed event-conversion and
      shorthand edits introduce no combat-state branch into the resolver pipeline.
- [x] 10.6 Run `openspec validate dice-combat --strict` and confirm it passes.
