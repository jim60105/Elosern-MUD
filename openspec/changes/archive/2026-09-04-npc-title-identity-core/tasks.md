...
- [x] 6.2 **批次整合契約（三件 change 共用，rubber-duck 複審後定案）**：`openspec/specs/npc-identity-titles/spec.md` 只有單一寫入者——批次最後落地的那個 change（建議批次順序下為 `npc-title-authored-identities`）。三件一起走時，本 change **不自行 sync delta、不掛 covers_requirement、不跑覆蓋 check**；本檔 6.3 的錨定測試映射表是整合步驟的依據。僅當本 change 單獨實作／歸檔時，才依 `.agents/skills/openspec-sync-specs` 同步 delta 並以 `uv run --locked python -m tools.spec_traceability list` 取 canonical ID 後執行 6.3。**實際走單獨歸檔路徑**：歸檔時依使用者指示單獨 sync 本 change——新建 `openspec/specs/npc-identity-titles/spec.md`（Purpose＋9 條需求，self-contained 全文搬移；change 2／3 的 delta 屆時對該 main spec 做 MODIFIED/ADDED）。
- [x] 6.3 掛 `covers_requirement` 標註（參數必須是字面值 ID，`from tools.spec_traceability import covers_requirement`；ID 於 sync 後取）：驗證器→1.5；組合器→1.6；屬性與無寫入面→2.5／2.6；opt-in 顯示路由→3.2／3.3；webclient 面板→4.2；緊湊列→5.1／5.2；prompt context→2.7；目標解析→5.3；既有契約不變→3.2 的框架迴歸案。單獨執行時跑 `uv run --locked python -m tools.spec_traceability check` 至零錯誤、九條 requirement 全覆蓋。**已執行**：以 `tools.spec_traceability list --json-output` 取 9 條 canonical ID，掛標 49 處（validator 8→`npc-titles-are-validated-single-line-plain-text`；composer 10＋面板邊界共標→`a-single-deterministic-composer-renders-the-npc-full-identity`；write-surface 3→`the-npc-title-is-a-creation-time-attribute-with-no-runtime-write-surface`；appearance 5＋echo 5＋follow-lost 1→`full-identity-appears-only-on-opt-in-text-display-surfaces`；panel 4→`the-webclient-exploration-panel-renders-the-npc-full-identity-on-entity-and-interact-rows`；combat／art／service 各 1→`compact-presentation-rows-keep-the-plain-npc-name`；dialogue 4→`the-npc-dialogue-context-carries-the-title-as-read-only-data`；targeting 1→`command-and-search-targeting-matches-the-plain-npc-key-only`；三路徑框架迴歸共標→`the-existing-appearance-and-exploration-contracts-are-unchanged`）。check 結果 1210/1210 covered、0 uncovered、0 errors。
