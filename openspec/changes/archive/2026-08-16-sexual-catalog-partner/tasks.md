...
- [x] 1.1 Confirm `sexual-act-seeds` has landed with `PARTNER_ACTS` containing exactly
...
- [x] 1.2 Confirm `world/rules/sexual_state.py`'s `duo_act_count`/`group_act_count`/`climax_count`
...
- [x] 1.3 Confirm `_act_family()`'s structural requirement that a non-`SELF`/`NONE`, non-異種/
...
- [x] 1.4 Confirm `world/rules/rulebook/sexual.yaml`'s `experience_titfuck_added` row
...
- [x] 1.5 Confirm `_handle_sexual_event`'s target-only application (`world/rules/action.py`,
...
- [x] 2.1 Extend `PARTNER_ACTS`'s `_act_family("關係", ...)` call with the four Tier 1 rows from
...
- [x] 3.1 Add `partner_deep_caress`, `partner_oral_service`, `partner_thigh_rub`, and
  `partner_foot_service` (`unlock={"duo_act_count": 15}`, otherwise matching Tier 1's shape).
- [x] 3.2 Add `partner_breast_sex` (`unlock={"duo_act_count": 15}`,
...
- [x] 4.1 Add `partner_anal_sex` (`unlock={"duo_act_count": 30, "climax_count": 10}`,
...
- [x] 4.2 Add `partner_mutual_masturbation` (same unlock, `actor_part=target_part="私處"`,
...
- [x] 5.1 Add `partner_group_caress` (`unlock={"duo_act_count": 30}`,
...
- [x] 5.2 Add `partner_group_orgy` (`unlock={"group_act_count": 15}`, `base_pleasure=20`,
  otherwise matching 5.1's shape).
- [x] 5.3 Add `partner_group_service` (`unlock={"group_act_count": 30}`, `base_pleasure=22`,
  otherwise matching 5.1's shape).

## 6. Collateral test-surface update (pinned per-category key set)

The empty-`unlock`-subset collateral `sexual-catalog-solo`/`sexual-catalog-shame` already
shipped; the only remaining per-change collateral is the pinned `SkillCategory.SEXUAL_ACT`
key set in `world/skills/tests/test_registry.py`, which breaks the moment any new act registers.

- [x] 6.1 In `world/skills/tests/test_registry.py`, extend the pinned
  `SkillCategory.SEXUAL_ACT` key set in `test_per_category_key_sets_match_the_d4_classification_table`
  with this change's fourteen keys (matching `test_registry_structure.py`'s key/registry
  agreement check).
...
- [x] 7.2 Add `world/skills/sexual_acts/tests/test_partner_catalog.py` (new `EvenniaTest`-based
  ...
  `SEXUAL_ACT_REGISTRY` before this change. The cast tests force a compliant resist roll
  (`patch("world.rules.action.roll_d100", return_value=1)`) and call `register_catalog()` in
  setUp, because `sexual-resist-cast-wiring` merged mid-flight (design D-6's last bullet).
- [x] 7.3 Add one test (not in the delta spec, a design.md D-3 regression) proving
...
- [x] 7.4 Apply `covers_requirement("sexual-catalog-partner::<id>")` (using the IDs from 7.1) to
  each test function whose assertions establish that requirement.
- [x] 7.5 Run `uv run --locked python -m tools.spec_traceability check` and confirm every
...
- [x] 8.3 Run `openspec validate sexual-catalog-partner --strict` and resolve any reported issue.
- [x] 8.4 Confirm no file outside `world/skills/sexual_acts/partner.py`, the new
  `world/skills/sexual_acts/tests/test_partner_catalog.py`, the section-6 collateral test file
  (`world/skills/tests/test_registry.py`), and this change's own artifacts (`proposal.md`/`tasks.md`
  edits and the synced `openspec/specs/sexual-catalog-partner/spec.md`) was touched, matching the
  amended proposal Impact list exactly — in particular, confirm `world/rules/action.py` and
  `world/rules/rulebook/sexual.yaml` both have zero diff from this change.
