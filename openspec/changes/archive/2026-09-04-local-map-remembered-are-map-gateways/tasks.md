...
- [x] 1.1 Add a module-level helper `_registered_gateways()` to
  ...
  one-`#` registry.
- [x] 1.2 Add `_wilderness_gateway_at(x, y)` and `_grid_gateway_at(x, y, z)`
...
- [x] 1.3 Add `_gateway_far_side_label(...)` implementing design D5: the
  ...
  name.
- [x] 1.4 Add the distinctness qualifier from design D5: given the ordered list
  ...
  gate room's `key`, **applied on BOTH the grid layer and the wilderness
  layer** — a `WildernessEntryPoint` can register more than one gate, so the
  entry's anchor display name is NOT unique on the wilderness layer either
  (rubber-duck run 2 caught this: the original draft of this task claimed it
  was). Proof: a unit test remembers both capital gate rooms on the grid layer
  and asserts the two labels are 「西部丘陵與谷地（南門）」 and 「西部丘陵與谷地（北門）」;
  a second test stands on both capital approach cells, walks away from both,
  and asserts the two wilderness-layer remembered labels are
  「聖潔王都（南門）」 and 「聖潔王都（北門）」, distinct and each within
  `MAX_STRING_CODE_POINTS`.

## 2. The bound, the ordering, and the shared candidate filter

- [x] 2.1 Add `MAX_REMEMBERED_GATEWAYS = 16` beside the existing bounds in
...
- [x] 2.2 Leave `_GraphBuilder.remembered(cap)`'s ordering contract exactly as
...
- [x] 2.3 In both coordinate-layer remembered loops, cap the emitted set at
...
- [x] 3.1 Replace `_wilderness_layer`'s remembered loop (`local_map.py:843`) so a
...
- [x] 3.2 Emit that node at the approach cell's own wilderness coordinates, with
  ...
  and the three flags.
- [x] 3.3 Add the negative case: a player whose record contains the gate room's
...
- [x] 3.4 Add the seven-chips regression test: a record seeded with eight visited
...
- [x] 4.1 Replace `_grid_layer`'s remembered loop (`local_map.py:477`) so a
...
- [x] 4.2 Emit that node at the gate room's own grid coordinates with 1.3's grid
...
- [x] 4.3 Drop a candidate whose `z_map_key` is not the map being drawn, with no
...
- [x] 4.4 Pin that an `AnchorRoom` alone is not a gateway: a visited plaza
...
- [x] 4.5 Confirm `_interior_graph`'s remembered loop is untouched and its
...
- [x] 5.1 Add the cross-space guard test: build a wilderness payload for a player
  ...
  position".
- [x] 5.2 Add a test asserting no remembered node ever lands on a
...
- [x] 5.3 Re-run the whole presenter suite and fix fallout in
...
- [x] 5.4 Update `web/tests/browser/test_browser_local_map.py`'s
...
- [x] 6.1 In `_wilderness_layer`'s in-view neighbour loop, label a neighbour cell
  ...
  validator.
- [x] 6.2 In `web/webclient-app/components/MapLattice.vue`, on a `wilderness`
  payload (`props.localMap.layer === "wilderness"`) only, suppress the visible
  ...
  draws its label. The suppression does NOT apply on `grid`/`instance`/
  `interior` payloads, where a label names an individual room rather than a
  shared region, so a coincidental name match between two distinct rooms must
  still draw both labels. Proof: `web/webclient-app/tests/world/local_map.test.js`
  — a wilderness payload whose nine in-view nodes share one label renders
  exactly one visible label and nine `<title>` elements, and a `grid` payload
  with two same-labelled nodes still draws both visible labels.
- [x] 6.3 Confirm no payload field, bound, or validator changed: node labels
...
- [ ] 6.4 Add the browser gate: the wilderness island shows one region name, not
  ...
  node's text.
  **DEFERRED** (implementer note): the shared minimap browser server's seed
  fixture leaves the character at 南門 (grid layer), not in the wilderness, and
  wilderness rooms are pooled/dynamic (no stable room key to `teleport` to);
  reaching a plain, unobstructed same-region 3×3 neighbourhood would require
  chaining several real in-game directional moves through the shared class-
  scoped server. The underlying logic is covered instead by
  `test_an_in_view_gate_approach_cell_names_the_place_behind_it` /
  `test_a_cell_in_a_different_region_still_says_so` (Python, wave 6.1) and by
  the two new `map_lattice.test.js` cases (wave 6.2); this end-to-end browser
  confirmation is left for whoever lands this wave's follow-up or strikes it.

## 7. Verification and suite gates

- [x] 7.1 `evennia test --settings settings.py web.webclient.presentation.tests.test_local_map`
...
- [x] 7.2 `evennia test --settings settings.py web.tests.browser.test_browser_local_map`
  and `web.tests.browser.test_browser_contextual_hud` green.
- [x] 7.3 `npx vitest run` and `node --test web/static/webclient/js/tests/*.test.js`
  green — untouched unless wave 6 lands.
- [x] 7.4 `tests/test_map_knowledge_contract.py` and
...
- [x] 7.5 `openspec validate local-map-remembered-are-map-gateways --strict`
  green, and the spec-test traceability gate (`uv run --locked python -m
  tools.spec_traceability check`) resolves every `@covers_requirement` anchor
  added in waves 3–5 (the two MODIFIED requirements, already live in the main
  spec). Wave 6's two tests carried no anchor for the ADDED requirement while
  it existed only in this change's delta spec — the gate indexes
  `openspec/specs/` only, so an anchor there would have reported
  `unknown-requirement-id`. At archive/sync time,
  `@covers_requirement("webclient-local-map::the-map-surfaces-state-a-place-name-only-where-it-adds-information")`
  was added to `test_an_in_view_gate_approach_cell_names_the_place_behind_it`
  and `test_a_cell_in_a_different_region_still_says_so` once the sync made the
