...
- [x] 1.1 In `web/webclient-app/tests/world/map_lattice.test.js`, add a
...
- [x] 1.2 Add the same baseline for the island's props (`markerNames: true`,
...
- [x] 2.1 In `web/static/webclient/js/elosern/local_map.js`, add `span:
...
- [x] 2.2 In the same Node gate, correct the surface table's `overlay` row from
...
- [x] 2.3 In `web/webclient-app/components/MapLattice.vue`, delete
...
- [x] 3.1 In `web/webclient-app/components/MapOverlay.vue`, add
...
- [x] 3.2 In `MapLattice.vue`, bind `:style="{ fontSize: `${markerNameFont}px` }"`
...
- [x] 4.1 In `MapLattice.vue`, replace the bare `11` in `layoutGeometry`'s
...
- [x] 4.2 In `MapLattice.vue`, delete the
...
- [x] 4.3 In the same function, replace `budget` with design D2's two-term
...
- [x] 4.4 Confirm the anti-ambiguity pass now covers the overlay because it sits
...
- [x] 4.5 Assert design D5's monotonicity directly: on any edge where the island
  truncates marker names (such as the crowded wilderness payload where the
  island's per-marker budget is 5–6 glyphs), the overlay's per-marker glyph budget
  is strictly greater than the island's on every edge that carries a marker. Proof:
  `map_lattice.test.js` — both surfaces rendered over crowded wilderness payloads,
  budgets compared per marker id.

## 5. Wave 5 — the browser gate

- [x] 5.1 In `web/tests/browser/test_browser_local_map.py`, add a crowded-edge
  overlay case under
...
- [x] 5.2 Re-run `web.tests.browser.test_browser_local_map` unchanged otherwise,
...
- [x] 6.1 `npx vitest run` green, and `web/webclient-app/tests/world/map_lattice.test.js`,
...
- [x] 6.2 `node --test web/static/webclient/js/tests/*.test.js` green and still
  ...
  rows already there.
- [x] 6.3 `npm run build-storybook && npm run showcase-coverage` green — no new
...
- [x] 6.4 `uv run --locked python -m tools.spec_traceability check` green — the
  ...
  `webclient-local-map::the-browser-minimap-renders-states-without-relying-on-color-alone`
  anchor stays valid.
- [x] 6.5 `openspec validate webclient-minimap-07-overlay-marker-name-fit --strict`
  green.
