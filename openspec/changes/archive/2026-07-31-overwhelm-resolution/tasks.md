...
- [x] 1.1 Confirm `world/rules/` holds change 9's `combat.py` (`effective_power`, `run_round`,
...
- [x] 1.2 Confirm the exact import paths and public names for `world.rules.combat.{effective_power,
...
- [x] 1.3 Inspect change 9's landed `_step7_build_event_log`-equivalent construction (however
      `run_round()` actually turns a `damage:*` `PendingEffect` into `EventEntry` instances) to confirm
      the `"roll"`/`"damage"` kind split and their landed data fields. The inspection found hit rolls
      carry `hit=True` plus a paired damage amount, while misses exist only as `roll` entries with
      `hit=False`; preserve those miss rolls and count damage entries as successful hits.

## 2. Rulebook data (`world/rules/rulebook/overwhelm.yaml`)

- [x] 2.1 Author `power_ratio_threshold: 100` with the design.md D-1/D-2 derivation referenced in an
...
- [x] 2.2 Author `max_estimated_rounds: 5` with design.md D-1's calibration table referenced in an
      inline YAML comment — the value that makes overwhelm mean "decided and quick," not merely
      "decided."
- [x] 2.3 Implement a small loader in `world/rules/overwhelm.py` (`OVERWHELM_YAML =
      yaml.safe_load(...)`), mirroring change 9's `COMBAT_YAML` loader pattern exactly.

## 3. The power-ratio signal (`world/rules/overwhelm.py`)

- [x] 3.1 Implement `team_effective_power(battlefield, team_key) -> float` per design.md D-1: sums
...
- [x] 3.2 Implement `power_ratio_verdict(battlefield, team_a, team_b) -> str | None` per design.md D-1:
...
- [x] 3.3 Test: a team whose aggregate `effective_power()` is ≥100x the other's yields that team as the
...
- [x] 4.1 Implement `_agility_saturation(attacker, defender) -> Literal["hit", "miss", "contested"]`
...
- [x] 4.2 Implement `_adjusted_agility(entity) -> float` as the shared helper both this signal and the
...
- [x] 4.3 Implement `hit_rate_verdict(battlefield, team_a, team_b) -> str | None` per design.md D-1:
      ...
      members.
- [x] 4.4 Test: a fully-saturated one-directional matchup yields the correct team key; a single
...
- [x] 5.1 Implement `_expected_damage_per_attack(attacker, defender) -> float` per design.md D-1: the
      ...
      **only** `COMBAT_YAML["damage"]["base_multiplier"]` (never `solid_hit_multiplier` or
      `crit_multiplier`) — a calibrated base-attack estimate. Makes zero
      calls to `roll_d100()`.
- [x] 5.2 Implement `estimated_rounds_to_conclude(battlefield, overwhelming_team, overwhelmed_team) ->
      ...
      `ZeroDivisionError`).
- [x] 5.3 Test: the four calibration-table matchups from design.md D-1 (elf vs. human elite ≈1.5
      ...
      doc's worked figure.
- [x] 5.4 Test that `estimated_rounds_to_conclude()` is current-hp-sensitive by design (decreases as the
...
- [x] 5.5 Test no `roll_d100()` call occurs anywhere in `_expected_damage_per_attack()` or
      `estimated_rounds_to_conclude()` (source-inspection or call-count assertion).

## 6. Combining the signals (`world/rules/overwhelm.py`)

- [x] 6.1 Implement `_decided_direction(battlefield, team_a, team_b) -> str | None` per design.md D-1:
...
- [x] 6.2 Implement `classify_overwhelm(battlefield) -> str | None` per design.md D-1: computes
...
- [x] 6.3 Test every combination from the `overwhelm-threshold` spec: ratio-only fires (and is within
...
- [x] 6.4 Test the round-bound gate specifically: a decided direction whose `estimated_rounds_to_
      ...
      bound).
- [x] 6.5 Test `classify_overwhelm()` is a pure function with no cached state: two consecutive calls on
...
- [x] 7.1 Implement `OverwhelmResult` (frozen dataclass): `event_logs: tuple[EventLog, ...]`,
      `rounds_elapsed: int`, `total_seconds: int`, `overwhelming_team: str | None`,
      `verdict_after: str | None`, `battle_over: bool`, per design.md D-3.
- [x] 7.2 Implement `resolve_overwhelm(battlefield, action_provider, max_rounds=12) -> OverwhelmResult`
...
- [x] 7.3 Confirm (grep-based check, mirroring change 9's own task 10.2 discipline) that
...
- [x] 7.4 Test the exact-consistency property (design.md D-3/D-5, `single-shot-resolution` spec): on a
...
- [x] 7.5 Test early-exit-on-reclassification: construct a fixture where a mid-loop `effective_value()`
      change (a buff wearing off during `run_round()`'s own upkeep, or a stat-multiplier activation) flips
      ...
      `overwhelming_team`.
- [x] 7.6 Test `total_seconds == rounds_elapsed * 6` and the golden single-round case: the elf-vs-
...
- [x] 7.7 Test the bounded-curbstomp golden case (design.md D-5): a decided, multi-round-but-still-
...
- [x] 7.8 Test the two boundary early-return cases: `resolve_overwhelm()` on an already-contested
...
- [x] 7.9 Test the `max_rounds` safety-cap golden case (design.md D-5): a constructed fixture where
...
- [x] 8.1 Implement `compress_event_logs(raw_logs, overwhelming_team, overwhelmed_team, rounds) ->
      tuple[EventLog, ...]` per design.md D-4: filters every `EventLog.entries` to drop successful
      `kind=="roll"` entries while retaining miss rolls via `dataclasses.replace()`; drops any
      resulting `EventLog` left with zero entries; builds
      ...
      `rounds`, `hits` (count of `"damage"`-kind entries), and `total_damage` (sum of their `amount`
      fields); prepends it to the filtered tuple.
- [x] 8.2 Confirm (via `git diff` / source inspection) that `world/rules/event_log.py` is not modified
...
- [x] 8.3 Test: a successful `"roll"`-kind entry is dropped; a miss `"roll"` entry survives unchanged;
      an all-successful-`"roll"` input `EventLog` is dropped entirely from the output; the summary
...
- [x] 8.4 Test fidelity: every individual `"damage"`-kind entry from the input is still present,
...
- [x] 9.1 Test `event_log.render_plain_text()` (change 8, unmodified) against the summary `EventLog`
...
- [x] 9.2 Test that joining `render_plain_text()` over every `EventLog` in a compressed encounter's
...
- [x] 9.3 Test `render_plain_text()` is called twice on the same compressed `EventLog` and returns
...
- [x] 10.2 Confirm `world/rules/overwhelm.py` never calls `WorldClock.advance()` or anything resembling
      it (grep-based check).
- [x] 10.3 Confirm this change modifies no file authored by any earlier change — `git diff --stat`
...
- [x] 10.4 Confirm change 9's own golden and calibration test suites
...
- [x] 10.5 Confirm change 8's own `EventLog`/`event_log.py` test suite still passes unmodified, and that
      a diff of `event_log.py` against the pre-change tree is empty.
- [x] 10.6 Confirm no fixture or test in this change's own suite requires more than `max_rounds` (12)
...
- [x] 10.7 Run `openspec validate overwhelm-resolution --strict` and confirm it passes.
