...
- [x] 1.1 Create `prompts/action_options.yaml` (schema_version 1) with `action_options.system` and
...
- [x] 1.2 Register both `PromptSpec` entries in `world/prompts/registry.py` with
...
- [x] 1.3 Extend `world/prompts/tests/test_loader.py` (every-layer-has-a-prompt parity,
...
- [x] 2.1 Add `"action_options"` to `LAYER_NAMES` in `world/ai/profiles.py`.
- [x] 2.2 Extend `default_profiles()` with the action_options defaults: `temperature` 0.7,
  `max_tokens` 320, `supports_response_format: true`, remaining fields shared with the
  local-first default.
- [x] 2.3 Add a per-layer required-flag constant map and enforce it in `build_profiles`
  (raise `ProfileValidationError` naming layer and field on violation).
- [x] 2.4 Add the import-time validation call in `server/conf/settings.py` **after every settings
  override** (one `build_profiles(LLM_PROFILES)` call at the end of the module, past the
  `secret_settings` block), so a disabled structured-output action_options slot fails at startup
  even when it arrives through the override mechanism.

## 3. Tests and verification

- [x] 3.1 `world/ai/tests/test_profiles.py`: action_options slot present in the effective
...
- [x] 3.2 Update the `llm-profiles` layer-enumeration assertion in the profile/prompt tests and
...
- [x] 3.3 Assert the effective `settings.LLM_PROFILES` exposes the slot (no other settings diff
  required).
- [x] 3.4 Run the owned package tests:
