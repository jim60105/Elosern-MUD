...
- [x] 1.1 In `web/webclient-app/components/MapLattice.vue`, add a `markerNames`
  ...
  the change.
- [x] 1.2 In the same file, keep `nameWidth: 0` for the island (`markerNames` without
...
- [x] 1.3 In the same file, place the island's left/right marker names **along** the band as
...
- [x] 1.4 In the same file, set the island's marker-name font to 13 user units (rendering at
...
- [x] 2.1 In `MapLattice.vue`, compute each marker's free span along its own edge from the
...
- [x] 2.2 In the same file, enforce the anti-ambiguity invariant: if fitting would make two
...
- [x] 2.3 Add the untruncated payload label as each island marker group's `<title>`, as the
...
- [x] 2.4 Pin the overlay disclosure path (design D8): assert that under `overlayChrome` the
...
- [x] 3.1 In `web/webclient-app/components/LocalMap.vue`, render `ul.local-map__remembered`
...
- [x] 3.2 In the same file, pass `:marker-names="true"` to `MapLattice` and render a
...
- [x] 3.3 In the same file, keep the island's SVG marker layer `aria-hidden="true"` and
...
- [x] 4.1 In `LocalMap.vue`, replace `measureCanvasBudget()`'s
  ...
  yields 144.
- [x] 4.2 Add a regression asserting the gutter cannot breach the cap: a lattice payload whose
...
- [x] 5.1 In `web/static/webclient/js/tests/local_map.test.js`, change the packing-invariant
...
- [x] 5.2 Update `web/tests/browser/test_browser_local_map.py`: assert the wilderness island
...
- [x] 5.3 Update `web/tests/browser/test_browser_contextual_hud.py` for the island's keyboard
...
- [x] 5.4 Run the full set — `npx vitest run`, `node --test web/static/webclient/js/tests/*.test.js`,
...
- [x] 6.1 Update `docs/design/elosern-redesign/REDESIGN.md` §7.2's "Accessibility floor"
