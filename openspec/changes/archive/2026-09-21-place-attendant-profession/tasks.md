...
- [x] 1.1 Create the `world/lore/dialogue/` package: `shape.py` holding `KeywordResponse` and
  ...
  content changes to fill. The moved row keeps every contract-pinned substring the
  scripted-dialogue spec requires (the `回報` keyword, the register-first fallback, the
  taught `guild` commands) but is re-authored in the clerk's in-character voice and
  capped at the dialogue panel's four pressable keyword choices (the session ruling).
...
- [x] 2.1 Add the `attendant` row to `world/rules/rulebook/professions.yaml`: one
...
- [x] 2.2 Cover the blueprint over a synthetic place row: a host that carries the dialogue
  ...
  shipped key.
- [x] 2.3 Cover the two rejections with synthetic rows — an attendant authoring `shop_key`
...
- [x] 3.1 In `validate_service_hosts`, after blueprint coverage resolves each component's
...
- [x] 3.2 Import the registry function-locally inside `validate_service_hosts`, matching how
  that function already reaches `profession_config`, so the config module keeps its import
  shape.
- [x] 3.3 Cover the rejection with a synthetic place row naming an unregistered key, and cover
...
- [x] 4.1 Assert the derived roster after this change equals the roster before it, field by
  field, and that no shipped row names `attendant`. This is the neutrality gate: the blueprint
  ships unused.
- [x] 4.2 Run the guild-config, guild-economy-sync and scripted-dialogue suites, plus
