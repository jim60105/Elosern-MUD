...
- [x] 1.1 Confirm C4a (`webclient-avg-stage-shell`) is archived, or will be before this change (archive order C1 → C2 → C3 → C4a → C4b → C4c).
...
- [x] 2.1 `styles/tokens.css`: `--header-h: 48px`, and add `--place-h: 68px` with a comment naming design D3.
  ...
  - in the 1350px block, shrink the brand row so it fits the 216px column: `.topbar-wordmark` 16px with `.2em` tracking, `.elosern-root .topbar-brand` `padding-inline: 16px`
  - delete the `.topbar-meta .meta-clock` / `.meta-loc` / `.sep` rules in the base, 1350px, and 1000px blocks, and turn `.elosern-root .topbar-meta` from the two-row grid into a plain flex row (drop `.meta-conn`'s grid placement)
  - delete the 1350px `.desktop-navigation button` width override: the component's own row rule wins at equal specificity, and the row buttons size to their labels

  `stories/Core/DesktopNavigation.stories.js`: the template's `--header-h` becomes `48px`.
- [x] 2.2 `components/TopBar.vue`:
...
- [x] 2.3 `components/DesktopNavigation.vue`:
...
- [x] 3.1 Add `web/webclient-app/components/PlaceCard.vue` per design D3:
...
- [x] 3.3 `AppClient.vue`: delete the `.scene-heading` block from `#backdrop`.
...
- [x] 3.4 `stories/Core/HudFrame.stories.js`: render sample place-card content in the new slot.
...
- [x] 4.1 Add `web/webclient-app/tests/place_card.test.js`:
  - both labels render
...
- [x] 4.3 Add `tests/desktop_navigation.test.js`: in exploration and combat modes, no button's text is `探索` or `戰鬥`; the component declares no `home` emit; activating 任務 emits `navigate` with `quests`; the map button is absent in combat.
...
- [x] 5.2 Add `test_stage_box_is_at_least_65_percent_at_the_reference_viewport` to `test_browser_contextual_hud_stage.py`, annotated `webclient-contextual-hud::the-webclient-renders-a-full-bleed-cinematic-stage-with-anchored-hud-surfaces`. At 1920x1080 in exploration, assert:
  ...
  - at 1280x720, the `.topbar-brand` content box does not overflow its column and does not intersect `.desktop-navigation` (the brand row fits the 216px column)
...
- [x] 5.3 `.github/browser-shards.json`: add `test_stage_box_is_at_least_65_percent_at_the_reference_viewport` to the shard that holds `test_browser_contextual_hud_stage`.

## 6. Specs and traceability

- [x] 6.1 Sync this change's deltas (`openspec archive` at the end, or `openspec-sync-specs` first), then run `uv run --locked python -m tools.spec_traceability list | grep the-place-card` and use that exact ID in 4.4 and 5.1.
- [x] 6.2 Run `uv run --locked python -m tools.spec_traceability check`; green.
...
- [x] 7.2 Run `agent-browser` against the running client at 1920x1080 and 1280x720:
...
- [x] 7.3 Run `openspec validate webclient-avg-place-card-top-bar --strict` and `git diff --check`; both clean.
