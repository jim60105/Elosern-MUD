...
- [x] 2.1 `components/CommandLine.vue`:
...
- [x] 2.3 `components/AppShell.vue` (design D2, D5, D7):
...
- [x] 3.1 `components/DesktopNavigation.vue` (design D6):
...
- [x] 4.1 `tests/command_line.test.js`:
...
- [x] 4.2 `tests/app.test.js`, `tests/preserved_contract.test.js`, `tests/bridge/app_shell_bridge.test.js`, `tests/bridge/bridge.test.js`, `tests/app_client_completion.test.js`:
...
- [x] 4.3 `tests/hud_frame.test.js`: `data-expanded` follows the prop, and the stage CSS hides the collapsed anchor. `tests/hud_drawer.test.js`: unchanged assertion on `var(--command-line-h)`; re-run it.
- [x] 4.4 `tests/desktop_navigation.test.js` (C4b's file):
...
- [x] 5.1 `web/tests/browser/browser_helpers.py`:
...
- [x] 5.2 `test_browser_shell_command_line.py`:
...
- [x] 5.3 `test_browser_contextual_hud_stage.py`:
...
- [x] 5.4 Re-point the tool-group selectors:
...
- [x] 5.5 Re-anchor the traceability annotations listed in design D9 to the three new IDs. Run `uv run --locked python -m tools.spec_traceability list` to confirm the new IDs after sync.
...
- [x] 5.6 `.github/browser-shards.json`: add the new tests to the shards that hold their modules. Run the modules on a local server and get green:
...
- [x] 6.1 Sync this change's deltas (`openspec archive` at the end, or `openspec-sync-specs` first).
...
- [x] 7.3 Run `openspec validate webclient-collapsible-command-line --strict` and `git diff --check`; both clean.
