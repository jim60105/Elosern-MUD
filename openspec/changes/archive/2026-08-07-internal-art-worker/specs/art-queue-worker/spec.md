...
- FROM: `### Requirement: The external worker contract validates every output against its input and confines paths to the store root`
- TO: `### Requirement: The internal worker contract generates every output through the sd-webui client and confines paths to the store root`
