...
- [x] 1.1 Extend the `npc_dialogue` per-kind semantic validator so `offer_quest` accepts exactly one
      ...
      reject and retry.
- [x] 1.2 Remove `offer_quest` from `_FORWARD_DECLARED_KINDS` in `world/rules/npc_intents.py`,
      leaving `("reveal_lore",)`.
- [x] 1.3 Add validator tests: valid `{"quest_key": ...}` passes; missing/empty/non-text/extra-field
      payloads reject and retry; exactly-64 passes and 65 rejects (boundary).

## 2. Applier

- [x] 2.1 Implement `_apply_offer_quest(npc, player, intent)` in `world/rules/npc_intents.py`:
...
- [x] 2.2 Implement the atomic application: `with transaction.atomic(): accept_quest(player,
...
- [x] 2.3 Wire `offer_quest` into `apply_npc_intent` dispatch; every failure path returns
      `IntentOutcome(applied=False, reason=...)` with the speech preserved.
- [x] 2.4 Applier tests (`EvenniaTest`): success assigns the quest and applies the guild affinity
...
- [x] 3.1 Update the `npc-dialogue` main spec by syncing the delta (offer_quest payload shape,
      executable offer_quest, not-yet-executable scenario now covers only `reveal_lore`) per the
      archive/sync workflow.
- [x] 3.2 Annotate new `dialogue-offer-quest` main requirements with `covers_requirement` (literal
      IDs from `python -m tools.spec_traceability list`) and keep `spec_traceability check` green.
- [x] 3.3 Run the affected Evennia test domains (world.rules, world.ai dialogue-layer tests) and
      confirm existing guild/quest/affinity suites stay green.
- [x] 3.4 Run `openspec validate --change dialogue-offer-quest --strict`.
- [x] 3.5 Run the affected test entry points with one shared `OPENSPEC_TEST_EVIDENCE` path, then
