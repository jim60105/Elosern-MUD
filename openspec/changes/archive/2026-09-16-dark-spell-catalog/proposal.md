...
- Sync `world/rules/rulebook/status_display.yaml` with the key census (the shipped import-time coverage check requires display metadata for exactly the live buff keys ∪ combat-modifier rule IDs and fails closed on drift): delete the `dark_atk_down` row, add rows for `dark_weaken` (衰弱), `dark_spread` (詛咒擴散), `dark_apotheosis` (深淵神格侵蝕), `dark_corrosion_deep` (深層侵蝕) and the `fear_locks_actions` rule id, per the table's existing label/severity conventions.
...
- `combat-modifier-table`: the one-table requirement is extended so the dark wave's psychological-stillness marker locks actions through the ordinary `buff_active`-origin `actions_per_turn: 0` row while `fear` stays an independent buffs key from any physical-stillness key (strict superset; all existing scenarios preserved verbatim).
