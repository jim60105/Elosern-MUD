...
- [x] 1.1 Create `tools/test_data_lint.py` with `check`, `list`, `report`, and `--json`
...
- [x] 1.2 Implement the shipped-token universe: import the locked catalogs under
...
- [x] 1.3 Implement Python AST scanning (statically resolvable string expressions:
...
- [x] 1.4 Implement ledger loading + violation rules per design D2 (`check` never seeds)

## 2. Seed the ledger and classify

- [x] 2.1 Create `tools/test_data_lint_seed.json` from design Appendix A (54 contract
  paths + reasons, including the 4 currently-clean contract files)
- [x] 2.2 Add the `Data-contract test:` tag line to the 54 Appendix-A files
- [x] 2.3 Generate `tools/test_data_freeze.json` (54 `contract` + 305 `debt` +
  frozen scan-derived `seedDebtPaths`) and verify `check` is green on the tree

## 3. Tests for the gate

- [x] 3.1 Create `tests/test_test_data_lint.py` (pure `unittest.TestCase` over temp
...
- [x] 3.2 Run `uv run --locked python -m unittest tests.test_test_data_lint -v` and
  `uv run --locked python -m tools.spec_traceability check`

## 4. Wiring and documentation

- [x] 4.1 Add the gate step to `.github/workflows/quality-gate.yml` beside the
  observability lint and traceability steps; verify the step command locally
- [x] 4.2 Add the authoring rule to `AGENTS.md` (Testing area) and
...
- [x] 4.3 Run `openspec validate add-test-data-independence-gate --strict` and
  `git diff --check`
