...
- Expand the already-tracked `.env.example` (added by 2a05f02) into the single reference for every environment variable this project and Evennia itself actually read, replacing the section that warned that `ART_SD_*` entries were inert — they will now be live.
