...
- [x] 1.1 Confirm `world/lore/anchors.py::ANCHOR_REGISTRY` and `world/lore/sync.py::{LoreRecord,
...
- [x] 1.2 Confirm `typeclasses/rooms.py` still contains only the stock `Room(ObjectParent,
      DefaultRoom): pass` and that no class named `GridRoom`, `AnchorRoom`, or `InstanceRoom` exists
      anywhere in the repository yet.
- [x] 1.3 Confirm `world/prototypes.py` is still the commented-out tutorial stub with no active
      `PROTOTYPE_LIST`/module-level prototype dict.
- [x] 1.4 Confirm, by import, that `evennia.contrib.grid.xyzgrid.xyzroom.{XYZRoom, XYZExit}`,
...
- [x] 1.5 Confirm `docker-entrypoint.sh` still runs only `evennia migrate --noinput` followed by
...
- [x] 2.1 Implement the frozen `AnchorPlacement` dataclass (`anchor_key: str`, `zcoord: str`,
      `entrance_xy: tuple[int, int]`) per design.md D-1.
- [x] 2.2 Implement `ANCHOR_PLACEMENT_REGISTRY: dict[str, AnchorPlacement]` with exactly one entry:
      `"capital_altoria": AnchorPlacement("capital_altoria", "capital_altoria", (2, 2))`.
- [x] 2.3 Test: `ANCHOR_PLACEMENT_REGISTRY` has exactly one entry, keyed `"capital_altoria"`.
- [x] 2.4 Test: every entry's `anchor_key` exists in `world.lore.anchors.ANCHOR_REGISTRY`.
- [x] 2.5 Test: `Anchor` (in `world/lore/anchors.py`) still has exactly its pre-change field set
...
- [x] 3.1 Import `ANCHOR_PLACEMENT_REGISTRY` in `world/lore/sync.py` and add it to `_ALL_REGISTRIES`
...
- [x] 3.2 Test: after `sync_all()` runs, a `LoreRecord` Script exists keyed
      `"lore:anchor_placements:capital_altoria"` with `db.fields` matching
      `_db_safe(asdict(ANCHOR_PLACEMENT_REGISTRY["capital_altoria"]))`.
- [x] 3.3 Test: calling `sync_all()` twice leaves exactly one such record (extend the existing
      idempotency test pattern in `world/lore/tests/test_sync.py` rather than duplicating its
      structure).
- [x] 3.4 Confirm (via `git diff`) that this task's edit to `world/lore/sync.py` is the only change
...
- [x] 4.1 Add `GridRoom(XYZRoom)` (import from `evennia.contrib.grid.xyzgrid.xyzroom`) with a
...
- [x] 4.2 Add `AnchorRoom(GridRoom)` with a persistent `anchor_key: str | None =
      AttributeProperty(default=None)` seam.
- [x] 4.3 Do **not** add any class or stub named `InstanceRoom` — confirm by source inspection that
      no such name exists anywhere in `typeclasses/rooms.py` after this task group.
- [x] 4.4 Test: creating a `GridRoom` via `GridRoom.create(key="test", xyz=(9, 9, "test_map"))`
      succeeds, `.xyz == (9, 9, "test_map")`, and `.scene_archetype is None` by default.
- [x] 4.5 Test: setting `room.scene_archetype = "tavern_interior"` succeeds with no registry lookup
...
- [x] 4.6 Test: creating an `AnchorRoom` via `AnchorRoom.create(key="test", xyz=(8, 8, "test_map"))`
      exposes both `.xyz` and `.scene_archetype` (inherited from `GridRoom`) and `.anchor_key`
      (defaulting to `None`).
- [x] 4.7 Test: setting `anchor_room.anchor_key = "does_not_exist"` on a freshly created `AnchorRoom`
...
- [x] 4.8 Test: `Room` (the stock class) is unchanged — instantiate it and confirm it has no
      `scene_archetype` or `anchor_key` attribute and is not an instance of `GridRoom`.

## 5. Prototypes (`world/prototypes.py`)

- [x] 5.1 Replace the inert tutorial-stub comment block with two active module-level prototype
...
- [x] 5.2 Test: `GRID_ROOM["typeclass"]` resolves to an importable `typeclasses.rooms.GridRoom`, and
...
- [x] 6.1 Create `world/maps/__init__.py` (empty package marker) and `world/maps/altoria_capital.py`.
- [x] 6.2 Author the verified `MAPSTR` from design.md D-6 exactly (the thirteen-node, twelve-link
      ...
      + 0 1 2 3 4
      ```
- [x] 6.3 Author the `PROTOTYPES` dict keyed by (X,Y) coordinate for all thirteen rooms per design.md
...
- [x] 6.4 Assemble `XYMAP_DATA = {"zcoord": "capital_altoria", "map": MAPSTR, "prototypes":
      PROTOTYPES}` and `XYMAP_DATA_LIST = [XYMAP_DATA]`.
- [x] 6.5 Test: `XYMap(dict(XYMAP_DATA), Z="capital_altoria", xyzgrid=None).parse()` succeeds and
      yields exactly the thirteen coordinates listed above, with no others.
- [x] 6.6 Test: `.calculate_path_matrix()` succeeds without error (confirms `scipy` resolves in the
      project's own environment for this specific map) and a shortest path exists between every pair
      of the thirteen coordinates.
- [x] 6.7 Test: the parsed link set has exactly twelve edges and is a tree (no cycle) — for example,
...
- [x] 6.8 Test: only `(2,2)`'s prototype entry has `"prototype_parent": "anchor_room"`; every other
      of the twelve has `"prototype_parent": "grid_room"`.

## 7. Settings and cmdset wiring

- [x] 7.1 Add `EXTRA_LAUNCHER_COMMANDS["xyzgrid"] = "evennia.contrib.grid.xyzgrid.launchcmd.
      xyzcommand"` to `server/conf/settings.py`, per design.md D-4/D-5.
- [x] 7.2 Append `"evennia.contrib.grid.xyzgrid.prototypes"` and `"world.prototypes"` to
      ...
      duplicate the entry).
- [x] 7.3 Add `evennia.contrib.grid.xyzgrid.commands.XYZGridCmdSet` to `commands/default_cmdsets.py::
      CharacterCmdSet.at_cmdset_creation()`.
- [x] 7.4 Test: `evennia xyzgrid help` (or an equivalent non-interactive invocation) runs without a
      settings-resolution error, confirming `EXTRA_LAUNCHER_COMMANDS` is wired correctly.
- [x] 7.5 Test: `CharacterCmdSet` contains the commands `XYZGridCmdSet` provides (for example,
...
- [x] 8.1 Implement `sync_grid()` per design.md D-3/D-5: obtain the singleton via `get_xyzgrid()`,
...
- [x] 8.2 Implement the idempotent Limbo-bridging `Exit` pair inside `sync_grid()`, per design.md
...
- [x] 8.3 Test: `sync_grid()` against an empty database containing a room keyed `"Limbo"` creates
      exactly thirteen `GridRoom`/`AnchorRoom` instances and exactly twenty-four intra-city
      `XYZExit` instances (one per travel direction: the twelve map links spawn two directed exits
      each — the xyzgrid contrib's `MapNode.spawn_links()` creates an exit for every direction a
      node links in), plus the two Limbo-bridging `Exit`s (twenty-six exits total).
- [x] 8.4 Test idempotency: calling `sync_grid()` a second time with unchanged map data leaves the
      ...
      return.
- [x] 8.5 Test in-place update: change one coordinate's `desc` in a test-local copy of the map data,
...
- [x] 8.6 Test the AnchorRoom/anchor_key/ANCHOR_PLACEMENT_REGISTRY cross-check named in design.md's
...
- [x] 8.7 Test the bridging exits specifically, against a database containing a room keyed `"Limbo"`:
...
- [x] 8.8 Test the North Gate dead end: the room at `(2, 4, "capital_altoria")` has exactly one exit
      (south, back into the city) and no other.
- [x] 8.9 Test the absent-Limbo degradation path: `sync_grid()` against a database with no object
      keyed `"Limbo"` still spawns all thirteen rooms and twenty-four intra-city exits, creates no
      bridging exit, and does not raise.

## 9. Server startup wiring (`server/conf/at_server_startstop.py`)

- [x] 9.1 Add a call to `world.maps.bootstrap.sync_grid()` inside `at_server_start()`, immediately
      after the existing `sync_all()` call, per design.md D-5.
- [x] 9.2 Test (source-order check): `at_server_start()`'s source lists the `sync_grid()` call after
      the `sync_all()` call.
- [x] 9.3 Test (integration): simulating a server start (calling `at_server_start()` directly against
...
- [x] 10.1 Add `"xyzgrid runtime"` (or similarly named) row(s) to `MATRIX_IMPORTS` covering
      `evennia.contrib.grid.xyzgrid.xyzgrid` (`XYZGrid`, `get_xyzgrid`) and
...
- [x] 10.2 Test: `ContribMatrixTests.test_matrix_imports_and_attributes` passes with the new rows
...
- [x] 11.1 Write one `EvenniaTest`-based integration test that: renames/creates the fixture's dbref
...
- [x] 11.2 Test: every one of the thirteen rooms is reachable from the South Gate via ordinary exit
...
- [x] 12.1 Run the full test suite added by this change (`world/lore/tests/`, `typeclasses/tests/`,
      `world/maps/tests/`, `tests/test_contrib_matrix.py`) and confirm every test passes.
- [x] 12.2 Confirm (via `git diff`) that every edit to an already-landed file (`world/lore/sync.py`,
...
- [x] 12.3 Confirm no file added or edited by this change contains a reference to `world/ai/` or an
...
- [x] 12.4 Confirm `settings.START_LOCATION` is unchanged (still `"#2"`) by source inspection, per
      design.md D-7's Non-Goal.
- [x] 12.5 Run `openspec validate map-anchor-grid --strict` and confirm it passes.
