...
- [x] 1.1 Add a deterministic absolute-day-ordinal helper in `world/rules/progression.py` derived
...
- [x] 1.2 Test (`unittest.TestCase`): the ordinal is strictly monotonic across a day roll, a season
      roll and a year roll, and is stable within one day.

## 2. The cadence claim

- [x] 2.1 Add the persisted claim store `entity.db.skill_practice_day` (`{skill_key: day_ordinal}`)
      with a read/write pair in `world/rules/progression.py`; tolerate a missing attribute.
- [x] 2.2 Gate `grant_skill_practice_xp()` on the claim for `SkillCategory.DIVINE_MYSTERY` ACTIVE
      skills, evaluated **before** `_claim_practice`, returning `False` when the day is taken.
- [x] 2.3 Record the day only on the path that actually awards, so a refused award never consumes it.

## 3. Rollback face

- [x] 3.1 Register `skill_practice_day` beside `skill_proficiency` in all three of its real homes:
...
- [x] 3.2 Test: after a rolled-back resolution, both `skill_proficiency` and `skill_practice_day` are
      byte-equal to their pre-action values and a same-day retry accrues.
- [x] 3.3 Test the same rollback on the COMBAT round path, not only the out-of-combat cast path, since
      the two reach different snapshot mechanisms.

## 4. Behavior contract tests

- [x] 4.1 New test module `world/rules/tests/test_divine_digestion_cadence.py` built on synthetic
...
- [x] 4.2 Annotate the new tests with `covers_requirement` using IDs from
      `uv run --locked python -m tools.spec_traceability list`; do not hand-write IDs.
- [x] 4.3 Register the new module in exactly one shard of `.github/evennia-shards.json`.
...
- [x] 5.2 Run the focused existing lineage label
...
- [x] 5.4 `openspec validate divine-mystery-digestion-cadence --strict`.
