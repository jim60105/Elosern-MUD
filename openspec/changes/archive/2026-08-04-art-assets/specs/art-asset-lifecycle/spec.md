...
The enqueue authority and lifecycle: `world/art/service.py` is the sole writer of asset/queue records,
reached only through deterministic seams — idempotent startup synchronization of scene and generic
monster subjects, startup recovery of explicit unique portrait policies, post-commit scheduling for
player creation and validated character import, post-commit named-NPC spawn scheduling through the
SceneBuilder path, and room-entry `ensure_scene_asset()`. Queue failure never rolls back gameplay.
