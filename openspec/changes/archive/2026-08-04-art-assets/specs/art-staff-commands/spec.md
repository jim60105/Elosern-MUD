...
The `@art` staff command family — `status`, `run`, `retry`, `requeue` — restricted to staff, with
status output that never leaks persona text or filesystem paths, and no access for ordinary players.
