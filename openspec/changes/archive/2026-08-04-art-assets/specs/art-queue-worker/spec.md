...
The asset-record contract, the idempotent subject-keyed queue, the shared scene+portrait
serialization lock, the external worker contract with output-to-input validation and store-path
confinement, the settings-configurable disableable scheduler, and same-origin media URL serving under
`server/.art/`.
