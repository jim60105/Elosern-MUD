...
existing onboarding rules define, while an unknown keyword SHALL NOT write. The
...
guild service, and `talk <guild-staff> 回報 <quest_id>` SHALL turn in exactly
that quest through `turn_in_quest` with the same atomic exactly-once
settlement and rejection semantics as `guild turnin`. Every other `guild_staff`
keyword SHALL keep the no-state guarantee.
