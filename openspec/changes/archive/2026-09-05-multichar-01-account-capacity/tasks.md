...
- [x] 1.1 `server/conf/settings.py`: add
...
- [x] 1.2 `.env.example`: add the commented `#ELOSERN_MAX_CHARACTERS=5` entry in the
  `ELOSERN_*` block, matching the file's existing comment style.
- [x] 1.3 `docs/development/settings-and-environment.md`: add the `ELOSERN_MAX_CHARACTERS` row
  (variable / reader / type / default / validation rule), matching the `ELOSERN_VUE_CLIENT` row's
  shape.
- [x] 1.4 `server/conf/test_settings.py`: confirm the env-scrubbing list is derived rather than
  hand-written; add `ELOSERN_MAX_CHARACTERS` explicitly if it is a literal list.

## 2. Login screens keyed to the session's puppet

- [x] 2.1 `typeclasses/accounts.py`: change `at_post_login` to resolve the session's own puppet
...
- [x] 2.2 Keep `render_pending_character_login` as the single coordinator (world introduction then
  creation start screen) and keep its `session=` targeting.

## 3. Localized OOC surface and the account command locks

- [x] 3.1 `commands/localized/account.py`: delete the `_AUTO_PUPPET_ON_LOGIN and
...
- [x] 3.2 `commands/default_cmdsets.py`: in `AccountCmdSet.at_cmdset_creation`, remove the
...
- [x] 3.3 `docs/game/command-reference.md`: update the `charcreate` / `chardelete` rows
...
- [x] 4.1 `server/conf/tests/test_env_overrides.py`: default is 5; `=1` and `=10` accepted; `=0`,
...
- [x] 4.2 New account-capacity test beside the account typeclass tests (`EvenniaTest`): create
...
- [x] 4.3 Login-screen test: an account owning one activated and one pending character, logging in
...
- [x] 4.4 `commands/tests/test_localized.py`: `離開角色` and OOC `看` render the playable-character
  list at both `MAX_NR_CHARACTERS=1` and a raised cap, and never print the retired
  「使用 進入世界 回到遊戲」 hint.
- [x] 4.5 `tests/test_command_docs.py`: extend the Developer-lock assertion to cover `charcreate`
  and `chardelete`, and keep the reference/overview drift checks green.
- [x] 4.6 Add the `covers_requirement` annotations for the four delta requirements
...
- [x] 5.2 `uv run --locked python -m tools.observability_lint check` and
  `uv run --locked python -m compileall -q world typeclasses commands server`.
- [x] 5.3 Repo-wide grep for `MAX_NR_CHARACTERS` to confirm no other module snapshots it at import
  time.
- [x] 5.4 Live container check: register a fresh account, complete creation, then confirm a
