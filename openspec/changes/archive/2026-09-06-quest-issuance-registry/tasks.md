...
- [x] 1.1 New module `world/rules/quest_issuance.py` with `IssuerKeyError` and a shared
...
- [x] 1.2 Add the two constructor helpers the later changes call —
...
- [x] 2.1 `Settlement` StrEnum with exactly `COUNTER = "counter"` and `AUTO = "auto"`.
- [x] 2.2 Frozen `QuestIssuance` dataclass carrying exactly `definition_key`, `issuer_key`,
...
- [x] 2.3 Merit rule: an `npc:`-namespaced issuance with non-zero `reward.merit` raises a named
  error at construction, before any registry write.

## 3. Registry and read seam

- [x] 3.1 `QUEST_ISSUANCE_REGISTRY: dict[tuple[str, str], QuestIssuance]` plus
...
- [x] 3.2 `resolve_issuance(definition_key, issuer_key)`: parse the key (letting the parser's error
...
- [x] 3.3 Emit the facade boundary event for private-commission registration per the observability
  catalog, with `quest` and `issuer` context keys.

## 4. Non-regression of the guild surface

- [x] 4.1 Confirm by inspection and test that `GUILD_OFFER_REGISTRY`, `GuildQuestOffer`,
  `register_guild_offer`, `get_guild_offer`, and `list_guild_offers` are untouched — same storage,
  same value type, same signatures.
- [x] 4.2 Test that a `guild:` key never writes into `QUEST_ISSUANCE_REGISTRY` and that resolving a
  `guild:` key reads only the guild offer registry.

## 5. Tests and wiring

- [x] 5.1 New focused test module `world/rules/tests/test_quest_issuance.py` covering: all three
...
- [x] 5.2 Annotate each test with `covers_requirement` against the new `quest-issuance` requirement
  IDs.
- [x] 5.3 Update `.github/evennia-shards.json` if the new test module needs an Evennia shard entry;
  a pure-unittest module needs none.
- [x] 5.4 Run the observability lint plus the focused tests in the same batch.
