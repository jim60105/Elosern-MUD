...
act catalog added 65 active skills on top of the 91 base active skills (plus the pre-existing
`divine_sexual_arts` active skill) — a theoretical maximum of 157 owned active skills — so any
character who unlocks a meaningful share of the act catalog
...
- **BREAKING (protocol bound)**: `MAX_SKILLS` rises from `32` to `192` — above the current
  theoretical maximum of 157 active skills with headroom for catalog growth, and a multiple of 16
