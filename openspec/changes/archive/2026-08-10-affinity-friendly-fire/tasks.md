...
- [x] 1.1 Add `friendly_fire_penalty_per_hit: 1` to `world/rules/rulebook/affinity.yaml` with a
      comment documenting the friendly-fire contract
- [x] 1.2 Extend `world/rules/affinity_config.py` to load and validate
      `friendly_fire_penalty_per_hit` (positive integer, bounded), failing closed on deviation
- [x] 1.3 Add `friendly_fire` to the closed `AffinitySource` set in `world/rules/affinity.py` and
      confirm `apply_affinity_change` handles it as a negative delta with no budget interaction

## 2. Combat-session damage scan

- [x] 2.1 In `world/rules/combat_session.py`, after a resolved player action round, scan the
...
- [x] 2.2 For each qualifying hit (target is an NPC in the snapshotted `player.db.party` set and
...
- [x] 2.3 Run the scan, all penalty writes, and any resulting auto-leave inside the round's
      transaction boundary; deliver the auto-leave notification after commit, never mid-transaction
- [x] 2.4 Ensure non-player-action damage (companion-vs-companion, enemy behavior, buff ticks)
      never enters the scan, and non-companion targets (including recordless ones) trigger no write

## 3. Tests

- [x] 3.1 Pure/unit tests: rulebook loading rejects missing/non-positive `friendly_fire_penalty_per_hit`;
      `friendly_fire` source accepted by the writer with no budget interaction
- [x] 3.2 `EvenniaTest` combat tests: AREA skill hitting two companions applies two -1 penalties;
...
- [x] 3.3 Auto-leave integration tests: penalty dropping a companion below 70 ends the party with
...
- [x] 3.4 Same-round snapshot tests: the first hit of an action triggers auto-leave and a later hit
...
- [x] 3.5 Regression: existing affinity, party, and combat-session suites stay green
...
- [x] 4.3 Run the focused test packages (world rules tests) and confirm green
