...
- [x] 1.1 Confirm `world/rules/` holds change 8's `action.py` (`ActionRequest`, `ActionResolver`),
...
- [x] 1.2 Confirm the exact import paths for `world.rules.combat.{Battlefield,
...
- [x] 1.3 Confirm `typeclasses/monsters.py`'s actual `Monster.threat_tier`/`Monster.behaviour_tree`
...
- [x] 2.1 Author `tier_default_archetype` per design.md D-2: one entry per `MONSTER_TIER_REGISTRY` key
...
- [x] 2.2 Author the `archetypes` table per design.md D-2: `instinctive`, `pack_hunter`, `brute`,
      `tactical_caster`, `apex_predator`, each declaring `target_strategy`, `skill_choice`, and
      `prefer_area_when_multiple_enemies`.
- [x] 2.3 Implement a small loader in `world/rules/monster_behaviour.py`
...
- [x] 2.4 Test: every `tier_default_archetype` value exists as a key in `archetypes`; every
...
- [x] 3.1 Implement `BehaviourProfile` (frozen dataclass: `target_strategy: str`, `skill_choice: str`,
      `prefer_area_when_multiple_enemies: bool`) per design.md D-2.
- [x] 3.2 Implement `resolve_behaviour_profile(monster) -> BehaviourProfile` per design.md D-1: reads
...
- [x] 3.3 Test: an unset `behaviour_tree` resolves to the correct tier default for each of the four
...
- [x] 4.1 Implement `_owned_damage_skills(entity) -> list[SkillDef]`: resolves
      `entity.skills.owned_keys()` through `SKILL_REGISTRY`, keeping `SkillKind.ACTIVE` entries whose
      `effects` contain at least one `damage:`-prefixed ID and whose current resource costs are
      affordable, preserving the entity's own owned-key order.
- [x] 4.2 Implement `_damage_school(skill: SkillDef) -> str`: parses the skill's
      `damage:<element>:<school>`-shaped effect ID exactly as change 9's `_handle_damage` does, returning
      `"physical"` or `"magic"`.
- [x] 4.3 Test: an entity owning a mix of `PASSIVE` and `ACTIVE` skills, and skills with and without a
      `damage:`-prefixed effect, yields exactly the `ACTIVE`+`damage:`-effect subset, in owned order; an
      unaffordable damage skill is excluded while an affordable later skill remains available; an entity
      owning zero such skills yields an empty list.

## 5. Living-enemy discovery (`world/rules/monster_behaviour.py`)

- [x] 5.1 Implement `_living_enemies(battlefield, actor) -> list[LivingEntity]`: finds `actor`'s own
      ...
      pattern.
- [x] 5.2 Test: a battlefield with one dead and one fled enemy on the opposing team yields only the
...
- [x] 6.1 Implement `_choose_target(entity, enemies, strategy) -> LivingEntity` per design.md D-3:
...
- [x] 6.2 Implement `_choose_skill(entity, candidates, strategy, target) -> SkillDef` per design.md D-3:
...
- [x] 6.3 Test: `_choose_target()`'s two strategies each select the documented candidate on a roster with
...
- [x] 7.1 Implement `monster_behaviour_policy(entity, battlefield) -> ActionRequest | None` per
...
- [x] 7.2 Confirm (grep-based check, mirroring changes 9/10's own discipline) that
...
- [x] 7.3 Confirm (grep-based check) that `world/rules/monster_behaviour.py` imports nothing from
      `world/ai/` and contains no reference to an LLM-client module or network call.

## 8. Tests

- [x] 8.1 `world/rules/tests/test_monster_behaviour_profile.py` — per the `monster-behaviour-profile`
      capability: task 2.4's and task 3.3's assertions.
- [x] 8.2 `world/rules/tests/test_monster_behaviour_selection.py` — per the `monster-action-policy`
      capability's target/skill-selection requirements: task 4.3's, 5.2's, and 6.3's assertions.
- [x] 8.3 `world/rules/tests/test_monster_behaviour_policy.py` — per the `monster-action-policy`
...
- [x] 8.4 `world/rules/tests/test_monster_behaviour_determinism.py` — per hard requirement 2 and the
      ...
      tasks 7.2/7.3.
- [x] 8.5 `world/rules/tests/test_monster_behaviour_golden.py` — per the golden fixed-seed requirement:
      construct one battlefield with a distinct low-hp/low-power enemy and a distinct high-hp/high-power
      enemy; run all four tier-default `Monster` profiles against that identical roster under the same
      fixed seed; assert the `low`-tier monster targets the low-hp enemy and the `calamity`-tier monster
      targets the high-power enemy; assert the full decision sequence is byte-identical across two runs
      under the same seed.
- [x] 8.6 `world/rules/tests/test_monster_behaviour_integration.py` — per the drop-in `action_provider`
...
- [x] 9.1 Run the full `world/rules/tests/` suite added by this change and confirm every test passes.
- [x] 9.2 Confirm this change modifies no file authored by any earlier change — `git diff --stat` against
...
- [x] 9.3 Confirm change 9's own test suites (`test_golden_combat.py`, `test_initiative_and_turn_loop.py`)
...
- [x] 9.4 Confirm change 8's own no-combat-branching tripwire suite (`test_no_combat_branching.py`) still
      passes unmodified — this change never touches `action.py`, `targeting.py`, or `event_log.py`.
- [x] 9.5 Run `openspec validate monster-behaviour --strict` and confirm it passes.
