...
- [x] 1.1 Confirm `world/rules/sexual_resist.py`'s `resist_verdict(actor, resister, *, rng=roll_d100) ->
...
- [x] 1.2 Confirm `world/rules/combat_session.py`'s `_scan_sexual_coercion` still reads
...
- [x] 1.3 Confirm `world/skills/sexual_acts/_builder.py`'s `SexualActDef.resistible: bool` field and
  `world/skills/sexual_acts/__init__.py`'s `SEXUAL_ACT_REGISTRY` (keyed identically to `SKILL_REGISTRY`)
  are unchanged.
- [x] 1.4 Confirm `world/rules/action.py`'s `_step3_targeting`, `_handle_pleasure_effect`,
...
- [x] 1.5 Confirm `world/rules/combat.py`'s existing `from world.rules.dice import roll_d100` /
...
- [x] 2.1 Add `from world.rules.dice import roll_d100` to `action.py`'s top-level imports, and import
  `resist_verdict` lazily inside `_step4b_sexual_resist_gate` (design.md D-6, amended at
  implementation time: a module-level `from world.rules.sexual_resist import resist_verdict` creates
  an import cycle — `action → sexual_resist → combat → action` — that breaks whenever `action` is the
  entry point, so the import moves into the gate function's body, matching the existing
  `_handle_sexual_event` lazy-import convention).
- [x] 2.2 Add `_resist_pending_effect(target, verdict) -> PendingEffect` per design.md D-3: description
  ...
  `apply=lambda: None`.
- [x] 2.3 Add `_step4b_sexual_resist_gate(request, skill, targets) -> tuple[list[Any], list[PendingEffect]]`
...
- [x] 2.4 In `ActionResolver.resolve()`, call `targets, resist_pending = _step4b_sexual_resist_gate(request,
...
- [x] 2.5 Add `"sexual_resist"` to `_ENTRY_TEMPLATES` with narrative Traditional Chinese text handling
...
- [x] 2.6 Add an `elif kind == "sexual_resist":` branch to `_entries_from_effect` parsing the three
...
- [x] 3.1 Run `uv run --locked python -m tools.spec_traceability list` and note the canonical requirement
  IDs for `sexual-resist-cast-wiring::*` from this change's delta spec.
- [x] 3.2 Add `world/rules/tests/test_sexual_resist_cast_wiring.py` (new `EvenniaTest`-based module)
  ...
  - The actor's own `hostile_act_count` and pleasure share are credited in both branches (design.md
    D-5), with the actor's own gain computed over the post-resist participant set (design.md D-7's
    crowd-multiplier note — a fully-resisted single-target cast has count 1, not 2).
  ...
  - The AREA synthetic act also declares one `sexual_event` (`masturbation_climax`) so the
    event-withholding half of R3 is asserted directly: the resisted target's `experience_types` gains
    nothing while the complying target gains 自慰 (added after rubber-duck review — the original test
    only withheld pleasure/counters).
  - The fully-resisted resource test also asserts `time_cost_seconds` is a non-negative int and that the
    result's `event_log.entries` contains a `skill_practice` entry (added after rubber-duck review — the
    original test asserted resource deduction and counters only).
...
- [x] 3.3 Apply `covers_requirement("sexual-resist-cast-wiring::<id>")` (using the IDs from 3.1) to each
  test function whose assertions establish that requirement.
- [x] 3.4 Run `uv run --locked python -m tools.spec_traceability check` and confirm every
  `sexual-resist-cast-wiring` requirement is covered.

## 4. Regression verification against already-shipped resist-adjacent code

- [x] 4.1 Update `world/skills/sexual_acts/tests/test_seed_acts.py::
  ...
  the moment task 2 lands. The mocked value is `1`: both fixtures are floor humans with equal contest
  scores, so roll 1 is a guaranteed comply. Also add `register_catalog()` to
  `SeedActCastingTests.setUp` — casting a resistible act reaches `resist_verdict()`, whose lazy
  `get_resist_config()` → `get_config()` chain requires the quest catalog. Done at implementation time.
- [x] 4.1b Update `world/rules/tests/test_sexual_act_effects.py`'s `_ActCastTestCase` fixture the same
  way (discovered at implementation time, design.md D-3a second half): every test-local act there is
  built `resistible=True`, so the module's casts both need `register_catalog()` in `setUp` and a
  compliant-roll mock in `_cast`.
- [x] 4.2 Re-run `world/skills/sexual_acts/tests/test_seed_acts.py` in full (including the fix from 4.1)
  ...
  lucky roll this proposal's static analysis missed. Re-run `world/rules/tests/test_sexual_act_effects.py`
  in full too (fix from 4.1b).
- [x] 4.3 Re-run `world/rules/tests/test_combat_session_sexual_coercion.py` (B6b's suite) unmodified and
  confirm it still passes — this proposal must not require any change to that file.
- [x] 4.4 Re-run `world/rules/tests/test_sexual_resist.py` (`B6a`'s own `resist_verdict()` tests) unmodified
...
- [ ] 5.1 Run `uv run --locked evennia test --settings test_settings.py --keepdb world.rules.tests
  world.skills.tests` and confirm the whole combined suite passes. (The pre-fix combined run failed
  exactly 5 tests, all in `test_sexual_act_effects.py`; those are fixed by 4.1b and the affected-module
  subset — 215 tests across every module touching the resist wiring — passes. The full combined run is
  the final handoff gate.)
- [x] 5.2 Run `uv run --locked python -m compileall -q world`.
- [x] 5.3 Run `openspec validate sexual-resist-cast-wiring --strict` and resolve any reported issue.
- [x] 5.4 Confirm no file outside `world/rules/action.py`, the new
  `world/rules/tests/test_sexual_resist_cast_wiring.py`, and the two test-fixture edits
  (`world/skills/sexual_acts/tests/test_seed_acts.py` task 4.1 and
  `world/rules/tests/test_sexual_act_effects.py` task 4.1b, both required by design.md D-3a) was
  touched, matching the proposal's Impact list exactly — in particular, confirm
  `world/rules/sexual_resist.py`, `world/rules/combat_session.py`, `world/skills/sexual_acts/_builder.py`,
  and every `world/skills/sexual_acts/*.py` catalog module (not its `tests/` subpackage) have zero diff
  from this change.
