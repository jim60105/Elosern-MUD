...
- [x] 1.1 Restyle `NarrativeFeed.vue` caption to the draft's `.feed-inner`: panel fill +
...
- [x] 1.2 Add the head row: committed-mode label (`敘述` / `戰鬥日誌`) on the left with the unread
...
- [x] 2.1 Mount the draft's semantic classes from committed line kinds at the existing render
...
- [x] 2.2 Vitest: sys line carries the class and marker is decorative; emphasis gold; plain
  lines gain no semantic class.

## 3. Condition island empty state

- [x] 3.1 `ConditionChips.vue`: render no island at all when the committed condition list is
  empty (delete the `無條件` statement path); update the contextual-hud empty-state tests.

## 4. Suggestion surface dedupe

- [x] 4.1 Remove the stream choice-point: delete `ChoicePointBlock.vue`, `lib/choicepoint.js`,
...
- [x] 4.2 Ensure the dock 建議 pane is the sole suggestion surface and carries the full status
...
- [x] 4.3 Remove the component from `component-manifest.json` and delete
...
- [x] 4.4 Migrate `tests/action/choice_point_block.test.js` assertions onto the dock pane
...
- [x] 4.5 Delete `web/tests/browser/test_browser_choicepoints.py` and remove the
...
- [x] 5.1 Focused Vitest suites (feed, chips, suggestions pane, manifest/deferred gates).
- [x] 5.2 `uv run --locked python -m tools.spec_traceability check` — the gap for removed IDs
  appears only after archive-sync, so run `check` against the active deltas' expectations.
- [x] 5.3 Live browser check 1600x900: head labels per mode, sys `◈` lines, empty conditions →
