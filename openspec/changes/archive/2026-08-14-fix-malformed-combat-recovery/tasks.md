...
- [x] 1.1 In `read_session` (`world/rules/combat_session.py:249`), wrap `from_storage(dict(raw))` so
...
- [x] 1.2 Unit tests: payloads `{"not": "a valid record"}`, an integer, and a string each make
...
- [x] 1.3 Test the still-persisted rejection contract: with a malformed payload persisted, `engage`
...
- [x] 2.1 Move strict parsing inside the recovery boundary: catch `CombatSessionError` from
...
- [x] 2.2 Keep the `settled_tick` fast path after a successful parse (log and clear leftover state,
      never settle again)
- [x] 2.3 Split the reconstruction handler: `CombatSessionError` from `reconstruct_battlefield` keeps
...
- [x] 2.4 Update the `restore_active_session` docstring to distinguish unparseable records (cleared,
      never settled) from well-formed records with invalid participant references (diagnostic
      termination)

## 3. Regression tests

- [x] 3.1 Test with exactly `{"not": "a valid record"}` persisted: `restore_active_session` clears
      ...
      the clock
- [x] 3.2 Test the startup entry: `restore_persisted_sessions()` clears the same payload and a
      subsequent `engage` against a co-located living monster succeeds with a fresh hostile session
- [x] 3.3 Guard the invalid-references contract: a well-formed record whose enemy dbref is deleted
...
- [x] 3.4 Guard the unrelated-failure contract: a valid terminal session whose settlement raises
...
- [x] 3.5 Annotate the new startup-recovery tests with
...
- [x] 3.6 Before archive/sync, obtain canonical IDs with `uv run --locked python -m
...
- [x] 4.1 Run the focused Evennia package tests: `world.rules.tests.test_combat_session`,
...
- [x] 4.2 Run `uv run --locked python -m tools.spec_traceability check`,
