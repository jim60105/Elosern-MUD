...
- [x] 1.1 Create `world/ai/tests/_director_helpers.py` with the module-level
...
- [x] 1.2 Create `test_scenario_director_proposals.py`
...
- [x] 1.3 Update `ScenarioDirectorOfflineTestRuleTests.test_no_live_client_constructor_or_socket_in_scenario_director_tests`
...
- [x] 1.4 Remove the moved classes and helpers from `test_scenario_director.py`;
      delete the file if nothing remains

## 2. Split `world/ai/tests/test_npc_dialogue.py`

- [x] 2.1 Create `world/ai/tests/_dialogue_helpers.py` with the module-level
...
- [x] 2.2 Create `test_npc_dialogue_prompts.py` (NPCDialoguePromptTests,
...
- [x] 2.3 Remove the moved classes and helpers from `test_npc_dialogue.py`;
      delete the file if nothing remains

## 3. Contract tests and verification

- [x] 3.1 Create `tests/test_ai_test_layout_contract.py`: AST-based (no
...
- [x] 3.5 Full suite: `MUD_TEST_SETTINGS=1 uv run --locked evennia test
      ...
      (measured 4,263 both before and after the split at the current HEAD)
...
- [x] 4.1 `openspec validate split-ai-test-files --strict`
