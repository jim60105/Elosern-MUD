...
- Author every node's effect composition over the wave's vocabulary: `damage:water:magic` coefficients, `buff_apply:` the 潮退 tier rows / `water_film` / bind / `mp_regen_lock` / `mana_reflux`, `gauge_transfer:` drains/restores with `GaugeTransferPolicy` shares and per-stack bonuses, `audience_condition` for 溺潮's max-zero damage rider, and 深海神格's take-and-give composite (enemy-audience drain-all + ally-audience restore, both from existing primitives).
