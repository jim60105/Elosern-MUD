...
- [x] 1.2 Implement bare pleasure_peak through canonical gain 100 then 0 with declared actor binding; verify starting phase boundaries, no artificial extension and an already locked caster rejection.
- [x] 1.3 Implement phase reaction loading/validation and one canonical transition hook using the existing condition evaluator and qualified ownership; verify invalid edges, rule/direct-gain sources and no retroactive mid-cycle grant.
- [x] 1.4 Implement persisted cycle marker lifetime and configured state maximum selection; verify afterglow actions, neutral expiry, qualification loss and unchanged fixed coefficients/gates.
- [x] 1.5 Expand all existing transition-capable caller snapshots, not only the new prefix; verify failure after action/clock phase entry restores marker, buffs, counters and underlying traits.
...
- [x] 2.5 Run tools.test_data_lint check, tools.spec_traceability check and openspec validate light-climax-empowerment --strict. For main-spec synchronization in the separately authorized workflow, obtain IDs using tools.spec_traceability list and annotate only tests that establish those requirements; verify no unknown IDs or uncovered main requirements are hidden. Keep all tasks unchecked until their described proof exists.

### Focused invocation

```sh
uv run --locked evennia test --settings test_settings.py --keepdb world.rules.tests.test_phase_spell_reactions world.rules.tests.test_climax_phase_cycle
uv run --locked python -m tools.observability_lint check
uv run --locked python -m tools.test_data_lint check
uv run --locked python -m tools.spec_traceability check
openspec validate light-climax-empowerment --strict
```
