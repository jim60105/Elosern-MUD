...
- [x] 1.1 Rewrite `.github/browser-shards.json` as 11 shards with
      ...
      - every process list stays ≤ 240 s estimated time using the measured
        per-test weights (combat ~50 s, creation/layout ~38 s, services/pointer ~47 s, exploration/
        reconnect ~40 s, art/harness ~36 s, shell
        family ~6.4 s),
      - every test method appears in exactly one list (the Step 3 contract
        test enforces this)
- [x] 1.2 Schema-check the manifest: 11 shards, indices 1–11 unique and
      sorted, each shard has non-empty `files_a` and `files_b`

## 2. Workflow: two-workspace browser job

- [x] 2.1 Replace the `browser` job steps: check out twice
      ...
      both workspaces
- [x] 2.2 The "Run browser shard ${{ matrix.index }}" step runs two
      ...
      `continue-on-error`
- [x] 2.3 Upload step: copy the per-process coverage files to the job root
      (`cp w-a/coverage-browser-shard-<n>-p1* .` and the `-p2` equivalent) so
      the unchanged gate globs keep matching, then upload
      `coverage-browser-shard-<n>-p1*`, `coverage-browser-shard-<n>-p2*`,
      and `evidence.browser-shard-<n>.jsonl` as
      `browser-shard-<n>-artifacts` with `if-no-files-found: error`

## 3. Contract tests

- [x] 3.1 Replace `test_browser_shard_manifest_owns_every_browser_test_file_exactly_once`
...
- [x] 3.2 `tests/test_browser_verification_contract.py`: update the browser
...
- [x] 3.3 `tests/test_evennia_test_optimization_contract.py`
...
- [x] 4.1 Update `AGENTS.md`, `docs/development/evennia-testing-guide.md`,
...
- [x] 5.1 Run `uv run --locked -m unittest discover -s tests -t .`,
...
- [x] 6.1 Sync the delta spec into
      ...
      `git diff --check` is clean (delta synced during implementation;
      archived 2026-08-16 with `openspec validate --all --strict` green)
