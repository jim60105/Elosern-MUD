...
- [x] 1.1 Add `world/quests/characterization.py` with the shared bound helper validating
...
- [x] 1.2 Add pure `unittest.TestCase` coverage: valid adult values; unpaired ages; `None`-valued
...
- [x] 2.1 Add the frozen `BlueprintPortrait` value object (exactly one `stable_key` field) and
...
- [x] 2.2 Wire the scenario director's blueprint validation to the shared helper (resolving the
      upper bound via `NPC_TIER_REGISTRY[tier].race_key` → `RACE_REGISTRY[race].lifespan`), with
      no inline duplicate of the rules
- [x] 2.3 Extend the output jsonschema, `to_payload()`, and `from_payload()` to carry all four
...
- [x] 2.4 Add scenario-director validation tests: valid named occupant (human 68 / elf 300 via
...
- [x] 3.1 Add `NPCTier("elven_civilian", ...)` (elf / `elf_common`) to `world/lore/npc_tiers.py`,
      consistent with the existing registry test (referenced static tier belongs to the referenced
      race)
- [x] 3.2 Add registry tests: the elven tier validates and its race resolves to the elf lifespan
      band

## 4. Compile boundary carry-through

- [x] 4.1 Extend `StageSpawnRequirement` in `world/quests/compile.py` to carry the optional
      characterization fields in deterministic order (frozen value)
- [x] 4.2 Wire `compile_quest_blueprint`'s `npc_req` parsing to the shared helper so malformed
      characterization rejects before registration or materialization; include the fields in the
      canonical digest serialization
- [x] 4.3 Add compile tests: valid fields preserved; unpaired/underage/out-of-band/malformed
...
- [x] 5.1 Update `world/ai/director_templates.py` to support the optional fields on template
      `npc_reqs` entries, with a valid named example
- [x] 5.2 Add a template registration test: malformed (underage) template rejected at
      registration; valid named template registers
- [x] 5.3 Apply the `scene-builder` anti-hallucination narrowing (mechanical numbers only) and
...
- [x] 5.4 Add a repository guard test asserting both validation layers (scenario director and
      compile) call the shared helper and no inline duplicate implementation exists

## 6. Verification

- [x] 6.1 Run the affected Evennia test domains
...
- [x] 6.2 Run `uv run --locked python -m tools.spec_traceability check` and confirm the new main
      requirements carry `covers_requirement` annotations
- [x] 6.3 Run `openspec validate blueprint-portrait-policy --strict` and confirm the change is
      apply-ready
