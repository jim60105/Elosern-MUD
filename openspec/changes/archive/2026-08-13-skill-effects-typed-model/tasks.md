...
- [x] 1.1 Create `world/skills/effects.py` with one frozen dataclass per prefix listed in the
...
- [x] 1.2 Implement `parse_effect(effect_id: str)` as a single dispatch function over the prefix
      (`effect_id.partition(":")[0]`), raising `ValueError` for anything unrecognized
- [x] 1.3 Write unit tests for every recognized prefix (happy path) and at least one unknown-prefix
      negative test

## 2. Wire SkillDef to the typed model

- [x] 2.1 Add `parsed_effects: tuple` field to `SkillDef`, populated in `__post_init__` via
      `tuple(parse_effect(e) for e in effects)`
- [x] 2.2 Confirm `SKILL_REGISTRY`'s module-level construction still imports cleanly (every existing
      effect string must parse under the new dispatch — if any doesn't, fix the string, not the parser)
- [x] 2.3 Remove the stale "stat_multiply is the only effect-ID convention interpreted by this package"
      claim from `world/skills/registry.py`'s module docstring; point to `effects.py` instead

## 3. Migrate the one existing consumer

- [x] 3.1 Update `world/skills/handler.py`'s `effective_value`/`_matching_multiplier` to read
      `StatMultiplyEffect` instances from `parsed_effects` instead of re-parsing the raw string with
      `_parse_stat_multiply`
- [x] 3.2 Confirm the full existing `skill-handler` spec scenario suite still passes unchanged (no
      behavior change intended, only representation)

## 4. body_enhancement reclassification

- [x] 4.1 Change `kind=SkillKind.ACTIVE` to `kind=SkillKind.PASSIVE` for `body_enhancement`,
      `body_enhancement_extreme`, `body_enhancement_basic` in `world/skills/registry.py`
- [x] 4.2 Grep the test suite, `game-command-docs`, and webclient skill-menu/OOB surfaces for these
...
- [x] 4a.1 Change `reincarnation_boon_yuna`'s `effects` from the malformed three-segment
...
- [x] 5.1 Add `world/skills/cost_tiers.py` with the five-tier MP range table from design doc §4.3, as a
      `dict`/`NamedTuple` keyed by tier name, for later spell-catalog proposals to import

## 6. Verify

- [x] 6.1 Run the full existing test suite; only `skill-handler`-adjacent and `skill-registry`-adjacent
