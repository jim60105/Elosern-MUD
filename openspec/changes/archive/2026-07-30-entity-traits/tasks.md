...
- [x] 1.1 Confirm `typeclasses/` and `world/rules/` exist as empty stub packages from change 1; add
      `typeclasses/tests/__init__.py` and `world/rules/tests/__init__.py`.
- [x] 1.2 Create `typeclasses/entities.py`, `typeclasses/characters.py`, `typeclasses/npcs.py`,
...
- [x] 2.1 Define `LivingEntity(ComponentHolderMixin, ObjectParent, DefaultCharacter)` per design.md
      D-1. Verify,
...
- [x] 2.2 Mount `entity.traits` as a `TraitHandler` (`evennia.contrib.rpg.traits`) in
      `at_object_creation()` (or the hook confirmed in task 2.1), with no traits added yet — trait
      population happens in task 3.
- [x] 2.3 Declare `race: str | None` and `subrace: str | None` attributes (both default `None`) per
      design.md D-3 — the lore-registry keys the trait-derivation functions in task 3 read as input.
- [x] 2.4 Declare the six non-trait handler placeholders (`sexual`, `buffs`, `equipment`, `skills`,
...
- [x] 2.5 Declare the `disguised_stats` attribute convention (`entity.db.disguised_stats`,
...
- [x] 3.1 Define `race_floor(race: RaceProfile) -> dict[str, int]` per design.md D-4: reads
      ...
      `world.lore.races`.
- [x] 3.2 Define `build_initial_traits(race_key: str, subrace_key: str | None = None, tier: str |
...
- [x] 3.2a Implement the tier/race cross-check inside task 3.2: `STATIC_TIER_REGISTRY[tier]` raises
...
- [x] 3.3 Define `initial_trait_config(race_key: str, subrace_key: str | None = None, tier: str |
...
- [x] 3.4 Define `GAUGE_REGEN_RATE_PCT` (a single flat placeholder regen-rate percentage of max,
...
- [x] 3.5 Define `_resolve_band_position(band: tuple[int, int | None], position: str) -> int` per
      ...
      value per call.
- [x] 3.6 Define `build_initial_traits_for_monster_tier(tier_key: str, position: str = "floor") ->
...
- [x] 3.7 Wire `PlayerCharacter`/`NPC` to a method (e.g. `apply_race_baseline(tier: str | None =
...
- [x] 4.1 Implement `get_display_value(entity, trait_key: str) -> int` per design.md D-8: returns
...
- [x] 4.2 Write `world/rules/tests/test_disguise_boundary.py`'s source-scanning regression test per
...
- [x] 4.3 Write the positive boundary test: construct a `LivingEntity` with true trait values and a
...
- [x] 5.1 Define `PlayerCharacter(LivingEntity)` with `guild_rank` (default `None`), `quest_log`
      (default `[]`), `wallet` (default `0`) — all declared seams per design.md D-11, no behavior.
- [x] 5.2 Define `NPC(LivingEntity)` with `dialogue_memory` (default `None`), `schedule` (default
      `None`) — declared seams per design.md D-11, no behavior.
- [x] 5.3 Define `Monster(LivingEntity)` with `threat_tier` (a `MonsterTier` key, required — used by
...
- [x] 6.1 `typeclasses/tests/test_entities.py` (`EvenniaTest`-based) — every subclass instantiates
...
- [x] 6.2 `typeclasses/tests/test_subclasses.py` — `PlayerCharacter` exposes `guild_rank`,
...
- [x] 6.3 `world/rules/tests/test_traits.py` — all eight trait keys present with the correct trait
      type (gauge/static/counter) per requirement; gauge traits have a max and non-`None` rate;
      static traits compute from base+mod; each pre-subrace baseline falls inside the constructing
      race/tier band, while every post-subrace value equals its documented fractional adjustment
      exactly (without clamping, since legitimate modifiers may cross a band edge). Values in the
      thousands still fail, catching both an hp-derived ratio and a baked-in skill multiplier.
- [x] 6.4 `world/rules/tests/test_race_scale.py` — elf `hp` max equals
      ...
      maximum.
- [x] 6.5 `world/rules/tests/test_subrace_order.py` — per design.md D-5: a `catkin` beastfolk
...
- [x] 6.6 `world/rules/tests/test_monster_scale.py` — a `calamity`-tier `Monster`'s `atk_phys` base
...
- [x] 6.7 `world/rules/tests/test_disguise_boundary.py` — the source-scanning regression test (task
      4.2) and the positive accessor test (task 4.3).
- [x] 6.8 `world/rules/tests/test_tier_construction.py` — per this change's tier-aware construction
...
- [x] 7.1 Run the full `typeclasses/tests/` and `world/rules/tests/` suites and confirm all tests
      pass.
- [x] 7.2 Instantiate one `PlayerCharacter`, one `NPC`, and one `Monster` in a live (or
...
- [x] 7.3 Confirm no module added by this change contains a hardcoded HP/MP/SP/magic-cap/combat-stat
...
- [x] 7.4 Confirm `get_display_value` is not referenced anywhere outside `world/rules/traits.py`
      and its own tests at the end of this change (it has no real caller yet — the three consumers
      are built by later changes).
- [x] 7.5 Confirm no function in `world/rules/traits.py` multiplies a stored trait value by 10, 100,
      or 1000, and that each static baseline starts within its documented band while every
      subrace-adjusted result equals its documented fractional delta — per design.md D-7, skill
      multipliers are never baked into `entity.traits`.
