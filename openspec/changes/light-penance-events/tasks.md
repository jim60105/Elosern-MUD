...
- [ ] 1.2 Implement bounded recent-action evidence and one structured-outcome planner shared by all resolver callers; verify perpetrator identity, NPC/field/battle producers, voluntary/resisted exclusions and rollback.
- [ ] 1.3 Implement exclusive world-time expiry and refresh; verify refetch and exact-boundary behavior with a fixed clock, without read-time storage writes.
- [ ] 1.4 Implement one conditional extra independent strike within existing damage staging; verify first-miss/second-hit, one debit/practice, ordered HP projection and one terminal knockout/defeat.
