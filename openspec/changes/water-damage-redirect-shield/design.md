...
Delete the `water_shield` bounds buff row and its display entry in this change (the node's real binding ships as `water_film` here; change 4 re-points the registry row's `buff_apply:` from the deleted key to `water_film` — the catalog change owns the registry line). The status display entry moves key (`水膜` label kept). `test_buffs.py`'s dev-era `test_buff_water_shield` shape test is updated to the new synthetic-shape posture (per-key authored-row correspondence tests were already retired in the light wave; only what exists stays truthful).
