...
**Reason**: The AVG stage design (`docs/superpowers/specs/2026-09-23-webclient-avg-stage-redesign-design.md` §4, §8.2) collapses the command panel in dialogue mode so the message window spans the band and both portraits share the stage. The design names this requirement as the one it replaces. Its scenario "The dock stays usable during a conversation" is false once the dock is hidden, and a MODIFIED block cannot drop it.
**Migration**: "The command region collapses in dialogue mode and the message window spans the band" states the collapse, the kept overview root, the dialogue focus home, and the key rules. Its "Movement stays reachable during a conversation" scenario replaces "Movement stays one action away" through the minimap. The overview reset on entering dialogue stays in `webclient-exploration-menu`. Tests annotated with the old ID re-anchor to the new one.
