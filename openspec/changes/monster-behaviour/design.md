...
- **No flee/disengage mechanic.** Design doc §6.3 lists "flee" as one of three ways a combat encounter
  ...
  dropped — a future change (unassigned) is the right owner once a flee-execution mechanism exists
  anywhere in the dependency chain for a behaviour tree to call into.
...
- **[Risk] No flee/disengage behaviour exists, so every monster fights to the death or until the
  ...
  chartered to extend. Named as an explicit gap for a future change once a flee-execution mechanism
  exists anywhere to call into (see Open Questions).
...
- Whoever eventually builds a flee-execution mechanism (unassigned) is expected to extend this change's
  decision tree with a flee branch once `Battlefield.fled` actually has a writer somewhere in the
  dependency chain — not possible to build correctly today, per D-1's/Non-Goals' reasoning.

## Open Questions

- **Should a flee/disengage decision be added once some future change builds a `Battlefield.fled`
  writer?** Not built here (Non-Goals) — no dependency this change has access to provides the execution
  mechanism, and inventing one would be out-of-scope architecture work for a monster-behaviour change.
  Left to whichever future change adds flee execution to name this change's decision tree as a consumer.
