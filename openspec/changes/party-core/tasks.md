...
- [ ] 1.1 Create `world/rules/party.py` with `PARTY_MAX_COMPANIONS = 4`, `join_party(npc, player)`,
      ...
      raise.
- [ ] 1.2 Add the `NPC.at_object_delete` hook calling `purge_npc_memberships(self)` so instance
      reclamation, scene teardown, and ordinary deletes free companion slots.
- [ ] 1.3 Add a membership-ownership contract test: no module outside `world/rules/party.py`
      assigns `db.party` or `db.party_member`.
- [ ] 1.4 Add unit/integration tests: valid join binds both sides; party-bound rejection at 4;
...
- [ ] 2.1 Add `run_npc_exchange(speech, character, client)` to `typeclasses/npcs.py` returning a
...
- [ ] 2.2 Refactor `at_talked_to` to compose `run_npc_exchange` + intent application with
      unchanged observable behavior; add typeclass tests proving both behaviors stay green.
- [ ] 2.3 Add dialogue-intent work: extend `NPC_INTENT_KINDS` and `NPC_DIALOGUE_OUTPUT_SCHEMA` in
      `world/ai/npc_dialogue.py` to eight kinds with `party_invite` carrying a boolean `accept`.
- [ ] 2.4 Add the per-kind semantic validator `_validate_party_payload` (exactly one field
      `accept`, a bool) and register it; keep registration atomic/idempotent.
- [ ] 2.5 In `world/rules/npc_intents.py`, route `party_invite` with `accept: true` to
...
- [ ] 2.6 Add validator/applier tests: valid `{accept: true|false}` passes; non-boolean, missing,
...
- [ ] 3.1 Add `commands/invite.py`: `invite <npc> [訊息]` (aliases 邀請, 組隊) resolving a local NPC
...
- [ ] 3.2 Add `commands/leave.py`: `leave <npc>` (alias 解散) resolving a bound companion and
      dismissing via `leave_party(..., reason="dismissed")` with no affinity change.
- [ ] 3.3 Mount `invite` and `leave` on the character cmdset (`commands/default_cmdsets.py`).
- [ ] 3.4 Add the webclient action pair `explore.party_invite` / `explore.party_leave` reusing the
      ...
      the Python/JS protocol validators, and add the webclient affordances for them.
- [ ] 3.5 Wire the auto-leave hook in `world/rules/affinity.py`: replace the no-op body with the
...
- [ ] 3.6 Add command/action tests: invite joins, AI refusal never overridden by the threshold,
...
- [ ] 4.1 Update `docs/game/commands.md` and `docs/game/command-reference.md` with `invite` and
      `leave` entries (keys, aliases, syntax, context); update `tests/test_command_docs.py`
      expectations.
- [ ] 4.2 Run the touched test labels (`world.rules` party/affinity/npc-intents, `world.ai`
      npc-dialogue, `commands`, `typeclasses` npc/entity, `web.webclient.actions`,
      `tests/test_command_docs.py`, `tests/test_ai_transport_contract.py`).
- [ ] 4.3 Run `uv run --locked python -m compileall -q world typeclasses commands server` and keep
...
- [ ] 4.4 Annotate substantive new tests with `covers_requirement` canonical IDs; run
