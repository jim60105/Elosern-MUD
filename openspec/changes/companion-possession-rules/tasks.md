...
- [ ] 1.1 `world/rules/player_control.py`: `is_player_driven(entity)` per the delta (puppeted PC
...
- [ ] 1.2 `world/rules/movement.py::charge_movement`: replace the isinstance check with the
  predicate (flight waiver logic untouched).
- [ ] 1.3 `typeclasses/exits.py::after_successful_movement` room-entry trigger site: gate on the
  predicate (with the live-session condition) instead of the puppeted-PC check.

## 2. The possession writer

- [ ] 2.1 `world/rules/possession.py`: `PossessionError` / `PossessionGateError` /
...
- [ ] 2.2 `release_possession(player, npc, reason)` (idempotent, mirrored clear, event);
...
- [ ] 2.3 `world/rules/party.py`: `leave_party` gains the handback-first guard (raises
...
- [ ] 2.4 `world/rules/service_gate.py::schedule_silenced`: OR in the possessed-NPC leg
  (`npc.db.possessed_by` non-null).
- [ ] 2.5 `typeclasses/npcs.py::LLMNPC.at_talked_to` (and the freeform seam entry that precedes
  the LLM call): refuse when `self.db.possessed_by` is set — fixed 「他現在無法回應你。」, zero
  writes; reuse the existing gate-return shape.

## 3. Commands and docs

- [ ] 3.1 `commands/possess.py`: `CmdPossess` (key `possess`, aliases `附身`, `possess` English
...
- [ ] 3.2 `docs/game/command-reference.md` + `docs/game/commands.md` canonical rows for both;
  curated manifest in `tests/test_command_docs.py` extended; drift contract green.

## 4. Tests

- [ ] 4.1 `world/rules/tests/test_possession.py` (EvenniaTest): gate matrix — one test per reason
...
- [ ] 4.2 `world/rules/tests/test_player_control.py` (pure-ish): four predicate rows incl. the
  stale-attribute window.
- [ ] 4.3 Command tests (`commands/tests/test_possess_commands.py`): resolution errors, gate
  lines, 歸位 release; register new modules in `.github/evennia-shards.json`.
- [ ] 4.4 Silence integration: schedule skip for a possessed non-service companion (the
...
- [ ] 4.5 `covers_requirement` annotations for every requirement across the four delta files.
...
- [ ] 5.2 `uv run --locked python -m tools.observability_lint check` (new modules are adopters —
  named facade imports, no freeze-list additions); `compileall -q world commands typeclasses`.
- [ ] 5.3 Grep: no new inline `isinstance(...PlayerCharacter)` in movement/trigger paths; the
