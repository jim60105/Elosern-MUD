...
skills via a `magic_growth_multiplier:<N>` effect-ID convention (`1.0` if none owned), and change 6's
`growth_rate_multiplier(entity)`. This function SHALL NOT write to any entity attribute.
...
- **WHEN** `effective_magic_growth_multiplier(entity)` is called on an entity with no owned skill
  carrying a `magic_growth_multiplier:` effect and no active `conferred_growth_rate` buff
...
- **WHEN** `effective_magic_growth_multiplier(entity)` is called on a human entity (`learning_multiplier
  == 1.0`) owning a `PASSIVE` skill whose `effects` includes `"magic_growth_multiplier:100"`, with no
  active buff
...
- **WHEN** a human entity has `world.rules.buffs.grant_conferred_growth_rate(entity,
  source_key="elosia", scale=0.5)` applied, no owned `magic_growth_multiplier:` skill, and
...
- **WHEN** an elf entity (`learning_multiplier == 10.0`) owns a `magic_growth_multiplier:100` passive
