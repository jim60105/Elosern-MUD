...
- [ ] 1.1 Add `apply_cost_modifier(amount: int, percentage: str | None) -> int` to
...
- [ ] 1.2 Add focused unit tests for the helper (in `world/rules/tests/test_combat_modifiers.py` or
...
- [ ] 2.1 Add a private `_adjusted_costs(actor, skill) -> dict[str, int]` in
...
- [ ] 2.2 Rewire `_step2_resource_check` (`action.py:245-251`) to compare stored trait values
      against `_adjusted_costs` instead of `skill.cost` verbatim
- [ ] 2.3 Rewire `_step6_resource_deduction` (`action.py:579-599`) to use `_adjusted_costs` for
...
- [ ] 2.4 Rewire the resource loop in `_skill_wide_failure` (`action_preview.py:96-98`) to apply
...
- [ ] 2.5 Tests (each with `covers_requirement` where a main-spec requirement applies): a
...
- [ ] 3.1 Add pure helpers in `world/rules/combat.py`: an adjusted-attack helper that adds the
...
- [ ] 3.2 Use both helpers in `_handle_damage` (`combat.py:274-275`) so the magnitude becomes
      `max(round(adjusted_attack * multiplier) - adjusted_defense, floor)`
- [ ] 3.3 Tests: physical damage with and without `atk_phys` rules (exact staged amount
...
- [ ] 3.4 Confirm `test_to_hit_calibration.py` and `test_golden_combat.py` still pass unchanged
      (to-hit math is untouched)

## 4. Damage-estimation surfaces

- [ ] 4.1 Switch `_expected_damage_per_attack` (`overwhelm.py:159-175`) to the shared
      adjusted-attack/defense helpers
- [ ] 4.2 Switch `_choose_skill.expected_damage` (`monster_behaviour.py:275-285`) to the shared
...
- [ ] 4.3 Tests: the overwhelm estimator includes both flat bonuses in its terms; a monster
      choosing between physical and magic candidates ranks the physical candidate with its
      `atk_phys` bonus

## 5. Data and presentation

- [ ] 5.1 Update inline comments in `world/rules/rulebook/combat_modifiers.yaml` for the four now-
...
- [ ] 5.2 Make no changes to `status_query.py` or `status_display.yaml`, and verify the status
...
- [ ] 6.1 Read `docs/development/spec-test-traceability.md`; obtain canonical requirement IDs with
...
- [ ] 6.2 Run `uv run --locked python -m tools.spec_traceability check` and confirm the existing
      annotations on the amended "eight previously-dead" requirement still resolve
- [ ] 6.3 Run the affected suites (`world.rules` package tests, `world.skills` tests if touched),
...
- [ ] 6.4 Run `openspec validate --change fix-combat-rule-sinks --strict` and confirm all
