...
- [ ] 1.1 Confirm change 12 (`map-anchor-grid`) is implemented: `typeclasses/rooms.py::GridRoom` and
...
- [ ] 1.2 Confirm, by import inside an `EvenniaTest`-backed process (`uv run --locked evennia test
...
- [ ] 1.3 Confirm no class named `SceneArchetypeMixin` or `TerrainRoom` exists anywhere in the
      repository yet, and that `typeclasses/exits.py` contains no class named `WildernessGateExit` or
      `WildernessReturnExit`.
- [ ] 1.4 Confirm `world/rules/rulebook/clock.yaml::command_defaults` has no `wilderness_move` key yet.

## 2. Wilderness terrain lore registry (`world/lore/wilderness_regions.py`)

- [ ] 2.1 Implement the frozen `WildernessRegion` dataclass (`key: str`, `display_name_zh: str`,
      `nation_key: str | None`, `terrain_flavor_zh: tuple[str, ...]`) per design.md D-1.
- [ ] 2.2 Implement `WILDERNESS_REGION_REGISTRY: dict[str, WildernessRegion]` with exactly seven
...
- [ ] 2.3 Test: `WILDERNESS_REGION_REGISTRY` has exactly seven entries, keys match the seven listed
      above, and no `display_name_zh` is duplicated.
- [ ] 2.4 Test: every entry's `terrain_flavor_zh` has at least two elements, and every `nation_key`
...
- [ ] 3.1 Implement the frozen `WildernessEntryPoint` dataclass (`anchor_key: str`, `wilderness_xy:
      tuple[int, int]`) per the `wilderness-gateway` spec.
- [ ] 3.2 Implement `WILDERNESS_ENTRY_REGISTRY: dict[str, WildernessEntryPoint]` with exactly one
      entry: `"capital_altoria": WildernessEntryPoint("capital_altoria", (60, 100))`.
- [ ] 3.3 Test: `WILDERNESS_ENTRY_REGISTRY` has exactly one entry, keyed `"capital_altoria"`.
- [ ] 3.4 Test: every entry's `anchor_key` exists in `world.lore.anchor_placement.
      ANCHOR_PLACEMENT_REGISTRY` (change 12).
- [ ] 3.5 Test: `WILDERNESS_ENTRY_REGISTRY["capital_altoria"].wilderness_xy` is within
...
- [ ] 4.1 Import `WILDERNESS_REGION_REGISTRY` and `WILDERNESS_ENTRY_REGISTRY` in `world/lore/sync.py`
...
- [ ] 4.2 Test: after `sync_all()` runs, seven `LoreRecord` Scripts exist keyed
      `"lore:wilderness_regions:<key>"` and one exists keyed
      `"lore:wilderness_entries:capital_altoria"`.
- [ ] 4.3 Test: calling `sync_all()` twice leaves the same counts (idempotency), extending the
      existing test pattern in `world/lore/tests/test_sync.py` rather than duplicating its structure.
- [ ] 4.4 Confirm (via `git diff`) that this task's edit to `world/lore/sync.py` is additive only, and
...
- [ ] 5.1 Add `SceneArchetypeMixin` per design.md D-2: a plain class carrying `scene_archetype: str |
      None = AttributeProperty(default=None)`.
- [ ] 5.2 Change `GridRoom`'s base classes from `(XYZRoom,)` to `(SceneArchetypeMixin, XYZRoom)`, and
...
- [ ] 5.3 Add `TerrainRoom(SceneArchetypeMixin, evennia.contrib.grid.wilderness.wilderness.
      WildernessRoom)` with no additional members beyond what the mixin and `WildernessRoom` already
      provide.
- [ ] 5.4 Test: re-run change 12's own `scene_archetype` tests for `GridRoom` (default `None`,
...
- [ ] 5.5 Test: `SceneArchetypeMixin` appears in both `GridRoom.__mro__` and `TerrainRoom.__mro__`, and
...
- [ ] 5.6 Test: `TerrainRoom.__mro__` does not include `evennia.contrib.grid.xyzgrid.xyzroom.XYZRoom`.
- [ ] 5.7 Test: creating a `TerrainRoom` (via `create_object`) with no `scene_archetype` supplied
...
- [ ] 6.1 Create `world/maps/wilderness_provider.py`. Implement `region_for_coordinates(x: int, y:
...
- [ ] 6.2 Implement `terrain_description(x: int, y: int) -> str` per design.md D-1's arithmetic
      formula (`(x * 92821 + y * 68917) % len(variants)`).
- [ ] 6.3 Test: `region_for_coordinates`/`terrain_description` are pure — calling either twice with the
      same `(x, y)` returns the identical result.
- [ ] 6.4 Test: every one of the seven region keys is returned by `region_for_coordinates` for at
      least one coordinate within the valid bounds (task group 7 supplies the bounds constants this
      test iterates over).
- [ ] 6.5 Test: the central mountain band (`x` in `[100, 123]`) returns `"central_mountains"` at both
...
- [ ] 6.6 Test: `region_for_coordinates(60, 100)` (the registered `capital_altoria` entry point)
      returns `"western_hills_valleys"`.
- [ ] 6.7 Test: `terrain_description(x, y)`'s return value is always one of
      `WILDERNESS_REGION_REGISTRY[region_for_coordinates(x, y)].terrain_flavor_zh`.
- [ ] 6.8 Test: `world/maps/wilderness_provider.py` contains no import from `world.ai`, no HTTP client
...
- [ ] 6.9 Test (literal pin, `wilderness-terrain/spec.md`'s own scenario): `terrain_description(60,
...
- [ ] 7.1 Add `WILDERNESS_KM_PER_CELL = 10`, `WILDERNESS_MAX_X = 223`, `WILDERNESS_MAX_Y = 223`, and
      `WILDERNESS_NAME = "elosern"`.
- [ ] 7.2 Implement `ElosernWildernessMapProvider(WildernessMapProvider)` per design.md D-4:
...
- [ ] 7.3 Test: `is_valid_coordinates(wilderness, (0, 0))` and `(223, 223)` return `True`;
      `(-1, 0)` and `(224, 0)` return `False`.
- [ ] 7.4 Test: `(WILDERNESS_MAX_X + 1) * WILDERNESS_KM_PER_CELL` squared is within 1% of 5,000,000
      (the design.md D-4 arithmetic, asserted as a regression, not just computed by hand once).
- [ ] 7.5 Test: `get_location_name((x, y))` returns `WILDERNESS_REGION_REGISTRY[
      region_for_coordinates(x, y)].display_name_zh` for several representative coordinates spanning
      multiple regions.
- [ ] 7.6 Test (`EvenniaTest`): entering the wilderness through `enter_wilderness(char, coordinates=
...
- [ ] 7.7 Test: moving a character to a second coordinate in a different region, then recycling that
...
- [ ] 8.1 Add `WildernessGateExit(Exit)` per design.md D-6 (as corrected): `at_traverse` fully
...
- [ ] 8.2 Add `WildernessReturnExit(evennia.contrib.grid.wilderness.wilderness.WildernessExit)` per
...
- [ ] 8.3 Test (`EvenniaTest`): traversing a `WildernessGateExit` places the traverser in a
      `TerrainRoom` at the registered entry coordinate, and `get_world_clock().tick` increases by
      exactly `9000`.
- [ ] 8.4 Test: an `enter_wilderness()` failure inside `WildernessGateExit.at_traverse` (simulate via
      an invalid coordinate) leaves `get_world_clock().tick` unchanged, and does not raise.
- [ ] 8.5 Test: every one of the eight directional exits on a room created by
      `ElosernWildernessMapProvider` is a `WildernessReturnExit` instance.
- [ ] 8.6 Test: traversing `"south"` from the registered entry coordinate moves the traverser to the
...
- [ ] 8.7 **Test (blocking-defect regression — the core proof of distance-proportional travel):**
      ...
      by `0`.
- [ ] 8.8 Test: traversing any other direction, or `"south"` from a non-entry coordinate, routes
...
- [ ] 8.9 Test: after returning via `WildernessReturnExit`, the traverser no longer appears in
      `WildernessScript.db.itemcoordinates`, and the vacated room appears in
      `WildernessScript.db.unused_rooms` (if no one else remained in it).
- [ ] 8.10 Test (Fix 5, movement hooks): a mock/stub `at_pre_move` on the traverser that returns
...
- [ ] 9.1 Add `command_defaults.wilderness_move: 9000` per design.md D-5's arithmetic (10 km/cell ÷ 4
      km/h = 2.5h = 9000s). Do not modify the existing `move`/`converse`/`cast` entries.
- [ ] 9.2 Test: `CLOCK_YAML["command_defaults"]["wilderness_move"] == 9000` and
      `CLOCK_YAML["command_defaults"]["move"] == 30` (unchanged).
- [ ] 9.3 Test (regression, raw arithmetic): the full 224-step continent-crossing arithmetic from
...
- [ ] 10.1 Implement `sync_wilderness()` per design.md D-7: call `create_wilderness(name=
...
- [ ] 10.2 Test: `sync_wilderness()` against a database where `sync_grid()` has already run creates a
...
- [ ] 10.3 Test idempotency: calling `sync_wilderness()` a second time leaves exactly one
      `WildernessScript` keyed `WILDERNESS_NAME` and exactly one gate exit at the North Gate.
- [ ] 10.4 Test the absent-North-Gate degradation path: `sync_wilderness()` against a database with no
      `capital_altoria` North Gate room still creates the `WildernessScript`, creates no gate exit, and
      does not raise.
- [ ] 10.5 Confirm (via `git diff`) that `sync_grid()`'s own body is unmodified by this task group.

## 11. Server startup wiring (`server/conf/at_server_startstop.py`)

- [ ] 11.1 Add a call to `world.maps.bootstrap.sync_wilderness()` inside `at_server_start()`,
      immediately after the existing `sync_grid()` call.
- [ ] 11.2 Test (source-order check): `at_server_start()`'s source lists `sync_wilderness()` after
      `sync_grid()`.
- [ ] 11.3 Test (integration): simulating a server start against a fresh test database results in the
...
- [ ] 12.1 Add a `"wilderness"` row (or rows) to `MATRIX_IMPORTS` covering
      `evennia.contrib.grid.wilderness.wilderness.{WildernessMapProvider, WildernessRoom,
      WildernessExit, WildernessScript, create_wilderness, enter_wilderness}` — the symbols this
      change actually calls.
- [ ] 12.2 Test: `ContribMatrixTests.test_matrix_imports_and_attributes` passes with the new rows
      included.

## 13. Full-suite round-trip integration test

- [ ] 13.1 Write one `EvenniaTest`-based integration test that: runs `sync_all()`, then `sync_grid()`,
...
- [ ] 13.2 Test: `return_appearance()` (inherited from `WildernessRoom`/`TerrainRoom`) renders without
      error for at least one wilderness room, proving the terrain layer is not just spawned but
      actually displayable.

## 14. Verification

- [ ] 14.1 Run the full test suite added by this change (`world/lore/tests/`, `typeclasses/tests/`,
      `world/maps/tests/`, `tests/test_contrib_matrix.py`) and confirm every test passes.
- [ ] 14.2 Confirm (via `git diff`) that every edit to an already-landed file (`world/lore/sync.py`,
...
- [ ] 14.3 Confirm no file added or edited by this change contains a reference to `world/ai/` or an
      LLM call.
- [ ] 14.4 Confirm `settings.START_LOCATION` is unchanged, and that this change does not alter
      `rulebook/clock.yaml`'s existing `move`/`converse` entries.
- [ ] 14.5 Run `openspec validate map-wilderness --strict` and confirm it passes.
