...
New `world/rules/movement_settlement.py` exposes `settle_movement(traversing_object, source_location, *, traverse, destination=None, wilderness_coordinates=None, wilderness_source_coordinates=None)` where `traverse` is the exit's own traversal body and `destination` is the room the traverser will land in when the lineage knows it (the plain-exit target and the wilderness grid-return target), needed so the snapshot can cover the destination room's quest-observation surfaces (D2). Shape:
...
- `MovementCostMixin.at_traverse` (new override, `typeclasses/exits.py`) opens the boundary around `super().at_traverse(...)`, covering `Exit` and `CostedXYZExit` (neither `DefaultExit` nor `XYZExit` overrides `at_traverse`; MRO reaches the mixin). `at_post_traverse` is untouched, so `after_successful_movement` ordering and the existing source-inspection test stay valid. The delegation is bound before the closure (`stock_traversal = super().at_traverse`) because Python's zero-argument `super()` cannot resolve inside a nested function that closes over other locals.
...
- `world/rules/npc_schedules.py::_first_traversable_exit` admits the mixin's delegating implementation (`MovementCostMixin.at_traverse`) alongside the stock `DefaultExit.at_traverse` for the schedule-route qualification check: the mixin delegates to the stock implementation and honors the requested destination, so an NPC schedule move through a project exit still qualifies; redirecting exits (wilderness, custom) stay excluded exactly as before.
...
- **Clock-singleton materialization on a failed move** → The pre-transaction snapshot reads the clock through `get_world_clock()`, which can create the idempotent world-clock Script when no advance has ever run; production always has the singleton after the first charged action, so the write is accepted and documented.
