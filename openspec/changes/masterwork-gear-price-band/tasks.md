...
- [x] 1.1 Add the `masterwork_gear` entry to `PRICE_TABLE` in `world/lore/economy.py` with floor
...
- [x] 1.2 Do NOT add the keepsake-offer rejection here. `commerce-assortment-registry` rewrites
...
- [x] 2.1 Move `dark_elf_kimono`, `shadow_blade`, `shadow_blade_echo`, `dark_elf_ninja_garb`,
...
- [x] 2.2 In `world/lore/items/data_regional_equipment.py`, move `elven_forest_veil` from `armor`
  to `masterwork_gear` and raise `elven_longbow` to `EPIC`.
- [x] 2.3 Raise `prism_charm` to `RARE` in `world/lore/items/data_armor_accessories_materials.py`
  and `elven_candied_blossom` to `RARE` in `world/lore/items/data_inspect_only_codex.py`.
- [x] 2.4 Confirm the nine remaining `relic` rows are untouched, and verify with
  `uv run --locked evennia test --settings test_settings.py --keepdb world.lore.tests.test_items`.

## 3. Handoff

- [x] 3.1 Run `uv run --locked python -m tools.test_data_lint check` and confirm no new tagged
  data-contract test was introduced by this change.
- [x] 3.2 Run `openspec validate masterwork-gear-price-band --strict` and the focused labels above.
