...
- [ ] 1.1 `commands/skip.py::CmdRest`: parse the optional
  `practice <skill>` clause after the duration token; parse → safety gate →
  booking preflight in that fixed order.
- [ ] 1.2 `world/rules/time_skip.py`: `preflight_practice_booking(actor,
  ...
  zh-tw messages.
- [ ] 1.3 Rejections: message printed, `WorldClock.advance()` never called, no
  booking recorded (zero clock advance on every rejection path).

## 2. Settlement writer

- [ ] 2.1 `world/rules/clock.py`: the `practice_settlement` stage reads the
  ...
  the booking.
- [ ] 2.2 COMBAT-sourced advance leaves the booking untouched (skipped stage).
- [ ] 2.3 `world/rules/progression.py`: closed-form hourly grant helper
...
- [ ] 2.4 `world/rules/rulebook/progression.yaml`: `PRACTICE_XP_PER_STUDY_HOUR:
  10.0` with the playtest-recalibration header note.

## 3. Tests

- [ ] 3.1 Command tests (extend the existing `commands` skip tests): valid
...
- [ ] 3.2 Clock integration: booking survives a COMBAT-source advance and
...
- [ ] 3.3 No new test modules expected → `.github/evennia-shards.json`
  untouched (register in-change if a new file is created).
- [ ] 3.4 Command docs (in this change): `rest` syntax row + description gain the
...
- [ ] V1 `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --keepdb commands world.rules`
...
- [ ] V4 `openspec validate declared-practice-skip --strict`
- [ ] V5 `git diff --check`
