...
- [x] 5.2 Verify the traceability gate and docs links, then focused labels (`world.ai.tests.test_client`, `tests.test_container_contract`) and `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --noinput --parallel 16 commands server typeclasses world web.webclient`; `uv run --locked python -m tools.spec_traceability check`; `openspec validate llm-endpoint-wire-configuration --strict`; `git diff --check` clean.
