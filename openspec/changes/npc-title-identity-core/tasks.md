...
- [ ] 6.4b **生產者清單迴歸案**（放 `world/rules/tests/test_npc_identity.py`）：靜態掃描全 repo 非測試碼建立 NPC 族實體的呼叫，斷言其檔案集合恰為已知五點——`world/imports/loader.py`、`world/quests/scene_builder.py`、`world/rules/guild_economy.py`、`world/rules/guild_exams.py`、`world/rules/onboarding.py`（後者為 onboarding 豁免，見 authored change design D7a）；新生產落點一出現即轉紅，強制新作者面被登記。掃描以 `pathlib` 走訪 `world/`、`typeclasses/`、`commands/`、`server/`，跳過 `tests/` 目錄與 `test_*.py`，並用 **DOTALL 正則**比對 `create_object\(\s*(NPC|LLMNPC)\b`（實際呼叫存在跨行寫法，單行前綴比對會漏）。
