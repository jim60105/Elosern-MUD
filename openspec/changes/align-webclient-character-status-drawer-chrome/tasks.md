...
- [ ] 1.1 Grep `web/tests/browser/` and `web/webclient-app/tests/` for every
...
- [ ] 1.2 Confirm (by reading `ConditionChips.vue:211-244`) the exact five `.chip--{beneficial,
...
- [ ] 2.1 Add a `character-status-drawer__section-label` element (a `<p>` or `<span>`, matching
...
- [ ] 3.1 Wrap the vitals section's three rows in a `.character-status-drawer__statgrid` grid container
...
- [ ] 3.2 Apply the same `statgrid`/`statrow` treatment to the traits section's rows
      ...
      Non-Goals).
- [ ] 3.3 Apply the same treatment to the two guild-counter rows (`character-status-drawer__guild-rank`,
...
- [ ] 4.1 Replace the condition roster's flex-row markup with a `.character-status-drawer__pillrow`
...
- [ ] 4.2 Apply the five severity tint variants from task 1.2 to the pill (`background`/`border-color`/
      `color` per `condition.severity`), matching `ConditionChips.vue`'s existing mapping exactly.
- [ ] 4.3 Confirm the empty state (`character-status-drawer__conditions-empty`, "無狀態") still renders as
      plain text beneath the new "狀態" heading, not as an empty pill row.

## 5. Update any structural-selector tests found in task 1.1

- [ ] 5.1 Update each test flagged in task 1.1 to select on the preserved `data-testid`/text content
      instead of the removed structural class, with no change to what the test actually asserts.

## 6. Verify

- [ ] 6.1 Extend the existing `web/webclient-app/stories/Data/CharacterStatusDrawer.stories.js`
...
- [ ] 6.2 Add a component test asserting the new section-heading elements are present for each section and
      that the condition pills carry the correct severity-based class/colour per `condition.severity`.
- [ ] 6.3 Add a browser or Vitest bounding-box assertion (reusing the non-intersection pattern from
...
- [ ] 6.4 Re-run the full existing `character-status-drawer` test suite (component + browser) to confirm no
      regression beyond the intended visual change — same testids, same values, same degrade-by-section
      behavior.
- [ ] 6.5 Re-screenshot the live client's `角色狀態` drawer at 1440×900 and visually confirm it now matches
      the design's card-tile/pill-badge/section-heading presentation.

## 7. Close out

- [ ] 7.1 `openspec validate align-webclient-character-status-drawer-chrome --strict`.
- [ ] 7.2 Run the focused JS gates (`npm test`, Storybook build + component-coverage) and the smallest
