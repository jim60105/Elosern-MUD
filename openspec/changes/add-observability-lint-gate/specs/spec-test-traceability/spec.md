...
Package-local project tests and top-level repository contract tests MUST have
disjoint discovery ownership and each test MUST execute exactly once. The
workflow MUST fail if any test fails, any requirement lacks a test
...
fails. The project SHALL target aggregate branch coverage of at least 90% as a
