...
- 新增 code：`world/observability/`、`tools/observability_lint.py`、`tools/observability_freeze.json`、命令基類事件層。
- 修改：`server/conf/at_server_startstop.py`、`server/scene_flavor_service.py`、`server/option_proposal_service.py`、`server/title_nomination_service.py`、`commands/` 基類、`.github/workflows/quality-gate.yml`、`tests/test_quality_gate_contract.py`、`AGENTS.md`、`.github/evennia-shards.json`（新測試模組登記）。
