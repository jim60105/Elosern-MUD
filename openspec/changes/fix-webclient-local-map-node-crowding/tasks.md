...
- [ ] 1.1 Add a Storybook story (or reuse `LOCAL_MAP_SAMPLE`) rendered at the island's real `230px`
      width and capture the current overlap visually, as a before-state reference for the fix.
- [ ] 1.2 Extend the existing `web/tests/browser/test_browser_local_map.py::
...
- [ ] 2.1 Replace the single `CELL` constant with separate column-pitch and row-pitch constants sized so
      a node's marker + its label's rendered line height + a minimum gap fit before the next row.
- [ ] 2.2 Update `nodePos` to use the column pitch for `x` and the row pitch for `y`, and update
      `canvasWidth`/`canvasHeight` accordingly (keep `LABEL_BAND` as the trailing margin below the last
      row).
- [ ] 2.3 Re-tune marker radii (`local-map__marker--*`) and/or `LABEL_MAX` only if the new pitch alone
      does not clear horizontal label-to-label crowding on a full row of adjacent nodes.
- [ ] 2.4 Verify the connector `<line>` elements are no longer fully occluded by the (now correctly
      spaced) markers at the new pitch.

## 3. Verify no regression to existing behavior

- [ ] 3.1 Confirm `activateNode`/`explore.move` submission still targets the correct node after the
      marker/cell size change (task 2 must not touch `edgeGeoms`, `nodeById`, or `activateNode`).
- [ ] 3.2 Re-run the existing `webclient-local-map` browser/Vitest coverage plus the new assertion from
      task 1.2 until green.
- [ ] 3.3 Re-check the H2 island-vs-dock non-overlap browser assertion at 1440×900 and 1280×720 (roadmap
      §8 risk item) now that the island's natural height may grow on multi-row rooms.
- [ ] 3.4 Visually re-verify `MapOverlay.vue` ("展開全地圖") inherits the fix with no code change of its
      own, using the same room/payload used for the before-state capture in task 1.1.
- [ ] 3.5 Add a maximal-height, minimal-width lattice fixture (e.g. a narrow 2-column × 64-row payload,
...
- [ ] 4.1 `openspec validate fix-webclient-local-map-node-crowding --strict`.
