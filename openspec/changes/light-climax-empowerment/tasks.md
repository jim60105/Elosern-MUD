...
- [ ] 1.2 Implement bare pleasure_peak through canonical gain 100 then 0 with declared actor binding; verify starting phase boundaries, no artificial extension and an already locked caster rejection.
- [ ] 1.3 Implement phase reaction loading/validation and one canonical transition hook using the existing condition evaluator and qualified ownership; verify invalid edges, rule/direct-gain sources and no retroactive mid-cycle grant.
- [ ] 1.4 Implement persisted cycle marker lifetime and configured state maximum selection; verify afterglow actions, neutral expiry, qualification loss and unchanged fixed coefficients/gates.
- [ ] 1.5 Expand all existing transition-capable caller snapshots, not only the new prefix; verify failure after action/clock phase entry restores marker, buffs, counters and underlying traits.
