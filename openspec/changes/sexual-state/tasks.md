...
- [ ] 3.2 Implement the three-way construction dispatch per design.md D-2/D-5: `entity.db.sexual`
...
- [ ] 3.4 Implement `build_monster_sexual_baseline() -> dict` per design.md D-5: floor level for every
...
- [ ] 3.7 Implement the public property surface on `SexualState` per design.md D-2: `.arousal`,
...
- [ ] 3.8 Implement `virgin`/`experience_types` accessors on `SexualState` per design.md D-2: `virgin`
      ...
      getter plus `add_experience_type(key)` — the only mutator, always a union, never a replacement
      or removal.
- [ ] 3.9 Mount `entity.sexual` in `typeclasses/entities.py`, **replacing** change 3's
...
- [ ] 4.1 Define `_VALID_CLIMAX_TRANSITIONS` per design.md D-4: the cycle
      `未達→接近`, `接近→{進行中, 未達}`, `進行中→餘韻`, `餘韻→{未達, 接近}`.
- [ ] 4.2 Implement `_apply_climax_phase_set(entity, target_level: str) -> str | None`: no-ops
...
- [ ] 5.1 Define `DECAY_CONFIG` per design.md D-6: `arousal` (interval 1800s, floor `平靜`),
...
- [ ] 5.2 Implement `decay_tick(entity, elapsed_seconds: int) -> None`: accumulates
...
- [ ] 5.3 Implement `reset_daily_counters(entity) -> None`: sets `climax_today` to `0`. No other
      field changes.
- [ ] 5.4 Confirm neither `decay_tick` nor `reset_daily_counters` references trait-regen scheduling,
...
- [ ] 5.5 Document, in `decay_tick()`'s own docstring, the target-field naming convention a future
...
- [ ] 6.2 `world/rules/tests/test_climax_phase_cycle.py` — per the `sexual-state-handler` capability:
...
- [ ] 6.3 `world/rules/tests/test_sexual_state.py` — handler-level tests for `virgin`/
...
- [ ] 6.4 `world/rules/tests/test_monster_sexual_baseline.py` — per the `sexual-state-handler`
...
- [ ] 6.5 `world/rules/tests/test_sexual_decay_and_reset.py` — per the `sexual-state-handler`
...
- [ ] 6.6 `world/rules/tests/test_sexual_state.py` (or a dedicated module) — construction-path tests:
...
- [ ] 8.1 Run the full `world/rules/tests/` suite added or extended by this change and confirm every
      test passes.
- [ ] 8.2 Confirm no function in `world/rules/sexual_state.py` writes `climax_phase`'s value except
      through `_apply_climax_phase_set()` (grep by hand, mirroring change 3's task 7.5 and change 6's
      task 6.2 discipline).
- [ ] 8.3 Confirm `world/rules/sexual_state.py` contains no `rulebook/sexual.yaml` reference, no
      `apply_event()` definition, and no per-rule test naming convention — a plain read-through
      confirming the scope boundary with change 7b held, since nothing in this change should
      anticipate or partially implement 7b's rule table.
- [ ] 8.4 Confirm `design.md`'s D-7 (the `variable_rule.md` ambiguity/self-contradiction analysis) is
      present and unedited from the original pass, so change 7b's author has it to start from.
- [ ] 8.5 Run `openspec validate sexual-state --strict` and confirm it passes.
