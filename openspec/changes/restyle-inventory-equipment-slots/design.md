...
The component owns a closed map from the four existing equipment-slot names to local inline SVGs. A symbol represents a fixed slot role, never the item in it. The three single-item slots display their actual item name beneath the cell; the accessory summary cell displays the committed accessory count and the existing detail list still displays every accessory name.
