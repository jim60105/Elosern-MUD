...
- [ ] 4.1 Authored venues and hosts: implement the church-place derivation in `world/lore/church/` from places authoring a `church` kwarg (the `shops.py` derivation pattern, incl. duplicate-kwarg fail-closed) and author the kwarg on the church place records (test the derived set is non-empty and complete); create the `ChurchHost` typeclass component (sibling of `GuildStaff`, zero-state adapter) and attach it to the one clergy NPC roster row (艾莉安娜·寒水 high celebrant; the sanctum steward 羅海西亞·芬威克 stays a plain merchant with no ChurchHost and no arousal seed, per owner decision) through her profession blueprint, adding the small raised-initial-arousal authoring kwarg to her spawn data; verify `place-driven-service-sync` convergence stays idempotent with the new component. Annotate ID 3.

- [ ] 4.2 Closing gate: register every test module this change added in exactly one `.github/evennia-shards.json` shard and put data-contract-tagged modules in the test-data ledger files (list them in the PR body). After the delta syncs to the main spec, run `uv run --locked python -m tools.spec_traceability check` and confirm all 4 IDs above resolve with zero uncovered requirements and zero unknown-annotation errors.
