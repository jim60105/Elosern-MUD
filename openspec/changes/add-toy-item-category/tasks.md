...
- [ ] 1.5 Add a Vitest case rendering one payload with a mapped icon key and one with an absent presentation, asserting the first shows that key's own glyph and label and only the second shows the fallback. Verify with the focused Vitest file for the inventory panel.
