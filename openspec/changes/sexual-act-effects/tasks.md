...
- [ ] 1.1 Create `world/rules/rulebook/sexual_act_effects.yaml` with `participant_multipliers`
      (`"1": 1.0`, `"2": 1.1`, `"3+": 1.2`) and `climax_extension_threshold: 20`.
- [ ] 1.2 Implement `load_effects_config()` in `world/rules/sexual_act_effects.py`, validating exactly
      the keys/shape in design.md D-3, failing closed on any deviation.

## 2. Core functions

- [ ] 2.1 Implement `resolve_part(entity, declared_part)` per design.md D-4, with the `Monster` import
      deferred inside the function body.
- [ ] 2.2 Implement `participants(actor, targets)` per design.md D-2.
- [ ] 2.3 Implement `compute_pleasure_gain(participant, part, base_pleasure, ratio,
      participant_count)` per design.md D-3, reading `PLEASURE_CONFIG` read-only.
- [ ] 2.4 Implement the explicit `_COUNTER_MUTATORS` table per design.md D-6 (all eleven entries).

## 3. Effect dataclasses and dispatch

- [ ] 3.1 Add `PleasureEffect(act_key: str)` and `SexualCounterEffect(act_key: str)` to
      `world/skills/effects.py`.
- [ ] 3.2 Add the two `parse_effect` dispatch branches using `_parse_single_arg`.

## 4. Effect handlers

- [ ] 4.1 Implement `_handle_pleasure_effect` in `world/rules/action.py` per design.md D-5: resolve
      ...
      participant.
- [ ] 4.2 Implement `_apply_pleasure_gain(entity, gain)` per design.md D-5's full sequence: capture
...
- [ ] 4.3 Implement `_handle_sexual_counter_effect` per design.md D-7: `actor_counters` on the actor,
      `participant_counters` on every other participant, via `_COUNTER_MUTATORS`.
- [ ] 4.4 Register both via `register_effect_handler`, surfaces `frozenset({"sexual"})`, no required
      event context.
- [ ] 4.5 Both handlers raise `RejectedAction(RejectReason.EFFECT_RESOLUTION_FAILED, ...)` if
...
- [ ] 5.1 Update `_act_family()` (`world/skills/sexual_acts/_builder.py`) to set
      `effects=[f"pleasure:{key}", f"sexual_counter:{key}", *(f"sexual_event:{name}" for name in
      row.sexual_events)]` for every row.
- [ ] 5.2 Add the `_FORBIDDEN_SEXUAL_EVENTS` frozenset per design.md D-8
...
- [ ] 5.3 Add the two new structural checks to `sexual-act-registry`'s existing test module (solo
...
- [ ] 6.1 `world/rules/tests/test_sexual_act_effects.py`: unit tests for `resolve_part`,
      `participants`, `compute_pleasure_gain` (including the sensitivity/shame multiplier cases and
      the ratio=0 case).
- [ ] 6.2 End-to-end test casting a test-local act (built the same way `sexual-act-registry`'s own
...
- [ ] 6.3 Climax-phase progression tests per design.md D-5's scenarios: 未達→接近 only on a first
...
- [ ] 6.4 Wetness tests: an arousal-band-crossing gain raises `wetness.value` by exactly one; a gain
      that stays within the same band leaves it unchanged.
- [ ] 6.5 `sexual_event:` reuse test: an act declaring `sexual_events=("frequent_stimulation",)`
...
- [ ] 6.6 Forbidden-events structural test: a hypothetical act declaring
      `sexual_events=("stimulus_applied",)` fails at `_act_family()` construction time; one declaring
      `("direct_stimulus_applied",)` succeeds.
- [ ] 6.7 Structural test for `_COUNTER_MUTATORS` per design.md D-6.

## 7. Traceability and verification

- [ ] 7.1 Run `uv run --locked python -m tools.spec_traceability list` and annotate every new test
      with `covers_requirement` for the three capabilities this change touches.
- [ ] 7.2 Run `uv run --locked python -m tools.spec_traceability check`.
- [ ] 7.3 Run `uv run --locked evennia test --settings test_settings.py world.rules world.skills`.
- [ ] 7.4 Run `uv run --locked python -m compileall -q world`.
- [ ] 7.5 Run `openspec validate sexual-act-effects --strict`.
