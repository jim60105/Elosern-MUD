...
- [ ] 1.1 In `web/webclient-app/components/LocalMap.vue`, delete the meta-row
  ...
  control".
- [ ] 1.2 In the same file, render as the FIRST child of the available branch a
...
- [ ] 1.3 Add the layering CSS (design D1): `position: relative` on
...
- [ ] 1.4 Give the affordance a `:focus-visible` treatment that draws on its own
  ...
  gate.
- [ ] 1.5 Leave `onIslandClick` and its
  ...
  takes focus instead).
- [ ] 1.6 Confirm the island root still carries no `role` and no `tabindex`, and
...
- [ ] 2.1 In `LocalMap.vue`, remove `selectedId`, `hoveredId`, `activeNode`,
...
- [ ] 2.2 Replace `detailParts` with a single computed that yields
...
- [ ] 2.3 Restyle `.local-map__detail` to the token-driven readout treatment
...
- [ ] 2.4 Confirm the remembered list's own item label is untouched, so a
...
- [ ] 3.1 In `web/webclient-app/stores/elosern.js`, replace
...
- [ ] 3.2 Cover every fallback branch in the same suite: map panel absent; map
...
- [ ] 4.1 Update `web/webclient-app/stories/World/LocalMap.stories.js` so the
...
- [ ] 5.1 Re-run `web/tests/browser/test_browser_contextual_hud.py`'s overlay
...
- [ ] 5.2 Extend `test_browser_contextual_hud.py` under
...
- [ ] 5.3 Extend `web/tests/browser/test_browser_shell.py`'s
...
- [ ] 5.4 Re-run `test_browser_local_map.py`, `test_browser_layout.py` and
...
- [ ] 5.5 Confirm `node --test web/static/webclient/js/tests/*.test.js` is
...
- [ ] 6.3 Browser classes in wave 5 green locally within the one-class budget,
  or deferred to CI per repo policy where they exceed it.
- [ ] 6.4 `uv run --locked python -m tools.spec_traceability check` green — all
  ...
  anchor stays valid.
- [ ] 6.5 `openspec validate webclient-minimap-04-island-single-affordance
  --strict` green.
