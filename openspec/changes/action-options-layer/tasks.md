...
- [ ] 2.2 Placeholder allowlist parity: a contract test asserting each prompt-library key's
...
- [ ] 3.1 Implement `register_action_options()` mirroring `register_npc_dialogue`: idempotent
  ...
  `action_code`/`params`).
- [ ] 3.2 Implement the shared total `_evaluate_enriched(parsed, *, fingerprint, affordances,
...
- [ ] 3.3 Implement `generate_action_options(context, client, *, fingerprint)` wrapping
...
- [ ] 3.4 FakeLLM suite (callable matchers on `len(descriptor.messages)` — e.g. `== 2` then
...
- [ ] 5.2 Trace the seven requirements to tests: bounded-context (1.4), prompt assembly (2.2/2.3),
