...
- [x] 1.1 Add a deterministic veil-recipe helper in `world/rules/skill_effects.py` returning the
...
- [x] 1.2 Test: the recipe returns integer values for exactly the five keys, and changing the registry
      band in a synthetic fixture changes the recipe's output.

## 1a. Provenance record

- [x] 1a.1 Add the provenance attribute beside the disguise layer, written only by
      `world/rules/skill_effects.py`; an absent record reads as mundane.
- [x] 1a.2 Register it everywhere `disguised_stats` already is: `_snapshot_entity_state` and
...
- [x] 1a.3 Extend the forbidden-module scan in `world/rules/tests/test_disguise_boundary.py` to
...
- [x] 2.1 Rewrite `_handle_set_disguise` in `world/rules/action.py` to use the recipe: apply to a
...
- [x] 2.2 Re-register `set_disguise` with an empty `requires_event_context`.
- [x] 2.3 Remove the now-dead `status_disguise` context special case from `commands/action.py`.
- [x] 2.4 Tests: self-cast veils then lifts on a second cast; a self-cast over a pre-seeded MUNDANE
...
- [x] 3.1 Test: the shared action preview reports no missing-effect-context failure for the disguise
      skill with an empty `event_context`.
- [x] 3.2 Run the existing disguise boundary tests unchanged and confirm the writer ledger still
      matches (the write stays in `world/rules/skill_effects.py`, so no ledger entry is added).
- [x] 3.3 Test: a rolled-back resolution restores `disguised_stats` AND the provenance record
      byte-equal together, for the apply path and the lift path.
- [x] 3.4 The veil becomes castable in combat for the first time, so verify `disguised_stats` is
...
- [x] 4.1 Put the new tests in `world/rules/tests/test_divine_veil_cast.py`, composed from synthetic
      skill definitions only — no shipped-content skill names, no data-contract tagging.
- [x] 4.2 Annotate with `covers_requirement` using IDs from
      `uv run --locked python -m tools.spec_traceability list`.
- [x] 4.3 Register the module in exactly one shard of `.github/evennia-shards.json`.
...
- [x] 5.2 Run the focused labels `world.rules.tests.test_disguise_boundary` and the command-side cast
      module covering `commands/action.py` to prove the ledger and the cast command still pass.
- [x] 5.3 Run `uv run --locked python -m tools.spec_traceability check` and
      `uv run --locked evennia test --settings test_settings.py --keepdb tests.test_evennia_test_optimization_contract`.
- [x] 5.4 `openspec validate divine-veil-cast-path --strict`.
