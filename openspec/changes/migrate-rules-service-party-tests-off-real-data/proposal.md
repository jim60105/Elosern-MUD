...
party/quest-delivery/issuance/starting-companion tests hardcode introductory_hunt, healing_potion, guild_branch_altoria, and preset keys (yuna_darknight).
...
tagged data-contract tests; party, quest delivery, and companion behavior tests are the remaining debt in this
area.

## What Changes

- Migrate the 7 flagged test files under world/rules/tests/ (party/companion cluster) to the synthetic test-data kit
  (`world/tests/synthetic_data.py`) or file-local synthetic fixtures
  (4 assertion-literal files, 2 setup-literal files, 1 quantity-pinned files).
...
- Remove this area's debt entries from `tools/test_data_freeze.json` and add the area
  closure test in `tests/test_data_independence_rules_party.py`. Its `@covers_requirement` annotation is added in the
...
| file | tier |
| --- | --- |
| `world/rules/tests/test_combat_party.py` | B |
| `world/rules/tests/test_party.py` | B |
| `world/rules/tests/test_party_offline_loop.py` | A |
| `world/rules/tests/test_quest_delivery.py` | A |
| `world/rules/tests/test_quest_issuance.py` | A |
| `world/rules/tests/test_quest_issuer_component.py` | S |
| `world/rules/tests/test_starting_companions.py` | A |
...
- `test-data-independence`: adds the Party, quest delivery, and companion closure requirement — every test file in this
  change's migration manifest passes the test-data lint gate and the manifest's freeze-list
  debt entries are removed. Files under world/rules/tests/ (party/companion cluster) owned by other changes (contract-tagged
  files, sibling migration areas) are unaffected by this requirement.

## Impact

- The 7 listed test files plus the new closure test file `tests/test_data_independence_rules_party.py`.
