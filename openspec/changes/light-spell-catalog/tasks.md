...
- [ ] 1.2 Declare the full 16-node tree and both separate clergy passives using finalized named policies; verify a disposable real-engine scenario exercises all distinct effect compositions rather than comparing registry tuples.
- [ ] 1.3 Remove holy_shield and exclusively obsolete buff/status/preset references without aliases; retain unrelated weapon/passive skills and verify runtime import/selection has no dangling reference.
- [ ] 1.4 Remove light catalog equality/key-count/tier-pair echo tests in-file (test_data_freeze.json is a per-file allowlist; adjust an entry only if its file stops naming shipped content, decided by tools.test_data_lint check output); migrate affected behavioral fixtures to synthetic data and verify traceability remains substantive, not waived.
- [ ] 1.5 Update the authoring guide and both player command documents; verify tests.test_command_docs and actual text/OOB availability for ordinary, gated and locked casts.
- [ ] 1.6 Verify the actual capstone two-parent gate, reverse-edge caps and recovery/ward/mixed/conditional/contact/peak-afterglow paths in a disposable offline engine scenario; observe HP/MP/buffs/phase/practice and remove the scenario only after successful proof.
