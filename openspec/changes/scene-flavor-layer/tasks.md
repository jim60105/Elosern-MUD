...
- [ ] 1.1 Add `prompts/scene_builder.yaml` with `schema_version: 1` and the
...
- [ ] 1.2 Register `PromptSpec("scene_builder.system", "scene_builder.yaml",
      ("scene_sentence", "quest_context", "room_name", "region"))` in
      `world/prompts/registry.py`.
- [ ] 1.3 Add/extend prompt-library tests: the file list includes `scene_builder.yaml`; the
...
- [ ] 2.1 Create `world/ai/scene_flavor.py` following the narrator module pattern: module
...
- [ ] 2.2 Implement `build_scene_flavor_prompt(context)` returning a deterministic (system, user)
...
- [ ] 2.3 Implement the four semantic validators (`flavor_non_empty`, `flavor_bounded_length`,
...
- [ ] 2.4 Implement `register_scene_flavor()` with the narrator's idempotent, atomic registration
      and identity-based uninstall-on-partial-failure pattern, plus `_is_registered` /
      `_require_registered`.
- [ ] 2.5 Implement `generate_scene_flavor(context, client)` (Deferred → `str | None`): rejects an
      explicit `None` client with a named error, requires registration, builds the prompt, runs
      `guarded_call("scene_flavor", client, descriptor)`, maps the sentinel to `None`, and catches
      `PromptUnavailableError` to `None`.

## 3. Tests

- [ ] 3.1 Pure tests (`unittest.TestCase`, FakeLLMClient): valid flavor resolves; digit-containing
...
- [ ] 3.2 Guardrail registration tests: idempotent re-registration; partial-failure rollback leaves
      no half-installed hooks; disabled `scene_builder` profile short-circuits to `None` with no
      network request.
- [ ] 3.3 Update the `world/ai` transport-boundary contract test to cover `scene_flavor.py` (no
      state writer, no typeclass, no live transport, no socket imports).
- [ ] 3.4 Write the substantive behavior tests above first; add `covers_requirement` annotations
...
- [ ] 4.1 Run `uv run --locked python -m world.prompts.validate` to confirm the new prompt file
      validates.
- [ ] 4.2 Run `uv run --locked openspec validate scene-flavor-layer --strict` and confirm all
