...
- [ ] 1.1 Confirm `world/rules/combat_session.py::_scan_friendly_fire`,
...
- [ ] 1.2 Confirm `world/rules/affinity.py::AffinitySource`, `apply_affinity_change`, and
...
- [ ] 1.3 Confirm `world/rules/event_log.py::EventEntry` still has fields `kind`, `actor`, `target`,
  `data`, `text_template` — the contract in design.md Decision 1 depends on this exact shape.
- [ ] 1.4 Confirm `sexual-resist-contest` (`B6a`)'s `ResistVerdict` field names
...
- [ ] 2.1 Add `sexual_forced_penalty` to `affinity.yaml`'s top level, a plain non-negative integer,
...
- [ ] 2.2 Add `sexual_forced_penalty` to `affinity_config.py`'s `_TOP_LEVEL_FIELDS` and to
...
- [ ] 2.3 Confirm `get_config().sexual_forced_penalty` is readable exactly like
  `get_config().friendly_fire_penalty_per_hit` already is.

## 3. `world/rules/affinity.py`: the new source

- [ ] 3.1 Add `SEXUAL_FORCED = "sexual_forced"` to `AffinitySource`.
- [ ] 3.2 Confirm no other logic in `apply_affinity_change` needs to change — `SEXUAL_FORCED` is a
...
- [ ] 4.1 Implement `_scan_sexual_coercion(actor, battlefield, logs) -> tuple[str, ...]`, structured
...
- [ ] 4.2 Widen `_snapshot_party_surfaces` (or a merged sibling snapshot) per design.md Decision 3's
...
- [ ] 4.3 Update `_restore_round_touched`'s guard (`if party_before or members_before:`) so it also
...
- [ ] 5.1 In `submit_player_action`, immediately after the existing
...
- [ ] 5.2 Confirm the combined `notifications` tuple still reaches the caller exactly where the
  existing single-scan `notifications` variable already does — no new return-shape change.

## 6. Tests

- [ ] 6.1 `world/rules/tests/test_combat_session_sexual_coercion.py` (or an appropriately named
...
- [ ] 6.2 One test per spec scenario across both delta specs
...
- [ ] 6.3 Construct synthetic `EventLog`/`EventEntry` fixtures carrying `kind="sexual_resist"` with
...
- [ ] 6.4 A rollback test: force an exception after `_scan_sexual_coercion` applies a penalty but
...
- [ ] 6.5 Two non-companion-NPC tests, both required to actually exercise the bug the rubber-duck
...
- [ ] 6.6 A combined-scan test: one round producing both a qualifying friendly-fire hit and a forced
  coercion entry, asserting both penalties apply within the same commit.
- [ ] 6.7 A non-NPC-target test: a `kind="sexual_resist"` entry whose target resolves to a `Monster`
  or `PlayerCharacter` applies no penalty and does not raise.

## 7. Validation

- [ ] 7.1 Run `uv run --locked python -m compileall -q world` to catch syntax errors early.
- [ ] 7.2 Run the new/extended test module(s) directly via `evennia test` (this touches
...
- [ ] 7.3 Run the full existing `world.rules.tests.test_combat_session` (or equivalent) module to
  ...
  scenario).
- [ ] 7.4 Run `uv run --locked python -m tools.spec_traceability check` and address any reported gap.
- [ ] 7.5 Run `openspec validate sexual-resist-turn-cost --strict` and resolve every finding.
...
- [ ] 7.7 Run `uv run --locked -m unittest discover -s tests -t .` to confirm no repository-wide
  contract regressed.
