...
- `web/webclient-app/AppClient.vue` (and `AppShell.vue` forwarding the slot and the window's `reading-change`) renders `DialogueChoices` in `#choices` exactly while all of these hold:
  ...
  - Exit items come from `store.view.rootMenu`, which is the overview in dialogue (C9b D1); the `exits` section is used.
...
- Spec deltas:
  ...
  - Restated:
    ...
    - desktop-shell "Required desktop surfaces remain visible and usable" (the dialogue choice rows move out of the message window into the centred choice list)
...
- `webclient-desktop-shell`:
  ...
  - MODIFIED "Required desktop surfaces remain visible and usable" (C10b): in dialogue the window carries the plate and the paged line; the choice rows render in the centred choice list.
...
- Dependencies:
  - Archive order: C9a → C9b → C10a → C10b (`webclient-dialogue-stage-actors`) → C10c (this change) → C11 (`webclient-motion-layer`).
