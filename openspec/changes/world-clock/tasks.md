...
- [ ] 1.1 Confirm `world/rules/` holds change 6's `buffs.py` (`tick_buffs`, `entity.buffs`), change 7's
      ...
      files.
- [ ] 1.2 Confirm the exact import paths and public names for `world.rules.buffs.{tick_buffs}`,
      ...
      step.
- [ ] 1.3 Confirm change 9's `run_round()` landed implementation genuinely calls `tick_buffs()`/
...
- [ ] 2.1 Author `seconds_per_hour: 3600`, `hours_per_day: 24`, `days_per_season: 90`,
...
- [ ] 2.2 Author `command_defaults: {move: 30, converse: 60, cast: 6}` per design.md D-9, with an
...
- [ ] 2.3 Author `max_settlement_quanta: 4320` and `max_sleep_seconds: 43200` per design.md D-4/D-7,
...
- [ ] 2.4 Implement a small loader in `world/rules/clock.py` (`CLOCK_YAML = yaml.safe_load(...)`),
      mirroring change 9's `COMBAT_YAML` loader pattern exactly.

## 3. WorldDateTime and the calendar (`world/rules/clock.py`)

- [ ] 3.1 Implement `WorldDateTime` (frozen dataclass: `year`, `season_index`, `day_in_season`,
      ...
      hardcoded literal.
- [ ] 3.2 Implement `WorldDateTime.season_name` reading `CLOCK_YAML["season_names"][season_index]`.
- [ ] 3.3 Test: tick `0` yields `year=0, season_index=0, day_in_season=1, hour=0, minute=0, second=0`;
      ...
      day_in_season=1`.
- [ ] 3.4 Implement `seconds_until_daypart(calendar: WorldDateTime, daypart_name: str) -> int` per
...
- [ ] 3.5 Test `seconds_until_daypart()`: a future-today daypart computes the same-day interval; a
...
- [ ] 4.1 Implement `AdvanceSource` (`StrEnum`: `COMMAND`, `COMBAT`, `SKIP`).
- [ ] 4.2 Implement `WorldClock` (dataclass: `tick: int`) with a `calendar` property returning
      `WorldDateTime.from_tick(self.tick)` — no other persisted field.
- [ ] 4.3 Implement `WorldClockScript(DefaultScript)` per design.md D-1: `at_script_creation()` seeds
...
- [ ] 4.4 Confirm (grep-based check) that `world/rules/clock.py`, `world/rules/skip_safety.py`, and
      `commands/skip.py` contain none of the literal tokens `datetime.now`, `time.time`, `utcnow`, or
      `at_repeat`.
- [ ] 4.5 Test: two `WorldClock` instances constructed with the identical `tick` produce field-for-
...
- [ ] 5.1 Implement `_settle_gauge_regen(entities, elapsed_seconds: int) -> None` per design.md D-4:
...
- [ ] 5.2 Confirm/flag: verify against the installed Evennia 6.1.0 `GaugeTrait` whether its own
      `.value` getter performs any implicit, wall-clock-timestamp-based regen; if so, document the
      override/bypass needed so `_settle_gauge_regen()`'s explicit write remains the sole source of
      regen (design.md D-4's flagged risk).
- [ ] 5.3 Test: an 8-hour (`28800`) elapsed value applied to a `hp` gauge with `rate=2` below max
...
- [ ] 6.1 Implement `SETTLEMENT_QUANTUM_SECONDS`, computed once at import time as
...
- [ ] 6.2 Implement `_has_settlement_work(entity) -> bool` per design.md D-4: `True` if
...
- [ ] 6.3 Implement `_settle_buffs_and_decay(entities, elapsed_seconds: int) -> None` per design.md
      D-4: loops `elapsed_seconds // SETTLEMENT_QUANTUM_SECONDS` times, calling `tick_buffs(entity)`
      and `decay_tick(entity, SETTLEMENT_QUANTUM_SECONDS)` for every entity each iteration; breaks the
      moment `_has_settlement_work()` is `False` for every entity in scope; hard-stops at
      `CLOCK_YAML["max_settlement_quanta"]` iterations regardless.
- [ ] 6.4 Test: a fully-settled entity (no buffs, every decay field at floor) exits after exactly one
      quantum check for a `28800`-second call — `tick_buffs`/`decay_tick` each called exactly once, not
      2,880 times (mock/call-count assertion).
- [ ] 6.5 Test: an entity with a `poisoned` buff (duration 300s) active at the start of a `28800`-second
      call ticks correctly through the buff's own duration at the derived quantum size, then exits
      early for the remainder — assert the exact call count matches `300 // SETTLEMENT_QUANTUM_SECONDS`
      plus the one trailing all-settled check, not `28800 // SETTLEMENT_QUANTUM_SECONDS`.
- [ ] 6.6 Test the startup assertion: monkeypatch a `DECAY_CONFIG`/`buffs.yaml`-equivalent interval not
      divisible by the current GCD and assert the assertion raises, naming the offending value.
- [ ] 6.7 Test the defensive cap: a fixture engineered so `_has_settlement_work()` never returns
...
- [ ] 8.1 Implement `ScheduledEvent` (frozen dataclass: `kind: str`, `due_tick: int`, `payload: dict`)
      per design.md D-5, matching change 8's `EventEntry` key-only, JSON-compatible discipline.
- [ ] 8.2 Implement `register_event_source(kind: str, source: Callable[[int, int], list[
      ScheduledEvent]]) -> None` and the internal `_EVENT_SOURCES` registry.
- [ ] 8.3 Implement `_settle_boundary_stages(start_tick, end_tick) -> list[ScheduledEvent]` per
...
- [ ] 8.4 Test: a single day-boundary crossing calls `reset_daily_counters()` exactly once per entity;
...
- [ ] 8.5 Test: before any `register_event_source()` call, `advance()` completes successfully with zero
      events for all four declared-seam kinds; after registering a synthetic source for a made-up
      `kind`, its `ScheduledEvent`s appear in `advance()`'s returned list when a boundary it reports for
      is crossed.
- [ ] 8.6 Test `ScheduledEvent.payload` contains no live entity/`Battlefield` reference for any event
      this change itself produces (the `daily_reset`-kind event).

## 9. advance() itself (`world/rules/clock.py`)

- [ ] 9.1 Implement `WorldClock.advance(seconds: int, source: AdvanceSource, entities: Iterable[
      ...
      caller passed.
- [ ] 9.2 Test: `advance()` only mutates state for entities in the supplied `entities` iterable — a
      separately-constructed `LivingEntity` with an active buff, not passed in, is unaffected.
- [ ] 9.3 Test reproducibility (hard requirement 4): construct two independent `WorldClock` + entity-
...
- [ ] 9.4 Test a serialize-reload-replay cycle: capture `(tick, entity attribute values)`, "reload" by
...
- [ ] 10.1 Implement `settle_combat_result(result, entities) -> list[ScheduledEvent]` per design.md
      D-10: calls `WORLD_CLOCK.advance(result.total_seconds, AdvanceSource.COMBAT, entities)`, accepting
      either a `BattleResult` or an `OverwhelmResult`.
- [ ] 10.2 Test `settle_combat_result()` with a synthetic `BattleResult`/`OverwhelmResult` (a minimal
...
- [ ] 10.3 Edit `commands/action.py::CmdCast` (change 8's already-landed implementation file — not its
      OpenSpec artifacts) to call `WORLD_CLOCK.advance(result.time_cost_seconds,
...
- [ ] 10.4 Test: a successful `CmdCast` invocation advances `tick` by exactly the resolved
      `time_cost_seconds`; a rejected invocation leaves `tick` unchanged.
- [ ] 10.5 Confirm (via `git diff`) that this task's edit to `commands/action.py` is the only change to
...
- [ ] 11.7 Test the safe case: no `Battlefield` membership and no living `Monster` present returns
      `None`.
- [ ] 11.8 Confirm `SkipRejectReason`'s definition carries no "allowed seconds"/partial-duration field
...
- [ ] 12.1 Implement `_parse_duration(text: str) -> int` (e.g. `"1h"` → `3600`, `"30m"` → `1800`),
      raising a named parse error for an unrecognized shape.
- [ ] 12.2 Implement `CmdRest` per design.md D-7: parses its argument via `_parse_duration()`; calls
      `evaluate_skip_safety()` first — on a non-`None` result, reports the rejection and returns without
      calling `advance()` or `_parse_duration()`'s result being used; otherwise calls
      `WORLD_CLOCK.advance(seconds, AdvanceSource.SKIP, entities=[self.caller])` and renders the result.
- [ ] 12.3 Implement `_seconds_to_full_regen(entity) -> int` per design.md D-7: computes
...
- [ ] 12.4 Implement `CmdSleep` per design.md D-7: takes no argument; safety-gated identically to
      `CmdRest`; computes its duration via `_seconds_to_full_regen()`; advances and renders.
- [ ] 12.5 Implement `CmdWaitUntil` per design.md D-7: parses `"until <daypart>"`; safety-gated
      identically; computes its duration via `seconds_until_daypart()` (task 3.4); advances and renders.
- [ ] 12.6 Implement `_render_skip_summary(seconds, events) -> str`: reports the elapsed duration and
      any due `ScheduledEvent`s (in particular a `daily_reset` event) in plain text, calling no LLM.
- [ ] 12.7 Test `CmdRest`: `"rest 1h"` on a safe actor advances by exactly `3600`; an unparseable
...
- [ ] 12.8 Test `CmdSleep`: duration matches the largest of the three computed gauge-regen times when
...
- [ ] 12.9 Test `CmdWaitUntil`: a future-today daypart computes the same-day remainder; a
...
- [ ] 12.10 Test `_render_skip_summary()`: a skip crossing a day boundary reports the reset; a skip with
      an empty `ScheduledEvent` list still reports the elapsed duration with no error.

## 13. Verification

- [ ] 13.1 Run the full `world/rules/tests/` suite added by this change and confirm every test passes.
- [ ] 13.2 Confirm `world/rules/clock.py`, `world/rules/skip_safety.py`, and `commands/skip.py` contain
      no reference to `world/ai/` and no LLM call anywhere (source-scan, mirroring change 8's own
      no-Narrator discipline).
- [ ] 13.3 Confirm this change modifies no file authored by any earlier change except the single,
      documented `commands/action.py::CmdCast` edit (task 10.3) — `git diff --stat` against the
      pre-change tree shows only new files under `world/rules/clock.py`,
      `world/rules/skip_safety.py`, `world/rules/rulebook/clock.yaml`, `commands/skip.py`,
      `world/rules/tests/`, plus that one modified line in `commands/action.py`.
- [ ] 13.4 Confirm change 6's, change 7's, change 9's, and change 10's own test suites still pass
      unmodified — this change never edits `buffs.py`, `sexual_state.py`, `combat.py`, `overwhelm.py`,
      or any of their rulebook YAML files.
