...
- [ ] 1.1 Add `capital_adornments` (王都飾品) to the assortment registry holding exactly the
...
- [ ] 1.2 Remove those seventeen keys from `general_sundries`, leaving eighteen: the tools, the
...
- [ ] 3.1 Add 聖潔王都首飾坊 off 市場街 `(2,3)`, host 艾蓮娜·鴉丘, title 聖潔王都首飾坊主,
...
- [ ] 3.2 Add 聖潔王都鍊金坊 off 東市 `(5,3)`, host 希碧拉·灰沼, title 聖潔王都鍊金坊主,
  human, female, `merchant`, referencing `capital_remedies`.
- [ ] 3.2a Author the two places as `PlaceKind.JEWELLER` and `PlaceKind.ALCHEMIST`.
- [ ] 3.3 Add both `shops:` rows to the commerce rulebook with opening hours matching the
  other capital shops.
- [ ] 3.4 Author a `dialogue_key` and a dialogue table for each of the two hosts. The
...
- [ ] 4.1 Pin the partition rule: the capital's accessory-slot keys equal the adornments
  bundle's keys exactly, computed from both sides rather than asserted against a literal list.
- [ ] 4.2 Pin that the union of every capital place's goods is unchanged by this change — the
  capital sells neither more nor less, only from different counters.
- [ ] 4.3 Both new shops are reachable, open and tradeable through the ordinary command path,
  with the stock listing naming each shop's own room.

## 5. The lore document

- [ ] 5.1 Line 206: 「奧莉薇亞·晶琢」 → 艾蓮娜·鴉丘. Line 224: 「賽菲拉·藥缽」 → 希碧拉·灰沼.
- [ ] 5.2 Line 146 promises this split is still pending. Rewrite it to record that it landed,
  and that 受洗聖水 now sits with the sundries only until the sanctuary opens.
- [ ] 5.3 Run the lore, rules-commerce and guild-economy-sync suites, plus the test-data lint
