...
- [ ] 1.3 Validate the record: unknown settlement, unknown race, unknown sex, a subrace whose own
...
- [ ] 1.4 Add `world/lore/settlements/places_altoria.py` holding the capital's guild hall and
...
- [ ] 1.5 Register the new registries in `world/lore/sync.py::_ALL_REGISTRIES`.
...
- [ ] 2.3 Verify the cross-registry authored-name uniqueness rule still fires on a planted
...
- [ ] 3.1 Verify the derived roster rows equal the removed YAML rows field for field — same names,
