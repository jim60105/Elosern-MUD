...
- [ ] 1.1 In `world/rules/progression.py`, delete `MAGIC_XP_PER_LEVEL`,
...
- [ ] 1.2 In `world/rules/rulebook/progression.yaml`, delete the `magic_xp_per_level`,
  `study_base_xp_per_hour`, and `combat_kill_xp` keys; keep whatever practice keys exist.
- [ ] 1.3 Grep-audit the repo for `magic_xp`, `accrue_magic_study`,
  `grant_combat_kill_xp`, `effective_growth_multiplier` — zero non-test hits after §3.

## 2. World-clock stage rename + placeholder

- [ ] 2.1 In `world/rules/clock.py`, rename the `magic_study` stage entry to
...
- [ ] 2.2 Drop `("magic_xp", None)` from the advance snapshot registry.

## 3. Snapshot / restore surfaces

- [ ] 3.1 `world/rules/action.py`: remove the `magic_xp` snapshot/restore pair from the
...
- [ ] 3.2 `world/rules/cast_settlement.py`: drop `("magic_xp", None)` from
  `_SETTLEMENT_ATTRS`.
- [ ] 3.3 `world/rules/upkeep.py`: keep `source_pk` attribution, defeat EventLogs, and
  quest planner effects; delete `grant_combat_kill_xp()` staging entirely.
- [ ] 3.4 `world/rules/character_creation.py`: remove `"magic_xp"` from the activation
  attribute list and the `"magic_xp": 0` initial write.

## 4. Skills: effect class + registry rows

- [ ] 4.1 `world/skills/effects.py`: delete `ElementMasteryRankEffect` and its
  `element_mastery_rank` parse branch (prefix now fails closed at parse).
- [ ] 4.2 `world/skills/registry.py`: switch every `<element>_mastery` row's
...
- [ ] 5.1 `world/rules/progression.py` (with §1): `can_cast_spell_tier`,
  `can_cast_skill`, `_element_effective_magic_level` deleted.
- [ ] 5.2 `world/rules/action.py`: delete the elemental tier rejection from
  `preflight`/`resolve` and the spell-tier check from the shared preview + submission
  revalidation + `build_combat_view` descriptors.
- [ ] 5.3 `world/rules/combat.py` `default_attack_policy`: propose the first affordable
  resolver-backed damage skill (no tier gating).
- [ ] 5.4 `world/rules/element_affinity` docs/docstrings: multiplier survives; delete
  element-effective-level wording.

## 6. Test re-pinning

- [ ] 6.1 `world/rules/tests/test_progression.py`: delete engine tests (study XP, kill
...
- [ ] 6.2 `test_upkeep_settlement.py`: XP assertions become no-progression-write
  assertions; defeat-entry/quest assertions unchanged.
- [ ] 6.3 `test_guild_exams.py`, `test_combat_session_recovery.py`,
  `test_character_creation.py`: drop `db.magic_xp` pins per §1–§4.
- [ ] 6.4 Webclient cast fixtures: delete the `traits.magic_power.base = 30` override
...
- [ ] 6.5 No new test modules → `.github/evennia-shards.json` untouched.

## Verification

- [ ] V1 `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings test_settings.py --keepdb world.rules world.skills world.imports typeclasses commands web.webclient`
...
- [ ] V4 `openspec validate magic-xp-engine-retirement --strict`
- [ ] V5 `git diff --check`
