...
- **THEN** each key is present with `SkillKind.ACTIVE`, `element=ELEMENT_REGISTRY["wind"]`, the
...
- **WHEN** an entity at magic level 15 with no `wind_mastery` skill attempts to cast `flight`
...
- **THEN** `can_cast_spell_tier` (from `element-mastery-cast-gate`) rejects the first entity's cast and
  permits the second entity's cast, for every 術師-tier 風 spell (`flight`, `tornado_blade`)
