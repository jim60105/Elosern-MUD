...
registry defined by this change (`RACE_REGISTRY`, `SUBRACE_REGISTRY`, `ELEMENT_REGISTRY`,
`MAGIC_TIER_REGISTRY`, `RANK_TITLE_REGISTRY`, `NATION_REGISTRY`, `GUILD_RANK_REGISTRY`,
`MONSTER_TIER_REGISTRY`, `ANCHOR_REGISTRY`, `PRICE_TABLE`), ensures a corresponding persistent DB
record exists, addressable by a key derived from the registry's category and the entry's own
`key`.
