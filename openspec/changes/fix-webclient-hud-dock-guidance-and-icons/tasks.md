...
- [ ] 1.1 In `web/webclient-app/components/ActionDock.vue`, add a `.visually-hidden` rule (the same
      ...
      local rule is a no-op. Apply the class to `.action-dock__description`.
- [ ] 1.2 Reword the shared legend string in both `ActionDock.vue`'s `.action-dock__description` and
...
- [ ] 1.3 Update all four literal-text assertions in `web/webclient-app/tests/action/action_dock.test.js`
      (lines 76, 83, 193, 198) to the new wording.
- [ ] 1.4 Update the Playwright keyword-substring assertion in `web/tests/browser/test_browser_shell.py`
      (~line 755: replace the `"/ 開啟指令"` keyword with `"/ 聚焦指令列"`).
- [ ] 1.5 Add a Vitest assertion (in `action_dock.test.js` or a new focused spec) that
      `.action-dock__description` is not actually visible — assert its rendered state (e.g. computed
      `clip`/width/height, or a Playwright `is_visible()`-equivalent check), not merely that it carries
      the `visually-hidden` class name, while `.dock-tab-bar__hint` is visible, whenever the tab bar
      renders.

## 2. Dock and combat-root tab glyphs matched to the redesign

- [ ] 2.1 In `web/webclient-app/components/dock-icons.js`, replace the `d` path values for `move`,
...
- [ ] 2.2 In `web/webclient-app/components/DockTabBar.vue`, add `stroke-linecap="round"
      stroke-linejoin="round"` to the tab icon `<path>` element so multi-subpath glyphs (`move`,
      `attack`, `flee`) and the compound `look` glyph render with the reference's rounded caps/joins.
- [ ] 2.3 `grep -rn` each of the ten *old* `d` string values across `web/webclient-app/tests/` and
...
- [ ] 2.4 Add a new Vitest unit test file for `dock-icons.js` (none currently exists) asserting
      `glyphPath(key)` returns the new literal string for each of the ten replaced keys, and still
      returns the prior unchanged value for at least one untouched key (e.g. `character`).
- [ ] 2.5 Visually verify the new glyphs render correctly at 16×16 in the `Action/ActionDock` and
...
- [ ] 3.1 In `web/webclient-app/components/ActionDock.vue`'s `.action-dock` rule, replace
...
- [ ] 3.2 Visually verify the new background/shadow in the `Action/ActionDock` Storybook story and
...
- [ ] 4.1 Confirm `openspec validate fix-webclient-hud-dock-guidance-and-icons --strict` passes against
      the delta spec in `specs/webclient-contextual-hud/spec.md`.
- [ ] 4.2 Run the focused test slice: `npm test -- action_dock` (Vitest) and the `test_browser_shell.py`
      shell-chrome test class (Playwright/managed browser), plus the new `dock-icons` unit test.
- [ ] 4.3 Re-check the live client (`podman compose`, `http://localhost:4001/webclient/`) with
