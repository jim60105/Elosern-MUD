...
background, and affinity elements (trimmed to the incoming race's input bound) - into the local
unsent form state and record that revision; an absent transient-fill field SHALL leave the local
value untouched. When the fresh apply completes while the player is on the concept tab, the form
SHALL switch to the custom tab automatically, and the client SHALL surface exactly one toast
naming the applied proposal (written by the action-feedback result slice, not the form); a panel
