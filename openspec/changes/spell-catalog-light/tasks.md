...
- [ ] 2.1 Add 10 new spell entries to `SKILL_REGISTRY` in `world/skills/registry.py` (via the `_elemental_spells` builder) for: `heal`, `light_arrow`, `purify`, `mass_heal`, `advanced_heal`, `holy_shield`, `holy_radiance`, `revival_light`, `goddess_blessing`, `heavens_judgment_light`.
- [ ] 2.2 Group the ten entries in registry-literal order as five tier-labeled pairs (學徒/術師/大師/賢者/主宰) with a `# 光 — <tier>` comment above each pair, matching this change's `design.md` Decisions section.
...
- [ ] 3.1 Add a `light_holy_shield` row to `world/rules/rulebook/buffs.yaml`: shield buff (defense bounds ceiling), applied by `holy_shield`.
- [ ] 3.2 Add a `light_blessing` row to `world/rules/rulebook/buffs.yaml`: party buff (rate/bounds-shaped multi-stat boost), applied by `goddess_blessing`.
...
- [ ] 4.1 Extend the registry-level parametrized round-trip test (design doc §9) to cover all ten 光 spells' `effects` strings.
- [ ] 4.2 Add tier-boundary tests for 光's four tier thresholds (magic level 15/16, 30/31, 70/71, 90/91) confirming `can_cast_spell_tier` rejects without `light_mastery` and permits with it, per this change's `specs/skill-registry/spec.md`.
...
- [ ] 5.1 N/A — 光 has no pre-existing anchor skill to recost; all ten entries are new.
