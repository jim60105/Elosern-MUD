...
- [ ] 1.1 Create the `world/lore/settlements/` package with `assortments.py` holding a frozen
...
- [ ] 1.2 Add the four capital assortments — weapons, armour, food, remaining sundries — covering
...
- [ ] 2.1 Create `world/rules/rulebook/commerce.yaml` with an `assortments:` section carrying each
...
- [ ] 2.2 Remove the `shops:` block from `world/rules/rulebook/guild_economy.yaml`, leaving merit
  thresholds, exam profiles, quest rewards and the service-host roster in place. Verify the guild
  catalog still loads.

## 3. The join

- [ ] 3.1 Split `validate_shop_configs` in `world/rules/guild_config.py` into assortment validation
...
- [ ] 3.2 Reject a shop referencing an unknown assortment, and reject an item key appearing in two
  of one shop's referenced assortments, each naming the shop and the offender.
- [ ] 3.2a Reject an assortment containing a keepsake (`relic`) band item at any price, naming the
...
- [ ] 3.3 Re-key the shops-completeness check on `shop_key` and delete
...
- [ ] 3.4 Replace `ShopDefinition.offered_item_keys` with `assortment_keys` in
...
- [ ] 4.1 Update the shop factories in `world/tests/synthetic_data/` to build assortment-shaped
  rows, and verify with
  `uv run --locked evennia test --settings test_settings.py --keepdb world.lore.tests.test_shops`.
- [ ] 4.2 Verify a purchase and a sale against the capital store settle exactly as before with
