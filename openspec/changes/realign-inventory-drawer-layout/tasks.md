...
- [ ] 1.1 In `EquipmentDoll.vue`, wrap the slot grid in a new
...
- [ ] 1.2 Remove the per-slot `.equipment-doll__slot-item` name line (the name
  ...
  `data-testid`.
- [ ] 1.3 Change the title element's text from `裝備人偶` to `裝備` and restyle
...
- [ ] 2.1 In `InventoryPanel.vue`, drop the `.inventory-panel`
  ...
  `services.inventory.wallet`; render the wallet row only for a committed
  non-negative integer wallet.
- [ ] 2.2 Keep the tile grid, ceiling note, inspector, selection/a11y behavior,
  and all existing testids byte-identical; do not add the mock's
  排序/篩選/找尋 pills.

## 3. Tests with the behavior

- [ ] 3.1 Update `web/webclient-app/tests/data/equipment_doll.test.js`:
  description-column grouping (incl. accessory group, unrecognised slot,
  duplicate rows) and `裝備人偶` absence / `裝備` + tag presence.
- [ ] 3.1a Re-express the wallet-singleton contract:
...
- [ ] 3.2 Update `web/webclient-app/tests/world/inventory_panel.test.js`:
  ...
  `npm test`.
- [ ] 3.3 Update managed browser evidence: add assertions for the equipment
...
- [ ] 4.1 Add variant exports to `stories/Data/EquipmentDoll.stories.js`
...
- [ ] 4.2 Run `npm run showcase-coverage` and `npm run build-storybook`; verify
...
- [ ] 5.1 `openspec validate realign-inventory-drawer-layout --strict`.
- [ ] 5.2 `uv run --locked python -m tools.spec_traceability check`; `git diff
  --check` clean.
