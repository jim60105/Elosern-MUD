...
- [x] 0.1 Re-read the live `parse_effect` `damage` branch, `_parse_damage_effect`, and both its call
...
- [x] 1.1 Add the school-membership check to `parse_effect`'s `damage` branch
...
- [x] 1.2 Re-implement `_parse_damage_effect` in `world/rules/combat.py` as a thin wrapper: call
  ...
  own.
- [x] 1.3 Update `_handle_damage`'s one call site (`_, school = _parse_damage_effect(effect_id)` →
...
- [x] 1.4 Update `_damage_school`'s one call site in `world/rules/monster_behaviour.py`
...
- [x] 2.1 Extend `world/skills/tests/test_effects.py`'s `test_malformed_damage_raises` with a
...
- [x] 2.2 Add a small architectural test (`world/rules/tests/`, colocated with existing `combat.py`
  ...
  contract today.
- [x] 2.3 Add one real-resolution regression test resolving a synthetic elementless
...
- [x] 3.1 Run the focused labels and gates above and record the results. Pass `MUD_TEST_SETTINGS=1`
  through the tool environment, never as an inline shell prefix. No full suite, no browser run, no
  aggregate-coverage run.
- [x] 3.2 Annotate the new tests with the canonical requirement IDs from
