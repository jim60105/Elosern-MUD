...
- [ ] 1.1 Confirm change 12 (`map-anchor-grid`) is implemented: `typeclasses/rooms.py::GridRoom`/
      `AnchorRoom` exist, `world/maps/altoria_capital.py::ALTORIA_CAPITAL_MAP_DATA` exists, and
      `sync_grid()` spawns the thirteen-room sample city. Do not proceed past task group 4 if this is
      not true yet.
- [ ] 1.2 Confirm change 13 (`map-wilderness`) is implemented: `typeclasses/exits.py::
...
- [ ] 1.3 Confirm no module named `world/rules/movement.py` exists yet, and that
      `typeclasses/exits.py::Exit` has no `MovementCostMixin` in its bases and no class named
      `CostedXYZExit` exists.
- [ ] 1.4 Re-verify (do not merely trust design.md) the two Evennia facts this change's whole design
...
- [ ] 2.1 Create `world/rules/movement.py`. Implement `charge_movement(traversing_object, cost_key:
...
- [ ] 2.2 Test: `charge_movement(player_character, "move")` advances `get_world_clock().tick` by
      exactly `CLOCK_YAML["command_defaults"]["move"]`.
- [ ] 2.3 Test: `charge_movement(npc, "move")` (an `NPC`-typeclassed, non-`PlayerCharacter` traverser)
      leaves the clock unchanged.
- [ ] 2.4 Test: `charge_movement()` always passes `AdvanceSource.COMMAND` to the underlying
      `WorldClock.advance()` call, for any registered `cost_key`.
- [ ] 2.5 Test: `charge_movement(player_character, "wilderness_move")` advances the clock by
...
- [ ] 3.1 Add `MovementCostMixin` per design.md D-2/D-3: a class attribute `movement_cost_key: str =
...
- [ ] 3.2 Change `Exit`'s base classes from `(ObjectParent, DefaultExit)` to `(MovementCostMixin,
      ObjectParent, DefaultExit)`. Do not change any other line of `Exit`'s body (it remains
      otherwise empty).
- [ ] 3.3 Add `CostedXYZExit(MovementCostMixin, evennia.contrib.grid.xyzgrid.xyzroom.XYZExit)` with no
      additional members beyond what the mixin and `XYZExit` already provide.
- [ ] 3.4 Test: a `PlayerCharacter` traversing a plain `Exit` instance advances the clock by exactly
      `CLOCK_YAML["command_defaults"]["move"]`.
- [ ] 3.5 Test: a locked `Exit` (`locks.add("traverse:false()")`) traversal attempt leaves both the
      traverser's location and the clock unchanged.
- [ ] 3.6 Test: a traverser whose `at_pre_move` returns `False` leaves both the traverser's location
      and the clock unchanged when attempting to traverse an `Exit` instance.
- [ ] 3.7 Test: `CostedXYZExit.create(key=..., location=..., destination=...)` produces a working
      ...
      `CLOCK_YAML["command_defaults"]["move"]`.
- [ ] 3.8 Test: an `NPC`-typeclassed traverser successfully traverses a `MovementCostMixin`-carrying
      exit (its location changes) but the clock does not advance.
- [ ] 3.9 Test: `traversing_object.move_to(destination, move_type="teleport")`, called directly with
      no `Exit` involved, leaves the clock unchanged even though the move succeeds.
- [ ] 3.10 Test: `traversing_object.move_to(destination, quiet=True)`, called directly (the exact
...
- [ ] 4.1 Add exactly one entry to `ALTORIA_CAPITAL_MAP_DATA["prototypes"]`: `("*", "*", "*"): {
...
- [ ] 4.2 Test: against a **fresh, empty** test database, after `sync_grid()` runs once, every one of
...
- [ ] 4.2b Test (retype path, scoped per design.md D-4's own verified finding — do not assume this
...
- [ ] 4.2c Confirm (development-environment task, not a `pytest`-style test): the implementer's own
      ...
      migration script.
- [ ] 4.3 Test: the sample city still has exactly thirteen rooms and twelve exits in the identical
...
- [ ] 4.4 Test: a `PlayerCharacter` successfully traversing any one of the twelve intra-city exits
      (spawned fresh, per task 4.2) advances the clock by exactly `CLOCK_YAML["command_defaults"][
      "move"]`.
- [ ] 4.5 Test: the Limbo↔South-Gate bridging `Exit` pair (change 12 D-7, `typeclasses.exits.Exit`,
...
- [ ] 5.1 In `WildernessGateExit.at_traverse`'s successful branch, replace the inline
...
- [ ] 5.2 In `WildernessReturnExit.at_traverse`, replace **both** inline `get_world_clock().advance(...)`
      ...
      logic.
- [ ] 5.3 Re-run change 13's own `wilderness-gateway` test suite unmodified (entry cost, unsuccessful
...
- [ ] 5.4 Test (new): `WildernessGateExit.at_traverse`'s source, inspected directly, calls
...
- [ ] 6.1 Confirm `openspec/changes/map-wilderness/design.md` has already been updated (this proposal
...
- [ ] 7.1 Confirm `docs/superpowers/specs/2026-07-29-ai-mud-engine-design.md` §11's Phase 3 table has
...
- [ ] 8.1 Write one `EvenniaTest`-based integration test that: runs `sync_all()`, `sync_grid()`,
...
- [ ] 8.2 Test: within the same integration test, confirm at least one deliberately failed traversal
...
- [ ] 9.1 Add `charge_movement`, `MovementCostMixin`, `CostedXYZExit` to `tests/
...
- [ ] 9.2 Run `world/rules/tests/test_clock.py`, `commands/tests/test_cmd_cast.py` (or equivalent),
      change 12's own `world/maps/tests/` suite, and change 13's own `typeclasses/tests/`/`world/maps/
      tests/` suites, and confirm every one still passes unmodified — proving this change's edits are
      additive, not regressions.

## 10. Verification

- [ ] 10.1 Run the full test suite added or touched by this change and confirm every test passes.
- [ ] 10.2 Confirm (via `git diff`) that every edit to an already-landed file (`typeclasses/exits.py`)
      is additive (new classes, one base-class-list edit to `Exit`) and removes no existing behavior.
- [ ] 10.3 Confirm no file added or edited by this change contains a reference to `world/ai/` or an
      LLM call.
- [ ] 10.4 Confirm `rulebook/clock.yaml`'s `converse` entry is unchanged and unconsumed by this
      change.
- [ ] 10.5 Delete or leave in place (per reviewer preference) the scratch verification probe
      `tmp/probe_movement_clock/`; it is not part of the shipped suite either way.
- [ ] 10.6 Run `openspec validate map-movement-clock --strict` and `openspec validate --all --strict`
