...
Fix note (apply pass 2, test-proven): the ownership leaf `world/rules/skill_ownership.py::owns_stored_skill` initially matched only `list`/`tuple`/`set`/`frozenset` values and silently returned `False` for every real entity — Evennia deserializes stored list attributes as `_SaverList`, a `Sequence` that is NOT a `list` subclass (the same note `items/reads.py` and `progression/_scaling.py` record). The committed trickle and oath branches therefore never fired on real entities. The suite's entity-level behavior tests exposed it; the fix accepts any non-string `Sequence` plus the plain set/tuple forms and keeps every other stored shape failing closed.

**D2b — the trickle step, once per advance, after the quantum loop**: `saintess_trickle_step(entity, resulting_tick)` in `world/rules/pleasure.py`, called from `advance`'s settlement staging as its own step AFTER `_settle_buffs_and_decay`, for non-COMBAT sources only (matching the loop's own combat posture — combat already floods arousal through stimulus/reactions), only when `seconds > 0` (a zero-second advance mutates no world state and must not fluctuate the gauge), and exactly once per `advance()` call (NOT per quantum — the quantum loop is a decay/buff cadence; the idle flavor is a per-advance step). Every write goes through `apply_pleasure_gain(..., stimulus=False)`: the sanctioned writer grew an explicit non-stimulus policy (duck fix F5) that keeps the gauge write and the wetness-on-band-up cascade but skips BOTH climax branches — otherwise a holder legitimately parked at 接近 after a 極限 spike (decay never clears that phase) would be promoted into 進行中 by a cosmetic +1 at pleasure 15, i.e. the idle trickle would autonomously open a climax below the 85 gate. Per call:
1. pleasure < 15 → `apply_pleasure_gain(entity, 15 − pleasure, stimulus=False)` (pin-up; covers grant-time seeding and any sub-floor reduction).
2. 15 ≤ pleasure ≤ 59 → deterministic ±1: direction = `crc32(f"{entity_id}:{resulting_world_tick}") % 2` — a stateless hash of the committed-world-tick AND the entity, so a rolled-back and retried advance recomputes the identical draw (no RNG consumed; no dice involved). Raw tick PARITY was the original plan and is refuted by shipped data (duck fix F6): every shipped non-combat advance duration is even (move 30, cast 6, item 6, skip 9000), so `resulting_tick % 2` never changes and the draw would freeze per entity — the hash avalanche over the whole tick is what makes successive advances alternate. Result clamped to [15, 59]; a draw whose clamped delta collapses to zero (− at the floor, + at the ceiling) issues no writer call at all.
3. pleasure ≥ 60 → no-op; ordinary decay owns the descent, the step re-arms on band re-entry.

A holder resting at the floor therefore stays inside {15, 16} or drifts one step up per draw (level never leaves 微興奮) — no stateless draw can promise strict 15↔16 alternation, and the binding guarantee is 「never below 15 after any settlement, ±1 per advance, bounded to the band, visibly moving」, not a frozen 15. Why the step is NOT inside `_has_settlement_work`'s quantum loop: (a) an idle holder at pleasure 0 has no other pending work and the loop early-exits before reaching any post-decay hook — the pin-up must run regardless, so the step lives outside that guard; (b) per-quantum firing would draw up to 180 steps in one sleep-length advance.
