...
- [ ] 1.1 Extend `NPC_DIALOGUE_OUTPUT_SCHEMA` in `world/ai/npc_dialogue.py` so `adjust_relation`
...
- [ ] 1.2 Add the per-kind semantic validator `_validate_relation_payload` (exactly one field
...
- [ ] 1.3 Add the semantic validator `_validate_no_affinity_leak`: when an affinity context was
      injected, a reply whose speech contains the affinity value or the cap as a decimal integer
      substring is a validation failure; register it in `_VALIDATORS`.
- [ ] 1.4 Add schema and validator tests: valid `{"kind": "adjust_relation", "delta": 3}` passes;
...
- [ ] 2.1 In `world/rules/npc_intents.py`, replace the `adjust_relation` forward-declared rejection
...
- [ ] 2.2 Extend `IntentOutcome` with a `delta_used: int | None` field; map the writer outcome by
...
- [ ] 2.3 Add applier tests: full-amount delta reports applied with the amount; partial-budget
...
- [ ] 3.1 Extend `build_npc_dialogue_prompt` with an optional `affinity_context` argument,
...
- [ ] 3.2 Update `LLMNPC.at_talked_to` in `typeclasses/npcs.py` to read the NPC's own affinity
...
- [ ] 3.3 Add prompt and seam tests: the block carries the true value, cap, and stage; a recordless
...
- [ ] 3.4 Update the `npc_dialogue.system` template in `prompts/npc_dialogue.yaml` (data) to
...
- [ ] 4.1 Add a `FakeLLMClient` replay test: an NPC dialogue reply carrying
      ...
      the number.
- [ ] 4.2 Add an offline test: with the `npc_dialogue` profile failing, the reply degrades to
      greeting/silence and no affinity changes.
- [ ] 4.3 Annotate substantive new tests with `covers_requirement` using canonical IDs from
...
- [ ] 4.4 Run the touched test labels (`world.ai`, `world.rules` npc-intents/affinity,
...
- [ ] 4.5 Run `openspec validate affinity-ai --strict` and
