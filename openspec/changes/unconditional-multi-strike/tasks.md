...
- [ ] 2.2 Synthetic behavior tests in a new module `world/rules/tests/test_unconditional_multi_strike.py`: the roll matrix, once-paid/once-practice, ordered projection, single terminal emission, nonlethal floor, atomic rollback, control-skill silence, and the construction matrix of the MODIFIED requirement's last scenario (both legal shapes construct; unknown kind / boolean / cap-2 / repeat_when-without-1 still raise). No catalog-row equality, key-set, or data-echo assertions anywhere (ratified NON-GOAL).
- [ ] 2.3 Register `world.rules.tests.test_unconditional_multi_strike` in exactly one shard (`rules-a`, beside `test_action_evidence`) in `.github/evennia-shards.json` (the wave's manifest appends sequence under supervisor); verify the ownership-contract test.
