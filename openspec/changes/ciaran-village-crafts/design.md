...
**Both new bundles touch the same two files the capital's shop changes touch.** `assortments.py` and `commerce.yaml` are edited by this change and by the capital's adornments and sanctum changes. The sections are disjoint but the files are not, so these three must not be worked in parallel.
