...
- Extend the catalog contract so an equipment item's codex numbers are checked against its rarity budget by test, not only by the loader at startup.
...
**The budget check reads a hand-authored fixture, not a parsed codex cell.** The inspect-only slice's codex parser only has to recognise one narrow shape — a table row whose first cell holds exactly one backticked key — and anything else is skipped by that rule alone. An adjustment cell is a different problem: real cells read `敏捷 -10%（生命上限 +15）` and `防禦 -4、快感 +20%、治療 +10%`, mixing Chinese field names, `、` separators, percent-versus-flat typing, and a parenthetical gauge-cap aside. A parser that mis-splits one of those under-checks a budget silently, which is the exact failure the check exists to prevent, and writing it well is the single task most likely to overrun the day. So the budget test compares the rulebook against a literal table of the 57 expected `(key, field, value, column)` tuples, hand-transcribed from the codex and reviewed as data. Transcription is a one-time cost that a reviewer can check line by line; a parser is a permanent liability that a reviewer has to trust. Alternative considered: parsing the cells with a narrow regex and failing loud on an unrecognised fragment. Rejected — "fail loud on anything unexpected" is the right discipline but still means maintaining a grammar for prose that the codex is free to reword.
...
- **Twelve unstocked items look like dead data until a storefront lands.** → They are reachable today through quest rewards, starting kits, presets, and the scenario director, all of which resolve against `ITEM_REGISTRY` directly. The spec pins this with a scenario so the state is intentional and tested rather than merely tolerated.
