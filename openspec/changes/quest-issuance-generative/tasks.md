...
- [ ] 1.1 Replace `CompiledQuest.issuer_branch_key` with an issuance descriptor carrying the issuer
  key and the settlement mode, keeping `reward` as it is.
- [ ] 1.2 `compile_quest_blueprint` maps the blueprint's declared issuer onto that descriptor,
...
- [ ] 1.3 Confirm the definition-key digest is unchanged — reward and issuer stay offer-level and
...
- [ ] 2.1 `register_generated_quest` dispatches on the issuer namespace: a guild issuance writes a
...
- [ ] 2.2 Preserve the all-or-nothing contract: preflight all three registries' equal/conflict states
...
- [ ] 2.3 `register_restored_quest` dispatches the same way.

## 3. Durable store

- [ ] 3.1 The `GeneratedQuestStore` payload carries the issuance (issuer key, settlement, reward)
  rather than a guild-shaped `offer` section; update the serializer and `payload_to_registrations`.
- [ ] 3.2 `_validate_restored_payload` checks the issuance binds the exact definition and that the
  namespace rules hold, raising loudly rather than registering a mismatched pair.
- [ ] 3.3 `restore_generated_quests` reconstructs and registers both kinds, definition first, and
  stays idempotent across repeated restarts.
- [ ] 3.4 Record that existing store contents are unreadable and must be cleared; write no migration.

## 4. Tests

- [ ] 4.1 Compile tests: a private commission compiles and registers into the issuance registry with
...
- [ ] 4.2 Registration atomicity tests: a conflicting issuance rolls back the definition and the
  requirements for both issuer kinds.
- [ ] 4.3 Restart tests: a restored private commission resolves its reward and settlement; restore is
  idempotent across repeated restarts for both kinds.
- [ ] 4.4 Annotate with `covers_requirement` against the modified `scenario-director` and
  `quest-lifecycle` requirement IDs; update `.github/evennia-shards.json` as needed.
- [ ] 4.5 Run the observability lint plus the focused compile, generated-store, scenario-director,
