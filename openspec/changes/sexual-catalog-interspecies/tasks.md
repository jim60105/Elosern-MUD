...
- [ ] 1.1 Confirm `world/skills/sexual_acts/interspecies.py` still ships `INTERSPECIES_ACTS = ()`,
...
- [ ] 1.3 Confirm `_act_family()`'s `_PARLESS_LINES = ("異種", "神之秘法")` check and its rejection of
...
- [ ] 1.4 Confirm `resolve_part`'s `Monster` collapse to `GENERIC_BODY_PART` (`world/rules/
  sexual_act_effects.py`) is unchanged.
- [ ] 1.5 Confirm `world/rules/rulebook/sexual.yaml`'s `experience_interspecies_added` row
...
- [ ] 1.6 Confirm `_handle_sexual_event`'s target-only application (`world/rules/action.py`) is still
...
- [ ] 2.1 Replace the empty `INTERSPECIES_ACTS = ()` with a `_act_family("異種", ...)` call
...
- [ ] 3.1 Add `interspecies_entangle` (`unlock={"hostile_act_count": 30}`, `actor_part="腰腹"`,
  `base_pleasure=18`, `actor_pleasure_ratio=0.7`, otherwise matching Tier 1's shape).
- [ ] 3.2 Add `interspecies_receive` (same unlock, `actor_part="私處"`, `base_pleasure=18`,
  `actor_pleasure_ratio=0.9`).

## 4. `world/skills/sexual_acts/interspecies.py`: Tier 3 (one act)

- [ ] 4.1 Add `interspecies_mating` (`unlock={"hostile_act_count": 30, "climax_count": 20}`,
...
- [ ] 5.1 Add `interspecies_domination` (`unlock={"interspecies_act_count": 20}`, `actor_part="大腿"`,
  `base_pleasure=22`, `actor_pleasure_ratio=0.6`, otherwise matching Tier 1's shape).
- [ ] 5.2 Add `interspecies_resonance` (same unlock, `actor_part="乳房"`, `base_pleasure=22`,
...
- [ ] 6.2 Add `world/skills/sexual_acts/tests/test_interspecies_catalog.py` (new `EvenniaTest`-based
...
- [ ] 6.3 Add one test (not in the delta spec, a design.md D-4 regression) proving
...
- [ ] 6.4 Apply `covers_requirement("sexual-catalog-interspecies::<id>")` (using the IDs from 6.1) to
...
- [ ] 7.3 Run `openspec validate sexual-catalog-interspecies --strict` and resolve any reported
  issue.
- [ ] 7.4 Confirm no file outside `world/skills/sexual_acts/interspecies.py` and the new
