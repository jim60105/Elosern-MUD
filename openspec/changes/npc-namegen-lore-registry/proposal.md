...
### New Capabilities
- `namegen-corpus-registry`: vendored 姓名語料的凍結承載——`NAME_PACK_REGISTRY`／`NAME_PACK_BY_RACE` 的形狀與種族綁定、import 時三條不變量（translit 全覆蓋、池非空＋映射合法、最長合成名過 `_validate_name`）、`・` 合成格式常量與 helper，以及經 `sync_all()` 的冪等 LoreRecord 鏡射。
...
- 新增 `world/lore/names.py`、`world/lore/tests/test_names.py`；修改 `world/lore/sync.py`（`_ALL_REGISTRIES` 一列）。
- `world/lore/names.py` 對 `world/rules/character_creation.py::_validate_name` 的依賴以驗證函式內延遲 import 落地（先例：`world/lore/titles.py` 對 rules 的延遲 import），避免 lore→rules 頂層依賴。
