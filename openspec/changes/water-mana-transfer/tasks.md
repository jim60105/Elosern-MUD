...
- [ ] 1.1 Inspect current source and exported references (codegraph/LSP) for `parse_effect`, `EffectPolicy`, `_resolve_effect_context`, `_handle_divine_drain` and the audience planner before editing; confirm the predecessor's writer API and 潮退 keys match ../water-spell-catalog/design.md.
- [ ] 1.2 Implement `ManaTransferEffect` parsing (closed directions; `fixed:<positive int>`, `fraction:` finite (0,1], `all`) and `ManaTransferPolicy` on `EffectPolicy` with DamagePolicy-posture fail-closed validation (share range, existing-buff marker keys, bonus-only-on-restore, coefficient rejection) per design D1, plus the validated `audience_condition` field (design D4) and the unconditional execution-tier bypass relaxation (`bypass_defense=True` with an empty predicate validates; multiplier-with-empty-predicate stays invalid).
- [ ] 1.3 Register the generic `mana_transfer` handler (`surfaces={"traits"}`): drain/restore legs stage one PendingEffect per mutated entity; drains through `remove_mp`/`apply_mp_change` with cast-skill attribution; caster share = round(actual drain × (authored share + bundle `recovery_share_bonus`)) capped at the actual drain; restore = authored base + per-key active-stack counts read on the CASTER via the buffs.py count helper; commit-time magnitude reads per design D2.
...
```sh
uv run --locked evennia test --settings test_settings.py --keepdb world.rules.tests.test_mana_transfer world.skills.tests.test_effects world.rules.tests.test_clock world.rules.tests.test_combat_modifiers world.rules.tests.test_conditional_damage
