...
- [ ] 1.1 Confirm whether change 12 (`map-anchor-grid`) and change 13 (`map-wilderness`) have been
...
- [ ] 1.2 Confirm, by source inspection, that no class named `InstanceRoom` exists anywhere in the
      repository yet, and that `world/maps/` (if change 12 has landed) contains no `instance.py`.
- [ ] 1.3 Confirm `world/rules/clock.py::_STAGE_ORDER`, `register_event_source`, `ScheduledEvent`, and
...
- [ ] 1.4 Confirm, by import, that `evennia.prototypes.spawner.spawn` resolves against the installed
...
- [ ] 2.1 Add `InstanceRoom(SceneArchetypeMixin, DefaultRoom)` per design.md D-1, with the six
      persistent attributes (`expire_tick`, `named`, `interacted`, `pin_reasons`, `owned_entities`,
      `origin_room`) and their documented defaults.
- [ ] 2.2 Implement `at_object_receive` setting `interacted = True` only for `PlayerCharacter`
      instances, never unsetting it once `True`.
- [ ] 2.3 Implement `at_object_delete` refusing deletion (returning `False`) while `pin_reasons` is
...
- [ ] 2.4 Test: `InstanceRoom.__mro__` includes `SceneArchetypeMixin` and excludes `XYZRoom`/
      `WildernessRoom`; `scene_archetype` defaults to `None` and accepts an arbitrary string.
- [ ] 2.5 Test: all six persistent attributes default correctly on a freshly created `InstanceRoom`,
      and persist across a re-fetch from the database.
- [ ] 2.6 Test: `at_object_receive` sets `interacted` for a `PlayerCharacter` entering, does not for an
      `NPC`/`Monster` entering, and never unsets it once `True`.
- [ ] 2.7 Test: `at_object_delete` refuses deletion (room still exists, `.delete()` returns `False`)
      while pinned, and separately while a `PlayerCharacter` is present; succeeds once neither
      condition holds.
- [ ] 2.8 Test (regression guard for the corrected rule): `at_object_delete` does **not** refuse
...
- [ ] 3.1 Add `world/prototypes.py::INSTANCE_ROOM = {"typeclass": "typeclasses.rooms.InstanceRoom",
      "desc": "..."}` with no explicit `prototype_key`, per design.md D-7.
- [ ] 3.2 Create `world/maps/instance.py` (if `world/maps/__init__.py` does not already exist from
      change 12, confirm it does before adding this module) and declare
      `INSTANCE_PROTOTYPE_WHITELIST: tuple[str, ...] = ("instance_room",)`.
- [ ] 3.3 Test: after Evennia's module-prototype loading runs, a prototype with `prototype_key ==
      "instance_room"` is registered and resolves to `typeclasses.rooms.InstanceRoom`.
- [ ] 3.4 Test: `INSTANCE_PROTOTYPE_WHITELIST == ("instance_room",)` exactly.

## 4. spawn_instance_room (`world/maps/instance.py`)

- [ ] 4.1 Implement `_validate_prototype_parent(prototype)`, raising `ValueError` for a
      `prototype_parent` not in `INSTANCE_PROTOTYPE_WHITELIST`, per design.md D-7.
- [ ] 4.2 Implement `spawn_instance_room(origin_room, prototype, *, exit_key, return_key,
      ...
      create the two plain `Exit` objects (no custom `at_traverse`).
- [ ] 4.3 Load `world/rules/rulebook/instance.yaml` into an `INSTANCE_YAML` module-level dict, mirroring
      `clock.py`'s own `CLOCK_YAML` loading pattern (task group 6 creates the YAML file itself).
- [ ] 4.4 Test: a whitelisted prototype spawns an `InstanceRoom`; a non-whitelisted one raises
      `ValueError` before `spawner.spawn()` is ever called (assert via a spy/mock or by asserting no
      new object was created).
- [ ] 4.5 Test: `expire_tick` equals `get_world_clock().tick + INSTANCE_YAML["default_ttl_seconds"]`
      when `ttl_seconds` is omitted, and `get_world_clock().tick + ttl_seconds` when given explicitly.
- [ ] 4.6 Test: `named` reflects the caller-supplied value, defaulting to `False`.
- [ ] 4.7 Test: exactly one `Exit` is created at `origin_room` (keyed `exit_key`, destination the new
...
- [ ] 4.8 Test: a character can traverse from `origin_room` into the spawned room and back to
      `origin_room` using ordinary Evennia exit-traversal commands, ending up in the identical
      `origin_room` object.
- [ ] 4.9 Test (Fix 2): `spawn_instance_room()` called with `origin_room` set to an existing
...
- [ ] 5.1 Implement `pin_instance_room(room, reason: str)` and `unpin_instance_room(room, reason: str)`
      per design.md D-8, operating on `room.db.pin_reasons` as a de-duplicated list.
- [ ] 5.2 Test: pinning twice with the same reason leaves exactly one entry; pinning with two distinct
      reasons leaves both.
- [ ] 5.3 Test: unpinning removes only the matching reason, leaving any other reason intact.
- [ ] 5.4 Test: unpinning an absent reason does not raise and leaves `pin_reasons` unchanged.
- [ ] 5.5 Implement `register_owned_entity(room, entity)` per design.md D-6/D-8, appending `entity` to
      `room.db.owned_entities` if not already present.
- [ ] 5.6 Test: registering the same entity twice leaves it in `owned_entities` exactly once.
- [ ] 5.7 Test: an entity never passed to `register_owned_entity()` is absent from `owned_entities`.

## 6. default_ttl_seconds rulebook data (`world/rules/rulebook/instance.yaml`)

- [ ] 6.1 Create `world/rules/rulebook/instance.yaml` with `default_ttl_seconds: 345600`, per
      design.md D-9's arithmetic (`4 * hours_per_day * seconds_per_hour`).
- [ ] 6.2 Test: `INSTANCE_YAML["default_ttl_seconds"] == 345600`, and equals
...
- [ ] 7.1 Implement `reclaim_due_instances(start_tick, end_tick) -> list[ScheduledEvent]` per
      ...
      see task 2.3's own correction note; do not reintroduce the blocking defect here.
- [ ] 7.2 Implement `_relocate_to_default_home(entity)` per design.md D-6: resolve
...
- [ ] 7.3 Implement `_clear_non_player_entities(room)` per design.md D-6: for every
...
- [ ] 7.4 Test: a due room with a `PlayerCharacter` present is deferred — still exists afterward,
      `expire_tick` unchanged, a `ScheduledEvent("instance_reclaim_deferred", ...)` is emitted.
- [ ] 7.5 Test: a due, unpinned room with no `PlayerCharacter` present is deferred identically when
      pinned.
- [ ] 7.6 Test (regression guard, task's own named blocking defect): a due, unpinned room containing
...
- [ ] 7.7 Test: a due room with no `PlayerCharacter` present and no pin, with `named == True and
...
- [ ] 7.8 Test: a due room with no `PlayerCharacter` present and no pin, with only one of
      `named`/`interacted` `True` (both combinations), is reclaimed, not promoted.
- [ ] 7.9 Test: a due, unpinned, unnamed room with no `PlayerCharacter` present is reclaimed — no
...
- [ ] 7.10 Test (the blocking defect's own regression check, required by the review): a due, unpinned,
...
- [ ] 7.11 Test: within the reclamation in 7.10, an `NPC` previously passed to
...
- [ ] 7.12 Test: a promoted room is skipped entirely (no event emitted for it) on a subsequent
      `reclaim_due_instances()` call.
- [ ] 7.13 Test: every `ScheduledEvent` returned contains only plain, JSON-compatible payload values —
      no live object reference.
- [ ] 7.14 Test (defense in depth): monkeypatch or subclass a room so `at_object_delete` still returns
...
- [ ] 8.1 Append `"instance_reclamation"` to the end of `world/rules/clock.py::_STAGE_ORDER`, per the
...
- [ ] 8.2 Implement `world/maps/instance.py::register_instance_reclamation()`, calling
      `world.rules.clock.register_event_source("instance_reclamation", reclaim_due_instances)`.
- [ ] 8.3 Call `register_instance_reclamation()` from the same startup flow that already invokes change
...
- [ ] 8.4 Test: `world.rules.clock._STAGE_ORDER` is exactly the ten-entry tuple the delta spec pins,
      with `"instance_reclamation"` last.
- [ ] 8.5 Test: before `register_instance_reclamation()` runs, `WorldClock.advance()` across a due
...
- [ ] 8.6 Test: after `register_instance_reclamation()` runs, `WorldClock.advance()` across a due,
      unoccupied, unpinned, unnamed room's `expire_tick` boundary reclaims it within that single call.
- [ ] 8.7 Test (source-order / integration): simulating a server start results in
...
- [ ] 9.1 Write the test the `settlement-stage-order` delta spec's own scenario names: register a
...
- [ ] 9.2 In the same test module, construct the transposed order in isolation (calling the two
...
- [ ] 10.1 Write one `EvenniaTest`-based integration test that: builds an `origin_room`, calls
...
- [ ] 10.2 Write a second integration test with `named=False` for the same setup, confirming the room
...
- [ ] 10.3 Write a third integration test that is the end-to-end version of the review's own named
...
- [ ] 11.1 Run the full test suite added by this change and confirm every test passes.
- [ ] 11.2 Confirm (via `git diff`) that every edit to an already-landed file (`world/rules/clock.py`,
...
- [ ] 11.3 Confirm no file added or edited by this change contains a reference to `world/ai/` or an LLM
      call, per this project's single-writer discipline (this change touches no `world/ai/` file at
      all, but the check costs nothing).
- [ ] 11.4 Confirm `tests/test_contrib_matrix.py::MATRIX_IMPORTS` already covers `evennia.prototypes.
      spawner.spawn` (added by change 1) and needs no new row from this change.
- [ ] 11.5 Confirm `docs/superpowers/specs/2026-07-29-ai-mud-engine-design.md` §11's Phase 3 table
...
- [ ] 11.6 Run `openspec validate map-instance --strict` and `openspec validate --all --strict` and
      confirm both pass.
