...
- Inventory Storybook stories and focused Vitest component tests become the visual and behavioral acceptance surface. This change depends on completed `add-item-presentation-metadata`, `expose-inventory-item-presentation`, and `relocate-inventory-drawer-essentials` changes. The subsequent `restyle-inventory-equipment-slots` proposal owns square-slot styling.
