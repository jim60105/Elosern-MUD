...
- [ ] 1.1 Add a regression test fixing the current observable behavior of all four shipped usable
...
- [ ] 2.1 Create `world/rules/item_effects.py` with `ItemStat` (hp/mp/sp/pleasure), `ItemTargetScope`
...
- [ ] 2.2 Implement the verb-discriminated entry parser: exactly one of `stat`, `apply_status`,
  `remove_status` (D2). Verify the two-verb and verbless entries both fail — the
  `item-effect-rulebook` delta's first two scenarios.
- [ ] 2.3 Implement stat-adjustment validation: closed stat vocabulary, non-zero integer amount,
...
- [ ] 2.4 Implement status validation: `apply_status` accepts only a concrete `BUFF_DEFINITIONS` key
...
- [ ] 2.5 Implement two-way registry alignment: the rulebook's item-key set equals exactly the usable
  registry keys, and every entry declares at least one effect. Verify the orphan, missing, and empty
  scenarios all fail startup.
- [ ] 2.6 Reject any scope other than the acting entity, with a message naming
  `add-item-effect-targeting` (D5). Verify the delta's two scope scenarios.
- [ ] 2.7 Reject an item declaring two stat adjustments to the same stat (D3). Verify with a test
  asserting the load failure and its message.

## 3. Registry and rulebook migration

- [ ] 3.1 Delete `ItemEffectKey` from `world/lore/items.py` and slim `ItemUseMechanics` to
...
- [ ] 3.2 Re-key `world/rules/rulebook/item_effects.yaml` to `items:` by item key, migrating the four
  ...
  pinned values.
- [ ] 3.3 Update `world/lore/tests/test_items.py` and `world/tests/synthetic_data.py` for the slimmed
  mechanics. Verify both suites pass.
- [ ] 3.4 Rewrite `world/rules/tests/test_equipment_combat_wiring.py`: its `ItemUseMechanics(
...
- [ ] 4.1 Replace `ItemUsePlan`'s single-effect fields with `steps: tuple[ItemEffectStep, ...]`, each
...
- [ ] 4.2 Rewrite `preflight_item_use` to resolve the profile, compute one step per effect, and reject
...
- [ ] 4.3 Implement the reason-code fallback: the shared code when every ineffective step agrees, else
  `no_effect` (D4). Verify both new preflight scenarios plus the three pinned single-effect reason
  codes from task 1.1.
- [ ] 4.4 Add `SP_FULL`, `PLEASURE_FULL`, `NO_EFFECT`, `STATUS_BLOCKED`, and `NOTHING_TO_REMOVE` to
  ...
  test over the enum).
- [ ] 4.5 Rewrite `_apply_plan` to walk the ordered steps, dispatching each verb to its shared entry
  ...
  branch" scenario.
- [ ] 4.6 Delete `_EFFECT_GAUGES`, `_FULL_REASON_BY_GAUGE`, `_GAUGE_NOUN_ZH`, and the
  `BLESSED_CLEANSE` branches. Verify the module no longer imports `ItemEffectKey`.
- [ ] 4.7 Add the sexual-state surface to `ItemTouchedJournal` so a pleasure write can be rolled back
...
- [ ] 5.1 Emit one `item_used` entry per effective step, dropping `effect_key` and carrying
  ...
  order.
- [ ] 5.2 Build each entry's Traditional Chinese text from the stat or status involved, with no
...
- [ ] 6.1 Add tests for a negative stat adjustment clamping at zero and reporting the actual drop —
  the delta's "A negative stat adjustment stops at zero" scenario.
- [ ] 6.2 Add tests for a negative pleasure adjustment: it clamps at zero and the arousal, wetness,
  and climax cascade does not fire on a reduction (design Risks).
- [ ] 6.3 Add tests for `apply_status` reaching `apply_buff`, including that an equipment-immunized
  debuff is refused, counts as ineffective, and reports `STATUS_BLOCKED` when it is the item's only
  effect.
- [ ] 6.4 Implement the source-qualified instance key for `apply_status` (D5b): pass
...
- [ ] 6.5 Add tests for `remove_status` selectors reporting `NOTHING_TO_REMOVE` when they match
  nothing, and for the `negative` selector still reporting `no_debuffs` (D5c).

## 7. Documentation and validation

- [ ] 7.1 Rewrite 第六層 of `docs/lore/items.md` for the new field model, and update the pending-effect
...
- [ ] 7.2 Update `docs/development/adding-items.md`: the §1 data-location table, §2 question 4, and the
...
- [ ] 7.3 Run the full deterministic suite plus task 1.1's regression test, and verify all four shipped
  items reproduce their pinned behavior exactly.
- [ ] 7.4 Run `openspec validate add-declarative-item-effects` and the repository's lint/observability
