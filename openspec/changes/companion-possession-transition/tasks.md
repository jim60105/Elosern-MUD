...
- [x] 1.1 Wire `retire_sequence` (dispatcher) + `reset_client_sequence` (ingress; the verified
...
- [x] 2.1 `world/rules/possession.py::_transfer_puppet`: retire/epoch-bump → additive
...
- [x] 2.2 `_mount_cmdset` / `_unmount_cmdset`: derived character act cmdset with the denylist
  (switcher family + PlayerCharacter-only panel commands); NPC default rebuild on release.
- [x] 2.3 `typeclasses/npcs.py::LLMNPC.at_pre_puppet`: accept the possession account (permit-only
  override, no re-verification).
- [x] 2.4 `_release` in `release_possession`: unpuppet B → strip grant → re-puppet A with the
  ladder; mid-failure keeps state, logs `step="possession_release"`, fixed return line.

## 3. Hooks and rendering

- [x] 3.1 `typeclasses/accounts.py::Account.at_post_disconnect`: call
...
- [x] 3.2 A's room-content display hook: 呆立入神 line when `db.possession` non-null.
- [x] 3.3 Reload-survival: no code (attributes persist); pinned by test 4.4.

## 4. Tests

- [x] 4.1 Swap mechanics (EvenniaTest, real sessions): verified swap leaves B puppeted / A
...
- [x] 4.2 Cmdset: 歸位 reachable while possessing; switcher family absent; denylist pinned against
  the landed `CharacterCmdSet`; default restored on release.
- [x] 4.3 Release ladder: clean return; refused-return keeps state + facade error event + fixed
  line; idempotent retry completes.
- [x] 4.4 `Account.at_post_disconnect` releases everything (simulate the disconnect lifecycle);
...
- [x] 4.5 Entranced rendering row (mirror set vs null).
- [x] 4.6 `covers_requirement` annotations; new/renamed test modules registered in
  `.github/evennia-shards.json`.

## 5. Verification

- [x] 5.1 Focused: `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings
  test_settings.py --keepdb world typeclasses web.webclient`.
- [x] 5.2 `tools.spec_traceability check`; `tools.observability_lint check`;
