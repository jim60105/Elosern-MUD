...
- 不動:`docs/game/commands.md`(`creation.roll_name` 是 UI action 不是玩家命令,判斷記錄於 design.md)、`character-creation-ux`(Telnet 精靈不收集 sex,缺省走 `DEFAULT_SEX`,語意不變)、`.github/evennia-shards.json`(全部測試落點皆已登記)
