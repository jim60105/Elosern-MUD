...
- `ui_action_result` envelope 的原始形態是 exact 七鍵(+ error 時的 `correlation_id`):`_send_action_result` 构造、`presentation/protocol.py::validate_ui_action_result` 服務器端驗證、`protocol.js::validateActionResult` 鏡像,三處同步。success 條件性 `data` 槽由前置 change `oob-result-data-slot` 在這三處落地——本 change 動工時該槽已存在且其 focused 測試(「A success result may carry an adapter data slot」等 scenario)已綠,本 change 只做消費端斷言。
...
4. 作為 `oob-result-data-slot` 已交付 `data` 槽的第一個消費者,以端到端測證明 success 結果携槽往返不破壞既有結果流(槽的雙端鏡像驗證本身屬前置 change,不在本 change 工作包)。
...
`build_custom_form()` 加 `sex: tuple[{key,label}, ...]`,由 `SEX_VALUES` 序 + 服務器標籤表(`female→女性`、`male→男性`、`other→其他`)派生;標籤住在 `world/rules/creation_wizard.py`(設計:「由 panel view 送出,前端不寫死」)。綁定:每 key/label 1..64 code points,列表 ≤8、非空。前端 `<select>` 選項順序即此列表順序,預設選中 `DEFAULT_SEX` 對應項(比較 key,不比標籤)。
...
- 擲名用**未種子化 `random.Random()`**,依設計 §4 原文「模組級無種子 `random.Random()`」定案為:`creation_actions.py` module 層級唯一的私有實例 `_ROLL_NAME_RNG = random.Random()`,**只在 `creation.roll_name` 的 roll 路徑讀取**(其他 action 零觸及)。設計 §4 的「模組級」是與 NPC 側「呼叫端自建種子實例」相對的語意(UI 骰不需要重放,故無種子;每次呼叫新建實例屬實作自由、非契約要求,且高頻 `time` 種子下相鄰呼叫間隔可能小於時鐘解析度)。單一私有實例把無種子 randomness 的 blast radius 鎖在一條 action 路徑,同時避免每次呼叫構造實例。
- 零持久寫、不刷 panel、不動草稿:result-only。`success` + `data: {display_name}`;名稱恒過 `_validate_name`(roller 保證 ≤64 + 界內字元)。
...
- `creation_menu.js`:`customPayload()` 加 `sex`(未選/預設 → 不帶鍵,靠服務器歸一;選了就帶);draft→state 還原讀 `draft.sex`;擲名 payload 純函數進 UMD(可 node --test)。
...
新 requirement 文字内以代號引用既有符號(`SEX_VALUES`、`DEFAULT_SEX`、`roll_name_for_race`、`creation.roll_name`、`data` 槽、`creation-roll-name` testid),與 D7 測試一一對映;`tools.spec_traceability list`/`check` 沿用上游 change 的驗證流程。

## Risks / Trade-offs

- **[結果 envelope 擴充觸及全部結果流]** → 該風險由前置 change 承擔(`data` 僅 success 可存在且默認缺席、其鏡像驗證場景與回歸測在該 change 落地);本 change 只新增第一個回 `data` 的 adapter,端到端測斷言其餘結果流不變。
- **[panel/draft 雙版本 bump 造成鏡像漂移]** → 既有雙向 parity 測試(`test_registry`/`ui_contract`)天然鎖定;版本常量各一處(服务端 `creation.py`、JS `protocol.js`)。
- **[回填竞態(擲名返回前玩家已打字)]** → 契约上回填是覆寫(同概念 fill),玩家随后可再改;Vitest 斷言只在 request-id 配對 success 時回填。
...
- **[null race 兜底名來自哪個包]** → 規則層契約:候選是 `sorted(NAME_PACK_BY_RACE.values())`(human／elf／orc;dwarf／halfling 不參與);adapter 測以「結果名零件恒落三包零件集」斷言,防止未來綁族悄悄擴大 UI 兜底面。
- **[`entity.sex` 寫入與 loader 雙路徑]** → 兩者都寫 `SEX_VALUES` 成員,語意同源常量,無分叉。

## Migration Plan

單次部署:後端(schema v4 + DRAFT_VERSION 3)+ 前端同版發布;殘留 v2 草稿被載入端視為不存在(玩家從頭填,未發布可接受);舊 v3 客戶端由 `CREATION_SCHEMA_VERSION` 鏡像閘門拒絕並要求重新載入。回滚 = 回退兩端版本;無資料遷移需反做(v3 閘門同樣拒新版草稿,行為一致)。
