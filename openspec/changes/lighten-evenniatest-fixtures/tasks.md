...
- [ ] 1.1 Run the read-only AST scan (from `plans/002-lighten-evenniatest-fixtures.md` Step 1, adapted to current paths) producing the candidate list; confirm the count is 200–210 classes
- [ ] 1.2 Manual exclusion pass, recording each exclusion with its reason in a
...
- [ ] 2.1 For each package (`world/rules`, `world/quests`, `world/skills`,
...
- [ ] 2.2 Revert (base + import) any class that fails after downgrade; record
      it in the change summary; expect 0–5 reverts

## 3. Contract pinning

- [ ] 3.1 Create `tests/test_fixture_base_contract.py` (new top-level file):
...
- [ ] 3.2 Run `uv run --locked -m unittest discover -s tests -t .`

## 4. Full-suite and static verification

- [ ] 4.1 `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings
...
- [ ] 4.2 Serial handoff profile once:
      `MUD_TEST_SETTINGS=1 uv run --locked evennia test --settings
      test_settings.py --keepdb commands server typeclasses world
      web.webclient`
- [ ] 4.3 `uv run --locked python -m tools.spec_traceability check` and
      `git diff --check`

## 5. OpenSpec and handoff

- [ ] 5.1 `openspec validate lighten-evenniatest-fixtures --strict`
- [ ] 5.2 Sync the delta spec into
