...
- [x] 1.2 Extend validated state reactions with actual HP-loss/new-negative-instance inputs and tier-based gains; verify alternate synthetic passive declarations without spell-key or profession-name branches.
- [x] 1.3 Enumerate all real negative-HP writers using codegraph/LSP and integrate each once, including items, rulebook and periodic rates; persist periodic source tier and verify no gain for miss/zero loss/immunity/refresh/resource cost.
- [x] 1.4 Add the recovery-only passive adjustment through existing combat modifiers and snapshot it into recovery profiles; verify equipment-independent benefit, snapshot stability, fractional numeric grants and no direct-heal double multiplier.
- [x] 1.5 Capture transitive phase/buff/state reactions in every initiating transaction; verify an item and a clock late-failure scenario plus a successful feedback-triggered action lock.
...
- [x] 2.5 Run tools.test_data_lint check, tools.spec_traceability check and openspec validate light-cleric-feedback --strict. For main-spec synchronization in the separately authorized workflow, obtain IDs using tools.spec_traceability list and annotate only tests that establish those requirements; verify no unknown IDs or uncovered main requirements are hidden. Keep all tasks unchecked until their described proof exists.

### Focused invocation

```sh
uv run --locked evennia test --settings test_settings.py --keepdb world.rules.tests.test_damage_state_feedback world.rules.tests.test_recovery_profiles
uv run --locked python -m tools.observability_lint check
uv run --locked python -m tools.test_data_lint check
uv run --locked python -m tools.spec_traceability check
openspec validate light-cleric-feedback --strict
```
