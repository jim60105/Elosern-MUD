...
- [ ] 1.2 Extend validated state reactions with actual HP-loss/new-negative-instance inputs and tier-based gains; verify alternate synthetic passive declarations without spell-key or profession-name branches.
- [ ] 1.3 Enumerate all real negative-HP writers using codegraph/LSP and integrate each once, including items, rulebook and periodic rates; persist periodic source tier and verify no gain for miss/zero loss/immunity/refresh/resource cost.
- [ ] 1.4 Add the recovery-only passive adjustment through existing combat modifiers and snapshot it into recovery profiles; verify equipment-independent benefit, snapshot stability, fractional numeric grants and no direct-heal double multiplier.
- [ ] 1.5 Capture transitive phase/buff/state reactions in every initiating transaction; verify an item and a clock late-failure scenario plus a successful feedback-triggered action lock.
