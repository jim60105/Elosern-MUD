...
2. `world/imports/validate.py` — `_check_entity_key_contract` adds `is_reserved_player_stable_key(key)` → rejection "digit-only entity keys are reserved for player characters (portrait stable-key collision)", mirroring the schema for the shared-contract philosophy.
