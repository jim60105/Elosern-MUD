...
- [ ] 1.1 Create `world/rules/lore_knowledge.py` with `CODE_CATEGORIES` — a closed mapping from
...
- [ ] 1.2 `record_lore_reveal` semantics: append-only namespaced set on `player.db.lore_discovered`,
      ...
      the record).
- [ ] 1.3 `list_discovered` returns deterministic (mapping order, then key) pairs; malformed record
      degrades to an unavailable diagnostic without reset or fabrication.
- [ ] 1.4 `lore_card` renders exactly each category's declared card fields (e.g. race key +
      description, region `terrain_flavor_zh`); unresolvable key raises a named error.
- [ ] 1.5 Pure unit tests: closed mapping (one registry per category), first reveal, repeat no-op,
...
- [ ] 2.1 Extend the `npc_dialogue` per-kind semantic validator so `reveal_lore` accepts exactly
...
- [ ] 2.2 Implement `_apply_reveal_lore(npc, player, intent)` in `world/rules/npc_intents.py`:
...
- [ ] 2.3 Remove `reveal_lore` from `_FORWARD_DECLARED_KINDS`, leaving the tuple empty; wire the
      kind into `apply_npc_intent` dispatch.
- [ ] 2.4 Applier tests: success records the discovery; unknown category / unresolvable key
...
- [ ] 3.1 Add the `lore` command to the character cmdset: `lore` lists discovered entries grouped
...
- [ ] 3.2 Update `docs/game/commands.md` and `docs/game/command-reference.md` for `lore` and update
...
- [ ] 3.3 Command tests: listing groups, card rendering, undiscovered/unknown not-found line,
      no registry-existence leak.

## 4. Spec and contract updates

- [ ] 4.1 Sync the `npc-dialogue` main spec from the delta (reveal_lore payload shape and
...
- [ ] 4.2 Annotate new `lore-knowledge` main requirements with `covers_requirement` (literal IDs
      from `python -m tools.spec_traceability list`) and keep `spec_traceability check` green.
- [ ] 4.3 Run the affected Evennia test domains (world.rules, world.ai dialogue-layer tests) and
      confirm existing guild/quest/affinity suites stay green.
- [ ] 4.4 Run `openspec validate --change lore-knowledge-codex --strict`.
- [ ] 4.5 Run the affected test entry points with one shared `OPENSPEC_TEST_EVIDENCE` path, then
