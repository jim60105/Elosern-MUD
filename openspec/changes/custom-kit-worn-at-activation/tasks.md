...
- [ ] 1.1 Extract the wearable-set rules (duplicate keys, `equipment_slot is None`, two keys
...
- [ ] 1.2 Make `_validate_starting_kit` (`world/lore/starting_kits.py`) call the helper after its
...
- [ ] 1.3 Refactor `_validate_preset_starting_equipment` to keep its malformed-entry guards and
...
- [ ] 2.1 Replace `starting_equipment: tuple[str, ...] = ()` at
...
- [ ] 2.2 Confirm snapshot-set completeness for the custom path rather than assuming it:
...
- [ ] 2.3 Confirm the shared toggle loop (~line 733), the `write_observer("starting_equipment")`
...
- [ ] 3.1 Replace `test_custom_activation_leaves_every_equipment_slot_empty` in
...
- [ ] 3.2 Add the load-failure test to `world/lore/tests/test_starting_kits.py`: a deliberately
      ...
      scenario.
- [ ] 3.3 Verify the existing custom-kit tests whose assertions stay true —
      ...
      emptiness itself.
- [ ] 3.4 Run the canonical suite plus traceability and data lint, all green:
...
- [ ] 4.1 Grep `docs/` and openspec-adjacent docs for "entirely unequipped", "unequipped", and the
      ...
      hit needing an edit).
- [ ] 4.2 State in the change notes that the command surface does not change, so
