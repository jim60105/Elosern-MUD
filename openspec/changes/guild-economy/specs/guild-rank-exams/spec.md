...
or examination. `requested_by` SHALL be audit metadata and SHALL NOT bypass validation.
...
- **THEN** one deterministic exam record, temporary opponent, and guild-exam combat session are created
...
- **THEN** it is rejected identically to the command request

#### Scenario: Duplicate active exam is rejected
- **WHEN** an actor with an active exam requests another
- **THEN** no second opponent, record, or session is created
