...
seed spell for 水 magic. Without the
other 10 水-element spells, players who invest in 水 magic have no tier progression to
...
- `skill-registry`: `SKILL_REGISTRY` gains an ADDED requirement declaring the full ten-key 水-element
  spell set (tier, target, MP cost, and typed `effects`), and the pre-existing 水 anchor skill(s)
