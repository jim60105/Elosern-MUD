...
- [ ] 1.1 `world/rules/rulebook/professions.yaml`: `schema_version: 1` + the three shipped rows
...
- [ ] 1.2 Header comment stating the assembly-time-blueprint semantics (design R3 D1) and that
  `default_binding` is stored-not-read until service-anchoring (R3 D6 seam).

## 2. The loader

- [ ] 2.1 `world/rules/profession_config.py`: frozen dataclasses `ProfessionComponent(type_key:
...
- [ ] 2.2 Component-type vocabulary: closed dict `PROFESSION_COMPONENT_TYPES` mapping
  ...
  reads it.
- [ ] 2.3 Cross-validation: schedule keys from `world/rules/npc_schedules.py`'s loaded template
...
- [ ] 3.1 `world/rules/tests/test_profession_config.py` (`unittest.TestCase`, pure): shipped-file
...
- [ ] 3.2 Vocabulary contract test: every component class defined in `typeclasses/components.py`
...
- [ ] 3.3 `covers_requirement` annotations on the four requirements (IDs from
  `uv run --locked python -m tools.spec_traceability list`).
- [ ] 3.4 Register `world/rules/tests/test_profession_config.py` in exactly one shard of
...
- [ ] 4.2 `uv run --locked python -m compileall -q world` and
...
- [ ] 4.3 Repo-wide grep proving `default_binding` has no runtime consumer outside the loader and
  its tests.
