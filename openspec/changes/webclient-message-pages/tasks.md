...
- [ ] 2.2 Create `web/webclient-app/lib/message_pages.js`. It has no Vue or DOM import and depends only on `lib/narrative_markup.js` and `lib/box_drawing.js`. It exports `segmentResponses(lines, marks)`, `responseBlocks(response)`, `paginate(blocks, fits)`, `pageIndexForOffset(pages, offset)`, and `responseLength(blocks)`, as specified in design D2–D4. Include the file header comment naming the design doc §6.1–§6.2 and this change. Verify with `grep -n "from \"vue\"\|document\.\|window\." web/webclient-app/lib/message_pages.js`, which returns nothing.
- [ ] 2.3 Create `web/webclient-app/tests/message_pages.test.js`. The fake `fits` counts code points per line (for example, 10 characters per line and 3 lines per page, with a `break` ending a line). Cover:
...
- [ ] 3.1 `web/webclient-app/stores/elosern.js`: add `ctx.narrativeSeq = 0` and `ctx.responseMarks = ref([])`, and expose `responseMarks` in the store's returned object next to `narrative`.
- [ ] 3.2 `web/webclient-app/stores/elosern/transport.js`:
...
- [ ] 3.3 Create `web/webclient-app/tests/store/narrative_responses.test.js` against the real store with a stub sender. Cover:
...
- [ ] 5.2 Sync this change's deltas into the main specs and run `uv run --locked python -m tools.spec_traceability check`, which is green.
...
- [ ] 6.2 Run `openspec validate webclient-message-pages --strict` and `git diff --check`. Both clean.
