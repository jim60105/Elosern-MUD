...
- [ ] 1.1 In `world/skills/effects.py`, delete the `RevealStrength` enum and strip the `strength`
  ...
  nothing.
- [ ] 1.2 Simplify the `reveal_disguise` branch of `parse_effect` so the bare prefix returns the
...
- [ ] 2.1 In `world/rules/skill_effects.py`, drop the `strength` parameter from `reveal_can_pierce`
...
- [ ] 2.2 Drop the `strength` parameter from `reveal_disguise_effect`, keeping its
...
- [ ] 2.3 In `world/rules/action/effects/conferral.py`, stop threading a strength through
...
- [ ] 2.4 Verify the reveal path no longer reads the veil's companion record at all: neither
...
- [ ] 3.1 In `world/rules/skill_effects.py`, delete the `DISGUISE_PROVENANCE_DIVINE` and
...
- [ ] 3.2 Update `apply_divine_disguise` to call `record_cast_placement(entity)` and
...
- [ ] 3.3 Update the sole reader in `world/rules/action/effects/conferral.py` to
...
- [ ] 3.4 Rename the record's key in the three carrier sites that move it without deciding from it:
...
- [ ] 3.5 Re-point — do NOT delete — the boundary regression assertion in
...
- [ ] 3.6 Verify the old name is gone everywhere:
...
- [ ] 4.1 Delete the `unveiling_eye` skill definition from `world/skills/registry.py` and change
...
- [ ] 4.2 Change `true_name_sight`'s declared effect from `reveal_disguise:true_name` to the bare
...
- [ ] 4.3 Verify no shipped content granted the removed skill:
  `grep -rn "unveiling_eye" world/ commands/ typeclasses/ tmp/` returns nothing after the edits.

## 5. Behavior coverage for the defect

- [ ] 5.1 In `world/rules/tests/test_divine_veil_reveal.py`, convert the mundane-strength cases: the
...
- [ ] 5.2 Add the regression test this change exists for, over a SYNTHETIC divine-capable entity
...
- [ ] 5.3 Update `world/rules/tests/test_divine_veil_cast.py` to the renamed accessor and boolean
...
- [ ] 5.4 Verify the two halves did not interact by running both veil modules together:
...
- [ ] 6.1 Update `docs/lore/skill-trees/divine-mystery.md`: remove `unveiling_eye` from the veil-line
  ...
  returns nothing.
- [ ] 6.2 Re-point the `covers_requirement` annotations for the two REPLACED requirements: the
...
- [ ] 6.3 Verify the new coverage stays behavior-only and is not added to
...
- [ ] 7.1 Run the focused modules this change touches and verify they pass:
...
- [ ] 7.2 Run `openspec validate collapse-veil-reveal-line --strict` and verify it reports no errors.
- [ ] 7.3 Re-read the design's Non-Goals and verify the diff re-introduces no grade axis under any
