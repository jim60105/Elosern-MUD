...
**Reason**: 交談 no longer opens a scripted-keyword frame or borrows the command line for free-form dialogue; since `explore-talk-open-action` (C9a) it submits `explore.talk_open` directly (design §8.1), and the dock now returns to the overview when a conversation opens. The requirement's "交談 keeps the scripted keyword frame" scenario describes a frame that no longer exists, and a MODIFIED block cannot drop it.
