...
### D2b — Status-display census sync (shipped drift check, mandatory in this change)
`world/rules/status_display.py` builds `STATUS_DISPLAY` at import and fails closed (`MissingDisplayMetadataError`; `world/rules/tests/test_status_display.py` re-asserts set equality) unless `rulebook/status_display.yaml` covers EXACTLY the live buff definition keys ∪ combat-modifier rule IDs. The D2 census therefore obliges display rows inside THIS change: DELETE `dark_atk_down`; ADD `dark_weaken` (衰弱/harmful), `dark_spread` (詛咒擴散/harmful), `dark_apotheosis` (深淵神格侵蝕/harmful), `dark_corrosion_deep` (深層侵蝕/harmful), and `fear_locks_actions` (恐懼鎖定行動/critical, matching the `paralysis_locks_actions`/`suffocation_locks_actions` row convention). Existing codes (`dark_curse`, `dark_corrosion`, `fear`, `fear_agility_and_accuracy_penalty`) keep their entries — durations and numbers move, keys don't.
...
| 13-node registry block + dark buff/modifier rows + status-display census sync + echo-test retirement + shard manifest final state | dark-spell-catalog | — |
...
Shared-file schedule: `dark-erosion-leech` owns `world/rules/buffs.py` (engine) + its test module + shard row; `dark-self-recovery-missing-fraction` owns `world/skills/effects.py` + `world/rules/combat.py` + the registry coefficient-whitelist hunk + existing test modules; the catalog change edits only the registry dark block, `buffs.yaml`/`combat_modifiers.yaml` dark rows, `status_display.yaml` (D2b), the two echo-test files, and the shard manifest's last edits. Two files are touched by two changes each but strictly sequentially, never concurrently: `registry.py` (the coefficient-whitelist hunk merges inside change 2, the dark-block swap inside the catalog) and `buffs.yaml` (the leech ships zero rows; the catalog owns every row) — the conflict matrix is empty by ownership plus batch order.
