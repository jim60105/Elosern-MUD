...
- [ ] 1.1 Expose committed `party.slots` and the combat-participant identity→token join in the
  Vue store (no new protocol surface; pure derivation from committed panels).
- [ ] 1.2 New `PartyStrip.vue` per the draft `.comps` island: `同伴 N / 4` header; per-slot cell
...
- [ ] 2.1 New `PartyDrawer.vue` on the shared drawer shell: `N / 4` sub-count; compbig rows
...
- [ ] 2.2 Wire both in `AppClient.vue` under the shared visibility matrix and escape/return
  discipline (drawer returns to the quickbar).

## 3. Showcase lockstep

- [ ] 3.1 `component-manifest.json`: add `PartyStrip`/`PartyDrawer` entries;
...
- [ ] 3.2 `tests/overlays/deferred_surfaces_absent.test.js`: remove the Party entry +
...
- [ ] 4.1 Vitest: quickbar mirrors committed slots (names/bars/numerals/stage, invite padding,
...
- [ ] 5.1 Focused Vitest suites + `tools.spec_traceability check`.
- [ ] 5.2 Live container check 1600x900: quickbar cells match the draft geometry; 請其離隊
