...
- [ ] 1.2 Implement validated effect audiences and the pure shared preflight/final audience planner; verify mixed selected teams, self binding, one-empty and all-empty audiences.
- [ ] 1.3 Route effects at the existing dispatcher and update authoritative preview/delivery attribution; verify ordinary companion damage/enemy healing remain reachable and unselected entities stay untouched.
- [ ] 1.4 Include explicitly self-bound recipients in snapshots and avoid per-effect duplicate practice; verify relationship changes after preflight and a late mixed-cast failure.
