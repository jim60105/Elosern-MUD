# Delta spec: webclient-dialogue-session (webclient-align-07-dialogue-session-state)

Chain note: this delta owns the session-state requirement only. The `dialogue` panel and
mode requirements land in webclient-align-10-dialogue-panel, which chains a MODIFIED
restatement of this requirement that restores the panel-degradation and presentation clauses.
