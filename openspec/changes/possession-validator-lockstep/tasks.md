...
- [ ] 1.1 `web/webclient/presentation/exploration.py`: replace the private `ACTION_IDS` tuple
...
- [ ] 1.2 `web/webclient/presentation/options.py`: extend the existing
...
- [ ] 1.3 `options.py:88`: bare `validators[action_id]` → `.get` + structured rejection naming
  the code (ProtocolValidationError-shaped, same lineage as the sibling rejections).

## 2. Client mirror

- [ ] 2.1 `web/static/webclient/js/elosern/protocol.js`: add `explore.possess` +
...
- [ ] 2.2 Audit `web/webclient-app/src/protocol/**` for equivalent enumeration tables (static pass
...
- [ ] 3.1 EvenniaTest regression at the composition layer: room with a bound companion → render
...
- [ ] 3.2 Totality pin: the `_ACTION_PAYLOAD_VALIDATORS` key set ⊇ `ACTION_CODE_ALLOWLIST`; and a
...
- [ ] 3.3 Node gate: extend `web/static/webclient/js/tests/protocol.test.js` with accept/reject
...
- [ ] 3.4 Panel-derivation pin: `exploration` accepted-action set equals the shared allowlist.
...
- [ ] 4.2 `node --test web/static/webclient/js/tests/protocol.test.js`; if Vue sources changed:
  `pnpm test`.
- [ ] 4.3 `uv run --locked python -m tools.spec_traceability check` green with annotations on the
