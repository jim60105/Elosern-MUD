# Delta spec: webclient-context-actions (possession-validator-lockstep)

Affordance payload validation crashed with a bare `KeyError` for the two possession codes (no
registered validator), and the production client mirror's enumeration tables never gained them.
Payload validation becomes a total function over the vocabulary with a structured rejection for
out-of-vocabulary codes, and the client-mirror parity is pinned by fixtures. Both requirements
reproduced in full.
...
#### Scenario: An unavailable form differs only in schema version
