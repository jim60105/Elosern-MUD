...
- [ ] 1.1 `web/webclient/presentation/local_map.py`: `from world.maps import
...
- [ ] 1.2 Extend `web/webclient/presentation/tests/test_local_map.py`: wilderness payload legend
...
- [ ] 2.1 `web/webclient-app/components/MapLattice.vue`: replace the modulo state-cycle
...
- [ ] 2.2 Extend `web/webclient-app/tests/world/map_lattice.test.js` (or `local_map.test.js`
...
- [ ] 2.3 Audit and update fixtures/stories that pin legend content or length:
...
- [ ] 3.1 Extend the local-map browser class (`web/tests/browser/test_browser_local_map.py`
...
- [ ] 3.2 Annotate the two new requirement-owning tests with `covers_requirement` literal IDs
      from `uv run --locked python -m tools.spec_traceability list`;
      `... tools.spec_traceability check` clean.
- [ ] 3.3 Confirm no player-command surface change (none expected); focused Python runs
