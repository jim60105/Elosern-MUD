# Delta spec: game-command-docs (multichar-01-account-capacity)

Raising `MAX_NR_CHARACTERS` above 1 makes Evennia's stock `charcreate` a working command for
anyone holding Player permission, while `docs/game/command-reference.md` already documents both
`charcreate` and `chardelete` as 管理員 commands. This delta makes the mounted locks match the
documentation and extends the admin-permission scenario to cover them.
