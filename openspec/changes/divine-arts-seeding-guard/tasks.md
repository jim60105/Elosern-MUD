...
- [ ] 1.1 In `world/lore/player_presets.py::_validate_preset_disguised_stats`, reject a non-empty
...
- [ ] 1.2 Cover it with a behavior test over a SYNTHETIC preset registry, not the shipped cards
...
- [ ] 2.1 Add `_check_disguised_stats_race(record)` to `world/imports/validate.py` returning the
...
- [ ] 2.2 Extend `_check_skills` to reject a `skills`/`passives` key whose registry entry declares
...
- [ ] 2.3 Cover both with behavior tests over SYNTHETIC records in
...
- [ ] 3.1 Remove the `disguised_stats` block from `world/imports/examples/example_character.json`,
  keeping `race: "human"` per design D5. Verify with
  `uv run --locked -m world.imports.validate world/imports/examples/example_character.json`, which
  must report no issues.
- [ ] 3.2 Verify the reference-example contract still holds:
  `uv run --locked evennia test --settings test_settings.py --keepdb world.imports.tests.test_reference_example`.
- [ ] 3.3 Confirm no shipped preset card violates the new preset guard — the lore registry raises at
...
- [ ] 4.1 Add the two new rejection rows to the validator table in
...
- [ ] 4.2 Annotate the new tests with `covers_requirement` for the three ADDED requirements, taking
...
- [ ] 4.3 Verify the data-contract gate stays clean — the new tests use synthetic fixtures and must
  NOT be added to `tools/test_data_freeze.json`:
  `uv run --locked python -m tools.test_data_lint check`.

## 5. Verification

- [ ] 5.1 Confirm no new test module was created, so `.github/evennia-shards.json` needs no entry:
...
- [ ] 5.2 Run `openspec validate divine-arts-seeding-guard --strict` and verify it reports no errors.
- [ ] 5.3 Re-read the design's Non-Goals and verify the diff adds no runtime check inside a writer,
