...
情慾秘術是獨立於元素魔法與神之秘法之外的技能分類（`SkillCategory.SEXUAL_ACT`），與光明教會信仰體系、精靈族生理特質緊密相關。它不走「使用次數換算等級」的熟練度系譜樹（見 [總覽與機械規則](/lore/skill-trees/index) §2），而以**行為目錄＋終身計數器解鎖**的骨架運作。招式本身就是技能，能不能使出取決於這具身體做過什麼。本頁描述現行實作（`world/skills/sexual_acts/`）的完整分類結構。
...
階梯的敘事邏輯是每條線都是一部**身體史**。解鎖的依據是經歷而非研讀。目錄本身就是「性經歷即修行」這條世界觀機制的直接體現，也是 [光屬性系譜樹](/lore/skill-trees/light) 聖禮路線刻意與之分離（走熟練度）、只在狀態層共享興奮／高潮系統的原因。
...
`_act_family()` 在 import 期做結構驗證（門檻型別、部位合法性、禁用事件、pair_events 契約），目錄作者的錯誤會在載入時炸掉，不會拖到遊玩時才爆。

## 效果字串與事件詞彙

技能面 effects 清單由 builder 自動生成，包括 `pleasure:<key>`、`sexual_counter:<key>`、每個事件一條 `sexual_event:<name>`（參與者範圍）或 `sexual_event_actor:<name>`（施為者範圍），宣告 `pair_events` 時附一條 `act_pair_event:<key>`。
...
`resistible=True` 的招式在生效前經 `resist_verdict()`（`world/rules/sexual_resist.py`）做二元競賽。雙方分數混合有效敏捷與物理攻擊，抵抗方再取親和階級修正。擲骰之前有三條**自動服從短路**：
