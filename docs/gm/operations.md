...
世界資料、任務目錄與公會經濟會在伺服器啟動時以可重複執行的方式同步。內容異動後，先在本機驗證資料與測試，再重新啟動服務套用登錄資料。既有持久狀態由其對應模組管理，啟動同步不應被當成手動覆寫玩家進度的工具。部署調校值（sd-webui 生成參數、美術排程、webclient 載入旗標等）透過環境變數設定且載入時驗證——完整的三層設定模型與變數清單見[設定與環境變數](/development/settings-and-environment)。
