...
| `local-map`, `local-map-detail`, `local-map-remembered`, `local-map-remembered-mirror`, `local-map-edge-markers-mirror`, `local-map__<suffix>` (`local-map__lattice`, `local-map__legend`, `local-map__title`, `local-map__marker--current`, `local-map__unavailable`, `local-map__actionable`); H5 adds `local-map__expand` (the full-map control, mounted with the `MapOverlay` it opens) | H2, H5 | REMAP-TO-TESTID |
| `dock-menu`, `dock-crumb`, `dock-crumb__<suffix>` (covers the breadcrumb back control `dock-crumb__back`), `suggestions-generating`, `dock-tab-<tab>` (prefix `dock-tab`: the combat root's `dock-tab-flee`, `dock-tab-skills`; the exploration-root tabs `dock-tab-move`／`dock-tab-look`／`dock-tab-interact`／`dock-tab-suggestions` retired with the scene-overview swap), `action-dock-description`, `exploration-detail`, `exploration-rest-form` | H3 | REMAP-TO-TESTID |
...
| `map-overlay`, `map-overlay-content`, `map-overlay-unavailable`; desktop-nav adds the overlay's view controls `map-overlay-zoom-in`, `map-overlay-zoom-out`, `map-overlay-recentre`, `map-overlay-legend-toggle`, `map-overlay-legend-popover` and the `map-overlay-remembered` place list | H5, desktop-nav | REMAP-TO-TESTID |
| `settings-overlay`, `settings-overlay-text-speed-<speed>` (covers `slow`／`normal`／`fast`／`instant`), `settings-overlay-auto-advance` (the typewriter reading-prefs controls), `help-overlay`, `creation-<suffix>` (prefix `creation`: `creation-body`, `creation-submit`, `creation-confirm`, `creation-reset`, `creation-concept-indicator`, `creation-preset-card`, `creation-race`, `creation-field-<name>`), `overlay-host`, `overlay-host-close`, `overlay-host-body` | H5, typewriter | REMAP-TO-TESTID |
...
| `dialogue-pick`, `dialogue-freeform`, `dialogue-exit`, `narrative-head` (narrative dialogue surface: choice rows, the freeform speech row, exit action button, and card header pinned outside scroll area; added by the webclient-align-11-dialogue-ux change) | dialogue-ux | REMAP-TO-TESTID |
...
| `stage-band` (the full-width bottom band carrying the dock and message regions), `message-window` + `message-dialogue`, `message-page`, `message-page-marker`, `message-live`, `message-log-open` (the AVG message window: host, dialogue scroll area, page surface — carrying its page/pages count attributes — page marker, polite live region, and the shell's 日誌 control; webclient-message-window-swap re-home of the narrative feed), `dialogue-box` (the message window's dialogue-variant portrait box), `place-card` + `place-card__location`／`place-card__time` (the top-bar place card), `reference-artwork` (the stage's actor reference artwork figure), `nav-tools`, `nav-settings`, `nav-tool-lineage`, `nav-tool-lore`, `nav-tool-codex`, `nav-tool-help` + `gallery-opener` (the desktop navigation bar's tool cluster and its portrait-gallery opener), `character-switcher-name` (the top bar's current-character name node), `party-strip` + `party-strip__<suffix>` (prefix party-strip__: `party-strip__header`, `party-strip__count`, `party-strip__row`, `party-strip__avatar`, `party-strip__token`, `party-strip__hp-bar`; the party quickbar island), `objective-tracker` + `objective-tracker__more` (the tracked-objectives island), `gallery-panel`／`gallery-detail` (the portrait gallery overlay bodies), `scene-overview` (the exploration dock's root frame host), `verb-popover` (a person chip's verb popover card), `shop-panel` + `shop-panel__<suffix>` (prefix shop-panel__: `shop-panel__title`, `shop-panel__unavailable`, `shop-panel__open`, `shop-panel__shop-absent`, `shop-panel__stock-section`, `shop-panel__sellable-section`, `shop-panel__stock--<item_key>`, `shop-panel__sellable--<item_key>`; the shop drawer body) (the AVG-stage / contextual-HUD / services surfaces the managed suite targets; swept in at the frozen-audit renewal) | avg-stage | REMAP-TO-TESTID |
...
| `.action-dock__legend` and `.action-dock__legend kbd` (the dock's shortcut-legend strip — the visible `action-dock-description` element — and its styled key chips; targeted by `test_browser_shell_dock.py`) | REMAP-TO-TESTID |
| `.topbar-right` (the top bar's right cluster wrapper, measured against the desktop navigation's fit; targeted by `test_browser_contextual_hud_stage.py` and `test_browser_shell_surfaces.py`) | PRESERVE-SAME-HOOK |
...
### frozen-audit-renewal-sweep — 2026-09-26（master 凍結契約復綠）

- **§2.3 受管目標全數涵蓋：** `tests/test_webclient_frozen_contract.py` 的受管瀏覽器目標掃描在 C4–C9 波次後回報 41 個未涵蓋識別碼（外加同一 legend 掛鉤的 `.action-dock__legend kbd` 複合選取器變體，共 42 個失敗子測試）；經逐一核對，全部識別碼在現行 app 中仍然存活（每個都由 `web/webclient-app/components/` 的 `data-testid` 或受管套件鎖定的 CSS 類別提供），因此本次只補登、不退役任何仍活的掛鉤。新增／展開的列：`stage-band`、`message-window` 家族（`message-dialogue`、`message-page`、`message-page-marker`、`message-live`、`message-log-open`）、`dialogue-box`、`place-card` 家族、`reference-artwork`、`nav-tools`／`nav-settings`／`nav-tool-*`＋`gallery-opener`、`character-switcher-name`、`party-strip` 家族、`objective-tracker` 家族、`gallery-panel`／`gallery-detail`、`scene-overview`、`verb-popover`、`shop-panel` 家族（avg-stage 列）；`map-overlay-*` 檢視控制與 `map-overlay-remembered`；`local-map-remembered-mirror`；`overlay-host-body`；`settings-overlay-text-speed-<speed>`／`settings-overlay-auto-advance`；`dialogue-freeform`；CSS 類別表新增 `.action-dock__legend`（含其 `kbd` 子元素）與 `.topbar-right`。
- **§2.3 退役子項目：** H3 列的 `suggestions-section` 移除（建議 root row 現為場景總覽的 footer chip，legacy root section 元素早已不存在，僅剩消極式 absence 斷言）；`dock-tab-<tab>` 前綴列改註明 `dock-tab-move`／`dock-tab-look`／`dock-tab-interact`／`dock-tab-suggestions` 四個探索 root tab 已隨 webclient-scene-overview-swap 退役，保留實存的 `dock-tab-flee`／`dock-tab-skills`。
- **§5 完整性聲明維持成立：** 本項為補登與退役子條目的文件同步，未改變凍結 façade（§1）或增量清單（§3）；`test_every_managed_browser_target_is_frozen` 與 re-mapped 集完整性檢查自此在 master 轉綠。
