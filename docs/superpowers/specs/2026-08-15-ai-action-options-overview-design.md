...
| D-4 (review fix) | **Session-scoped options presentation state.** `session.ndb.options_state` (fingerprint, status, generation token, displayed set) is owned by the trigger service and read by every `context_actions` render. | An async `ready` result survives the next snapshot; dismiss state survives re-renders (review R3). |
| D-5 (review fix) | **Pending registry + per-session tokens.** `pending[fingerprint] → [PendingSubscriber(session, token)]`; a dismiss invalidates only its own session's subscriber and token. | "One LLM call per fingerprint" holds mid-flight across multiple windows; evict-vs-generation is deterministic per session (review R4/R15). |
| D-6 (review fix) | **No `dialogue` kind in v1.** Conversation is steered by the exploration kind's `talk_scripted`/freeform affordances; there is no persistent dialogue session from which a "current conversation partner" is derivable. | Removes the provenance problem and the degraded-empty-card contradiction (review R5). |
| D-7 (review fix) | **Eligibility digest in the fingerprint.** The fingerprint includes the digest of the canonical eligible-affordance list (action_id + params, labels excluded), so schedule-gate flips, exit locks, and monster/object changes invalidate cached proposals. | The exact-match promise holds by construction: a cached card can never name an action that stopped being current (review R13/R2). |
| D-8 (review fix) | **Card-count bounds are status-dependent.** Validation accepts 0–5; the 3–5 minimum is a *generation* rule for AI `ready` sets. | Degraded rooms with nothing actionable render an empty-state line instead of failing validation (review R14). |
...
| 7 | `context-actions-suggestions` | 1 | Server validator extension adding the `suggestions` section (combat fields preserved, available + unavailable forms), `PANEL_ALLOWLIST` → 5, `protocol.js` mirror, presenter reading `context.options_state` snapshot, parity test. **Roadmap amendment (recorded in change 1's proposal/design):** `context_actions` is already v3 (combat) since the skill-category-combat-panel change; change 1 (`action-options-affordance-contract`) lands the exploration available form as **v4**; this change therefore bumps to **v5** — the earlier "context-actions-v3 / PANEL_ALLOWLIST → 3" wording is superseded |
