...
2. **單一事實來源**：所有清單（技能、状態、任务、同伴、図鑑）消費既有 payload schema，
...
| 局部地圖/小地图 | 節點網格（current/已訪/未訪/記憶）；fog-of-war | 點開全地圖 | **戰鬥時隱藏** |
| 目標追蹤 | 1–3 追蹤任务目標＋路徑 | 點開任務 | 右下角；戰鬥收縮為純目標 |
| 同伴快帶 | ≤4 同伴（肖像/HP/狀態/羈絆 stage）| 點開同伴面板 | **無專面板＝目前缺口，補上** |
| 通知 toast | 事件（升级/任务完成/解鎖/金錢）| 點掉 | 優先級隊列，max 4 |
...
**D. 服務 surface**（由探索 進入）：
- **公會**：註冊 / 任務板(接取) / 任務紀錄(回報·放弃[確認]·詳情) / 功·階(考核)
...
| **任務** | 追蹤/進行中/完成/失敗；目標 checklist(計數 3/10)；報酬(銅/物品/经验)；追蹤/放弃[確認] | 分類 |
...
**戰鬥差異（關鍵 UX）**：小地图消失；同伴快帶變成「參戰 party/foes 框架」；
