...
| [`possess`](/game/command-reference?id=possess) | 附身至同伴身上行動。 |
| [`unpossess`](/game/command-reference?id=unpossess) | 結束附身並歸位。 |
...
| 指令 | 說明 |
| --- | --- |
| [`cast`](/game/command-reference?id=cast) | 施放技能；持有屬性主宰者可搭配 `@<scale>` 比例調整威力與 MP 消耗。探索中瞄準同場怪物會直接開戰並以該技能為首個行動；傷害技能只能瞄準怪物。具狀態或接觸條件的法術需滿足條件方可施展；性愛技能亦屬可施放之列，隨遊玩解鎖後可經 `combat actions` 的分類檢視。 |
