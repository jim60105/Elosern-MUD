...
class ChoicePointsBrowserTest(BrowserAcceptanceTest):
    ...
    def test_stream_freeform_card_sends_the_label_speech_once(self):
        ...
        # complete-ui-command-echo: a freeform stream card is a deliberate
        # mutation, so its dispatch echoes exactly one `talk <NPC> <speech>`
        # line — the central descriptor fill composing the committed display
        # name — and never a second raw echo.
        _wait_inp_line(
            page,
            inp_before + 1,
        ...
        self.assertEqual(
            self._narrative_inp_count(page),
            inp_before + 1,
            "a freeform stream card dispatch echoes exactly one command line, never a second",
        )
        self.assertEqual(
